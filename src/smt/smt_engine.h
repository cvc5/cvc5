...
/* -------------------------------------------------------------------------- */
  ...
  class CVC5_EXPORT SmtEngine
  {
   ...
   public:
    ...
    /**
     * Helper method for the API to put the last check result into the
     * statistics.
     */
     ...
     * query. Only permitted if produce-models is on, and the block-models
     * option is set to a mode other than "none".
     *
     ...
     * location type and dataT is the data type for the heap. This command
     * should be executed only once, and must be invoked before solving
     * separation logic inputs.
     */
     ...
     *
     * The given sygusType may not correspond to the actual function type of
     * func but to a datatype encoding the syntax restrictions for the
     * function-to-synthesize. In this case this information is stored to be
     * used during solving.
     *
     * vars contains the arguments of the function-to-synthesize. These
     * variables are also stored to be used during solving.
     *
     ...
     *
     * Invariant constraints are not explicitly declared: they are given in
     * terms of the invariant-to-synthesize, the pre condition, transition
     * relation and post condition. The actual constraint is built based on the
     * inputs of these place holder predicates :
     *
     * PRE(x) -> INV(x)
     * INV() ^ TRANS(x, x') -> INV(x')
     * INV(x) -> POST(x)
     *
     * The regular and primed variables are retrieved from the declaration of
     * the invariant-to-synthesize.
     */
     ...
     * functions, functions-to-synthesize, and which constraints compose it).
     * The built conjecture is a higher-order formula of the form
     *
     * exists f1...fn . forall v1...vm . F
     *
     * in which f1...fn are the functions-to-synthesize, v1...vm are the
     * declared universal variables and F is the set of declared constraints.
     *
     * @throw Exception
     */
    Result checkSynth();
     ...
     * a variable of type (Set T) that is used in quantifier annotations and
     * does not occur in constraints.
     *
     * @param p The pool to declare, which should be a variable of type (Set T)
     * for some type T.
     * @param initValue The initial value of p, which should be a vector of
     * terms of type T.
     */
     ...
     * or NOT_ENTAILED query).  Only permitted if the SmtEngine is set to
     ...
     * returns true if at least one instantiation was printed. The type of
     * output (list, num, etc.) is determined by printInstMode.
     */
     ...
     * the current assertion stack (call it A) and the conjecture (call it B).
     * If this method returns true, then interpolant is set to a formula I such
     * that A ^ ~I and I ^ ~B are both unsatisfiable.
     *
     ...
     * ENTAILED query).  Only permitted if cvc5 was built with unsat-core
     * support and produce-unsat-cores is on.
     */
   ...
   private:
    ...
    /**
     * Internal method to get an unsatisfiable core (only if immediately
     * preceded by an UNSAT or ENTAILED query). Only permitted if cvc5 was built
     * with unsat-core support and produce-unsat-cores is on. Does not dump the
     * command.
     */
    UnsatCore getUnsatCoreInternal();

    /**
     * Check that a generated proof checks. This method is the same as
     * printProof, but does not print the proof. Like that method, it should be
     * called after an UNSAT response. It ensures that a well-formed proof of
     * false can be constructed by the combination of the PropEngine and
     * ProofManager.
     */
    void checkProof();
     ...
     *
     * The check is made by determining that the assertions imply the solution
     * of the interpolation problem (interpol), and the solution implies the
     * goal (conj). If these criteria are not met, an internal error is thrown.
     */
     ...
     * models, and is in "SAT mode", otherwise a recoverable exception is
     * thrown.
     *
     ...
     * this method was called.
     */
    theory::QuantifiersEngine* getAvailableQuantifiersEngine(
        const char* c) const;

    // --------------------------------------- callbacks from the state
    /**
     * Notify push pre, which is called just before the user context of the
     * state pushes. This processes all pending assertions.
     */
    void notifyPushPre();
    /**
     * Notify push post, which is called just after the user context of the
     * state pushes. This performs a push on the underlying prop engine.
     */
    void notifyPushPost();
    /**
    ...
    /**
     * The unsat core manager, which produces unsat cores and related
     * information from refutations. */
     ...
     * SmtEngine in scope. It says the SmtEngine in scope until it is
     * destructed, or another SmtEngine is created.
     */
