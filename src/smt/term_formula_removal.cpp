...
Node RemoveTermFormulas::run(TNode node, std::vector<Node>& output,
  ...
  //   For details on this operator, see http://planetmath.org/hilbertsvarepsilonoperator.
  if( node.getKind() == kind::CHOICE && !inQuant ){
    // Make the skolem to represent the ITE
    skolem = nodeManager->mkSkolem("choiceK", nodeType, "a skolem introduced due to term-level Hilbert choice removal");
