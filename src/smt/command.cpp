...
std::string sexprToString(cvc5::Term sexpr, bool noQuotes)
{
  ...
  {
    // S-expressions can only contain reals in decimal form. Thus, we convert
    // them to a decimal string here. This is required for printing commands
    // correctly (e.g., when dumping the benchmark) and some of the code (e.g.,
    // setting the SMT-LIB version) expects a decimal string.
    int64_t num;
    uint64_t denum;
    std::tie(num, denum) = sexpr.getReal64Value();
    std::stringstream ss;
    if (num < 0)
    {
      ss << "-";
  ...
  // if sexpr is not a special constant, it is an array of sub-sexprs
  std::stringstream ss;
  auto it = sexpr.begin();
