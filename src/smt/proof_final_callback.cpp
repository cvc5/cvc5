  ...
  else if (r == PfRule::ANNOTATION)
  {
    ...
    if (args.size() > 0)
    {
      ...
      if (getInferenceId(args[0], id))
      {
        ...
        // Use e.g. `--check-proofs --proof-annotate -t im-pf` to see a list of inference that appear in the final proof.
        Trace("im-pf") << "(inference-pf " << id << " " << pn->getResult() << ")" << std::endl;
      }
    }
  }
  // print for debugging
