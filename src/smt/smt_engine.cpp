  ...
  */
  ...
  if (options::proof()) {
      if (options::bitvectorAlgebraicSolver()) {
        ...
        Notice() << "SmtEngine: turning off bv algebraic solver to support proofs" << std::endl;
        options::bitvectorAlgebraicSolver.set(false);
      }
      if (options::bitvectorEqualitySolver()) {
        if (options::bitvectorEqualitySolver.wasSetByUser()) {
          throw OptionException(
                  "--bv-eq-solver is not supported with proofs");
        } 
        Notice() << "SmtEngine: turning off bv eq solver to support proofs" << std::endl;
