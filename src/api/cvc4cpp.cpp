...
Grammar::Grammar(const Solver* s,
    ...
    : d_s(s),
      ...
      d_non_terminals(non_terminals),
      d_unres_types(),
      d_nts_to_unres(),
      d_datatype_decls(),
      d_allow_const()
{
  for (Term ntsymbol : d_non_terminals)
  {
    // make the datatype, which encodes terms generated by this non-terminal
    d_datatype_decls.emplace(ntsymbol, DatatypeDecl(d_s, ntsymbol.toString()));
    // make its unresolved type, used for referencing the final version of
    // the datatype
    Sort urs = d_s->getExprManager()->mkSort(ntsymbol.toString());
    d_unres_types.insert(urs);
    d_nts_to_unres[ntsymbol] = urs;
  }
}

void Grammar::addRule(Term non_terminal, Term rule)
{
  CVC4_API_ARG_CHECK_EXPECTED(
      d_datatype_decls.find(non_terminal) != d_datatype_decls.end(),
      non_terminal)
      << "one of the non terminals given in the predeclaration.";

  CVC4_API_ARG_CHECK_EXPECTED(non_terminal.getSort() == rule.getSort(), rule)
      << "the Sorts of rule and non_terminal to be the same.";

  addSygusConstructorTerm(d_datatype_decls[non_terminal], rule);
}

void Grammar::addRules(Term non_terminal, std::vector<Term> rules)
{
  CVC4_API_ARG_CHECK_EXPECTED(
      d_datatype_decls.find(non_terminal) != d_datatype_decls.end(),
      non_terminal)
  ...
  for (size_t i = 0; i < rules.size(); ++i)
  {
    CVC4_API_ARG_AT_INDEX_CHECK_EXPECTED(
        non_terminal.getSort() == rules[i].getSort(), "rule sort", rules[i], i)
        << "the Sorts of rule " << i << " and non_terminal to be the same.";
    addSygusConstructorTerm(d_datatype_decls[non_terminal], rules[i]);
  }
}

void Grammar::addAnyConstant(Term non_terminal)
{
  CVC4_API_ARG_CHECK_EXPECTED(
      d_datatype_decls.find(non_terminal) != d_datatype_decls.end(),
      non_terminal)
      << "one of the non terminals given in the predeclaration.";
  d_allow_const.insert(non_terminal);
}

void Grammar::addAnyVariable(Term non_terminal)
{
  CVC4_API_ARG_CHECK_EXPECTED(
      d_datatype_decls.find(non_terminal) != d_datatype_decls.end(),
      non_terminal)
      << "one of the non terminals given in the predeclaration.";
  addSygusConstructorVariables(d_datatype_decls[non_terminal],
                               non_terminal.d_expr->getType());
}
...
Sort Grammar::ret()
{
  ...
  if (!d_sygus_vars.empty())
  {
    bvl = d_s->mkTermInternal(BOUND_VAR_LIST, d_sygus_vars);
  }

  for (const Term& i : d_non_terminals)
  {
    bool aci = d_allow_const.find(i) != d_allow_const.end();
    Type btt = i.d_expr->getType();
    d_datatype_decls[i].d_dtype->setSygus(btt, *bvl.d_expr, aci, false);
    // We can be in a case where the only rule specified was (Variable T)
    // and there are no variables of type T, in which case this is a bogus
    // grammar. This results in the error below.
    CVC4_API_CHECK(d_datatype_decls[i].d_dtype->getNumConstructors() != 0)
        << "Grouped rule listing for " << d_datatype_decls[i]
        << " produced an empty rule list.";
  }

  // now, make the sygus datatype
  std::vector<CVC4::Datatype> datatypes;
  std::set<Type> unres_types = sortSetToTypes(d_unres_types);

  for (const Term& i : d_non_terminals)
  {
    datatypes.push_back(*d_datatype_decls[i].d_dtype);
  }

  std::vector<DatatypeType> datatypeTypes =
      d_s->d_exprMgr->mkMutualDatatypeTypes(
          datatypes, unres_types, ExprManager::DATATYPE_FLAG_PLACEHOLDER);
...
void Grammar::addSygusConstructorTerm(DatatypeDecl& dt, Term term) const
{
  ...
  if (!args.empty())
  {
    Term lbvl = d_s->mkTermInternal(BOUND_VAR_LIST, args);
    // its operator is a lambda
    op = d_s->mkTermInternal(LAMBDA, {*lbvl.d_expr, *op.d_expr});
  }
...
{
  std::map<Term, Sort>::const_iterator itn = d_nts_to_unres.find(term);
  if (itn != d_nts_to_unres.end())
  {
    Term ret = d_s->d_exprMgr->mkBoundVar(term.d_expr->getType());
    args.push_back(ret);
  ...
  if (term.d_expr->isParameterized())
  {
    ...
    nret = d_s->d_exprMgr->mkExpr(term.d_expr->getKind(),
...
void Grammar::addSygusConstructorVariables(DatatypeDecl& dt, Sort sort) const
{
  ...
  for (unsigned i = 0, size = d_sygus_vars.size(); i < size; i++)
  {
    Term v = d_sygus_vars[i];
...
Grammar Solver::mkGrammar(const std::vector<Term>& bound_vars,
                          const std::vector<Term>& non_terminals) const
{
  CVC4_API_ARG_SIZE_CHECK_EXPECTED(!non_terminals.empty(), non_terminals)
      << "non-empty vector";

  return Grammar(this, bound_vars, non_terminals);
}
...
Term Solver::synthFun(const std::string& symbol,
                      ...
                      Sort sort) const
{
  return synthFunInternal(symbol, bound_vars, sort);
}
...
Term Solver::synthFun(const std::string& symbol,
                      ...
                      Grammar g) const
{
  return synthFunInternal(symbol, bound_vars, sort, false, &g);
}

Term Solver::synthInv(const std::string& symbol,
                      const std::vector<Term>& bound_vars) const
{
  return synthFunInternal(symbol, bound_vars, d_exprMgr->booleanType(), true);
}
                      ...
                      Grammar g) const
{
  return synthFunInternal(
      symbol, bound_vars, d_exprMgr->booleanType(), true, &g);
}
