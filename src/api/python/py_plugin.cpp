...
PyPlugin::PyPlugin(PyObject *obj, TermManager& tm) : Plugin(tm), m_obj(obj), tm(tm) {
...
std::vector<Term> PyPlugin::check()
{
    if (this->m_obj) {
        ...
        std::vector<Term> result = cy_call_vec_term_func(this->m_obj, "check", &error);
        if (!error.empty())
...
std::vector<Term> PyPlugin::plugin_check()
{
    return Plugin::check();
}
