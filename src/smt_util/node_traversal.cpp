...
NodePostorderIterator::NodePostorderIterator(TNode n) : d_stack{{false, n}}
{
  enqueueChildrenUntilLeaf();
}

NodePostorderIterator::NodePostorderIterator() : d_stack() {}
...
TNode& NodePostorderIterator::operator*() { return d_stack.back().second; }
...
void NodePostorderIterator::enqueueChildrenUntilLeaf()
{
  if (d_stack.size() > 0)
  {
    ...
    while (!d_stack.back().first)
    {
      ...
      // use integer underflow wrapping behavior to reverse-iterate over the
      // children,
