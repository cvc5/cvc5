...
void TheoryArrays::queueRowLemma(RowLemmaType lem)
{
  ...
  if (options::arraysEagerLemmas() || bothExist)
  {
    ...
    if (aj != aj2) {
      ...
      d_im.assertInference(
          aj.eqNode(aj2), true, InferenceId::ARRAYS_EQ_TAUTOLOGY, d_true, PfRule::MACRO_SR_PRED_INTRO);
    }
    ...
    if (bj != bj2) {
      ...
      d_im.assertInference(
          bj.eqNode(bj2), true, InferenceId::ARRAYS_EQ_TAUTOLOGY, d_true, PfRule::MACRO_SR_PRED_INTRO);
    }
    if (aj2 == bj2) {
      return;
    }
