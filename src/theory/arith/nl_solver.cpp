...
bool hasNewMonomials(Node n, const std::vector<Node>& existing) {
  ...
  while (!worklist.empty()) {
    ...
    if (!Contains(visited, current)) {
      ...
      {
        if (std::find(existing.begin(), existing.end(), current)==existing.end()) {
          return true;
        }
      } else {
...
const NodeMultiset& NlSolver::getMonomialExponentMap(
    Node monomial) const {
...
void NlSolver::setMonomialFactor(Node a, Node b,
...
Node NlSolver::mkMonomialRemFactor(
...
int NlSolver::compareSign(Node oa, Node a, unsigned a_index,
  ...
  if (sgn == 0) {
    ...
    if (aexp % 2 == 0) {
      ...
      exp.push_back(
          nm->mkNode(sgn == 1 ? GT : LT, av, d_zero));
        ...
        if (its == d_ci[x][coeff].end()) {
          ...
          if (jk == UNDEFINED_KIND)
          {
            ...
            if (jk == type) {
              ...
              d_ci[x][coeff][rhs] = jk;
              d_ci_exp[x][coeff][rhs] = nm->mkNode(
              ...
              for (unsigned j = 0; j < itm->second.size(); j++) {
                ...
                if(!mmv.isConst()){
                  ...
                  if (mmv_sign == 0) {
                    ...
                    Node infer_lhs =
                        nm->mkNode(MULT, mult, t);
                    Node infer_rhs =
                        nm->mkNode(MULT, mult, rhs);
                    Node infer = nm->mkNode(
          ...
          if (ita == d_mono_diff[a].end()) {
            ...
            for (std::map<Node, std::map<Node, Kind> >::iterator itcac =
              ...
              for (std::map<Node, Kind>::iterator itcar =
                ...
                Node rhs_a_res_base =
                    nm->mkNode(MULT, itb->second, rhs_a);
