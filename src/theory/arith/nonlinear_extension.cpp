                                      ...
                                      std::map<Node, NlLemmaSideEffect>& lemSE)
{
  ...
  d_nlnSlv.initLastCall(assertions, false_asserts, xts);
  ...
  //-----------------------------------initial lemmas for X
  // functions
  lemmas = d_nlnSlv.checkInitialRefine();
  filterLemmas(lemmas, lems);
  if (!lems.empty())
  {
    Trace("nl-ext") << "  ...finished with " << lems.size() << " new lemmas."
  ...
  // run the full refinement in the new non-linear solver
  lemmas = d_nlnSlv.checkFullRefine();
  filterLemmas(lemmas, wlems);
