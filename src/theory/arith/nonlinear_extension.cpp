                                    ...
                                    std::vector<Node>& gs)
{
  ...
  unsigned tdegree = d_trExt.getTaylorDegree();
  bool ret = d_model.checkModel(
      passertions, false_asserts, tdegree, lemmas, gs);
  return ret;
}
                                      ...
                                      std::map<Node, NlLemmaSideEffect>& lemSE)
{
  ...
  std::vector<Node> lemmas;
  d_trExt.initLastCall(assertions,false_asserts,xts,lemmas,lemsPp);
  ...
  lemmas = d_trExt.checkTranscendentalInitialRefine();
  filterLemmas(lemmas, lems);
  if (!lems.empty())
  {
    Trace("nl-ext") << "  ...finished with " << lems.size() << " new lemmas."
  ...
  lemmas = d_trExt.checkTranscendentalMonotonic();
  filterLemmas(lemmas, lems);
  if (!lems.empty())
  {
    Trace("nl-ext") << "  ...finished with " << lems.size() << " new lemmas."
  ...
  {
    lemmas = d_trExt.checkTranscendentalTangentPlanes(lemSE);
    filterLemmas(lemmas, wlems);
  }
    ...
    std::map<Node, NlLemmaSideEffect>& lemSE)
{
  ...
  do
  {
    ...
    if (complete_status != 1)
    {
      ...
      {
        d_trExt.incrementTaylorDegree();
        needsRecheck = true;
        // increase precision for PI?
        // Difficult since Taylor series is very slow to converge
        Trace("nl-ext") << "...increment Taylor degree to " << d_trExt.getTaylorDegree()
                        << std::endl;
      }
      else
      {
