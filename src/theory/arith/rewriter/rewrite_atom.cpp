...
Node buildIntegerEquality(Sum&& sum)
{
  ...
  if (constant.first.isConst())
  {
    ...
    {
      Trace("arith-rewriter::debug") << "\thas non-integer constant, thus false" << std::endl;
      return mkConst(false);
    }
  }

  auto minabscoeff = removeMinAbsCoeff(sum);
  Trace("arith-rewriter::debug") << "\tremoved min abs coeff " << minabscoeff << ", left with " << sum << std::endl;
  ...
  Trace("arith-rewriter::debug") << "\tbuilding " << left << " = " << sum << std::endl;
