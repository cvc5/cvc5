...
Node ArithMSum::mkNode( const std::map< Node, Node >& msum ) {
  ...
  for( std::map< Node, Node >::const_iterator it = msum.begin(); it != msum.end(); ++it ){
    Node m;
    if( !it->first.isNull() ){
      if( !it->second.isNull() ){
...
int ArithMSum::isolate( Node v, const std::map< Node, Node >& msum, Node & veq_c, Node & val, Kind k ) {
...
int ArithMSum::isolate( Node v, const std::map< Node, Node >& msum, Node & veq, Kind k, bool doCoeff ) {
  Node veq_c;
  Node val;
