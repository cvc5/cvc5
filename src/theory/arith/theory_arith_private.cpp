...
bool TheoryArithPrivate::attemptSolveInteger(Theory::Effort effortLevel, bool emmmittedLemmaOrSplit){
  ...
  if(!options().arith.useApprox){ return false; }
  ...
  if(!options().arith.trySolveIntStandardEffort){ return false; }
...
void TheoryArithPrivate::solveInteger(Theory::Effort effortLevel){
  ...
  ApproximateSimplex* approx =
    ...
    int maxDepthPass1 = d_likelyIntegerInfeasible ?
      depthForLikelyInfeasible : options().arith.maxApproxDepth;
...
void TheoryArithPrivate::importSolution(const ApproximateSimplex::Solution& solution){
  ...
  if(d_qflraStatus != Result::UNSAT){
    ...
    int16_t oldCap = options().arith.arithStandardCheckVarOrderPivots;
...
bool TheoryArithPrivate::solveRealRelaxation(Theory::Effort effortLevel){
  ...
  bool noPivotLimit = Theory::fullEffort(effortLevel) ||
    !options().arith.restrictedPivots;

  SimplexDecisionProcedure& simplex = selectSimplex(true);

  bool useApprox = options().arith.useApprox && ApproximateSimplex::enabled() && getSolveIntegerResource();
...
bool TheoryArithPrivate::postCheck(Theory::Effort effortLevel)
{
  ...
  size_t nPivots =
      options().arith.useFC ? d_fcSimplex.getPivots() : d_dualSimplex.getPivots();
  ...
  if(!emmittedConflictOrSplit && Theory::fullEffort(effortLevel) && !hasIntegerModel()){
    ...
    if(!emmittedConflictOrSplit && d_hasDoneWorkSinceCut && options().arith.arithDioSolver){
...
void TheoryArithPrivate::propagateCandidates(){
  ...
  for(; i != end; ++i){
    ArithVar var = *i;
    if(d_tableau.isBasic(var) &&
       d_tableau.basicRowLength(var) <= options().arith.arithPropagateMaxLength){
      ...
      for(; !basicIter.atEnd(); ++basicIter){
        ...
        if(d_tableau.getRowLength(ridx) <= options().arith.arithPropagateMaxLength){
