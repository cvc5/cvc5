...
void ApproximatePi::refine()
{
    ...
    Integer den = Integer(512) * k.pow(4) + Integer(1024) * k.pow(3) + Integer(712) * k.pow(2) + Integer(194) * k + Integer(15);
    den = den.multiplyByPow2(d_next_k * 4);

    Rational summand(num, den);

    d_lower += summand;
    d_upper = d_lower + Rational(Integer(1), Integer(1).multiplyByPow2(d_next_k * 4));
