...
void LiaStarExtension::processSideEffect(const NlLemma& se)
{
  d_trSlv.processSideEffect(se);
}

void LiaStarExtension::computeRelevantAssertions(
...
bool LiaStarExtension::checkModel(const std::vector<Node>& assertions)
{
...
{
  Trace("nl-ext") << "LiaStarExtension::checkFullEffort" << std::endl;
  ...
  Trace("nl-ext") << "LiaStarExtension::interceptModel begin" << std::endl;
...
void LiaStarExtension::runStrategy(Theory::Effort effort,
                                   const std::vector<Node>& assertions,
