...
class MatrixEntry
{
 ...
 private:
  ...
  {
    return (d_rowIndex == ROW_INDEX_SENTINEL && d_colVar == ARITHVAR_SENTINEL)
           || (d_rowIndex != ROW_INDEX_SENTINEL
               && d_colVar != ARITHVAR_SENTINEL);
  }

 public:
  EntryID getNextRowEntryID() const { return d_nextRow; }

  EntryID getNextColEntryID() const { return d_nextCol; }
  EntryID getPrevRowEntryID() const { return d_prevRow; }

  EntryID getPrevColEntryID() const { return d_prevCol; }

  void setNextRowEntryID(EntryID id) { d_nextRow = id; }
  void setNextColEntryID(EntryID id) { d_nextCol = id; }
  void setPrevRowEntryID(EntryID id) { d_prevRow = id; }
  void setPrevColEntryID(EntryID id) { d_prevCol = id; }

  RowIndex getRowIndex() const { return d_rowIndex; }

  ArithVar getColVar() const { return d_colVar; }

  const T& getCoefficient() const { return d_coefficient; }

  T& getCoefficient() { return d_coefficient; }

  void setCoefficient(const T& t) { d_coefficient = t; }
...
class MatrixEntryVector
{
 ...
 public:
  MatrixEntryVector() : d_entries(), d_freedEntries(), d_size(0) {}
 ...
 private:
  bool inBounds(EntryID id) const { return id < d_entries.size(); }
...
class MatrixVector
{
 private:
  ...
  class Iterator
  {
   ...
   public:
    EntryID getID() const { return d_curr; }
    ...
    bool atEnd() const { return d_curr == ENTRYID_SENTINEL; }
 ...
 public:
  ...
  const_iterator begin() const { return Iterator(d_head, d_entries); }
  const_iterator end() const { return Iterator(ENTRYID_SENTINEL, d_entries); }
...
class Matrix
{
 ...
 public:
  ...
  {
    d_columns.clear();
    for (typename ColumnTable::const_iterator c = m.d_columns.begin(),
                                              cend = m.d_columns.end();
         c != cend;
         ++c)
    {
      const ColumnVectorT& col = *c;
      d_columns.push_back(
          ColumnVectorT(col.getHead(), col.getSize(), &d_entries));
    }
    d_rows.clear();
    for (typename RowTable::const_iterator r = m.d_rows.begin(),
                                           rend = m.d_rows.end();
         r != rend;
         ++r)
    {
  ...
  Matrix& operator=(const Matrix& m)
  {
         ...
         ++c)
    {
      ...
      d_columns.push_back(
          ColumnVector<T>(col.getHead(), col.getSize(), &d_entries));
    }
    d_rows.clear();
    for (typename RowTable::const_iterator r = m.d_rows.begin(),
                                           rend = m.d_rows.end();
 ...
 private:
  ...
  void releaseRowIndex(RowIndex rid) { d_pool.push_back(rid); }

 public:
  size_t getNumRows() const { return d_rows.size(); }

  size_t getNumColumns() const { return d_columns.size(); }

  void increaseSize() { d_columns.push_back(ColumnVector<T>(&d_entries)); }
  ...
  uint32_t getRowLength(RowIndex r) const { return getRow(r).getSize(); }

  uint32_t getColLength(ArithVar x) const { return getColumn(x).getSize(); }
  ...
  /**  to += mult * buffer. */
  void rowPlusBufferTimesConstant(RowIndex to,
                                  const T& mult,
 ...
 public:
  uint32_t size() const { return d_entriesInUse; }
  uint32_t getNumEntriesInTableau() const { return d_entries.size(); }
  uint32_t getEntryCapacity() const { return d_entries.capacity(); }

  void manipulateRowEntry(RowIndex row,
                          ArithVar col,
                          const T& c,
                          CoefficientChangeCallback& cb)
  {
