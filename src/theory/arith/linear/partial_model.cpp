...
bool ArithVariables::hasNode(ArithVar a) const { return d_vars.isKey(a); }
...
ArithVariables::var_iterator::var_iterator(const VarInfoVec* vars,
...
bool ArithVariables::var_iterator::operator==(
    const ArithVariables::var_iterator& other) const
{
...
ArithVar ArithVariables::var_iterator::operator*() const { return *d_wrapped; }

void ArithVariables::var_iterator::nextInitialized()
{
  VarInfoVec::const_iterator end = d_vars->end();
  while (d_wrapped != end && !((*d_vars)[*d_wrapped].initialized()))
  {
    ++d_wrapped;
  }
}
...
bool ArithVariables::VarInfo::setAssignment(const DeltaRational& a,
                                            BoundsInfo& prev)
{
  Assert(initialized());
  d_assignment = a;
  int cmpUB =
      (d_ub == NullConstraint) ? -1 : d_assignment.cmp(d_ub->getValue());
...
std::pair<ConstraintP, ConstraintP> ArithVariables::explainEqualBounds(
    ArithVar x) const
{
...
void ArithVariables::setAssignment(ArithVar x,
...
//   // Is worth mentioning that this is not strictly necessary, but this
//   maintains the internal invariant
...
{
  AssertArgument(c != NullConstraint,
...
{
  return d_vars[x].d_cmpAssignmentLB >= 0 && d_vars[x].d_cmpAssignmentUB <= 0;
}
...
void ArithVariables::revertAssignmentChanges() { clearSafeAssignments(true); }
void ArithVariables::commitAssignmentChanges() { clearSafeAssignments(false); }
...
{
  out << "model" << x << ": " << asNode(x) << " " << getAssignment(x) << " ";
  if (!hasLowerBound(x))
  {
    out << "no lb ";
  }
...
bool ArithVariables::boundsQueueEmpty() const { return d_boundsQueue.empty(); }
...
void ArithVariables::invalidateDelta() { d_deltaIsSafe = false; }
