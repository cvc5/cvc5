...
InferBoundAlgorithm::InferBoundAlgorithm() : d_alg(None) {}

InferBoundAlgorithm::InferBoundAlgorithm(Algorithms a) : d_alg(a)
{
  Assert(a != Simplex);
}
...
Algorithms InferBoundAlgorithm::getAlgorithm() const { return d_alg; }
...
void ArithEntailmentCheckParameters::addAlgorithm(
    const inferbounds::InferBoundAlgorithm& alg)
{
  d_algorithms.push_back(alg);
}

ArithEntailmentCheckParameters::const_iterator
...
const DeltaRational& InferBoundsResult::getValue() const { return d_value; }
...
Node InferBoundsResult::getLiteral() const
{
  ...
  NodeManager* nm = d_term.getNodeManager();
...
Node InferBoundsResult::getExplanation() const { return d_explanation; }
...
bool InferBoundsResult::budgetIsExhausted() const { return d_budgetExhausted; }
