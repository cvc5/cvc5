...
/** representative set
 * 
 * This class contains finite lists of values for types, typically values and types that exist
 * in the equality engine of a model object.  In the following, "representative" means a value
 * that exists in this set.
 * 
 * This class is used for finite model finding and other exhaustive instantiation-based
 * methods. The class goes beyond just maintaining a list of values that occur in the equality
 ...
 * exhaustive instantiation strategies that enumerate over small interpreted finite types.
 */
class RepSet {
public:
  RepSet(){}
   ...
   * Resets d_type_reps[tn] and repopulates by running the type enumerator for that type exhaustively.
   * This should only be called for small finite interpreted types.
   */
  bool complete( TypeNode t );
  /** get term for representative 
   * Returns a term that is interpreted as representative n in the current model, null otherwise.
   */
  Node getTermForRepresentative( Node n ) const;
  /** set term for representative 
   * Called when t is interpreted as value n. Subsequent class to getTermForRepresentative( n ) will return t.
   */
