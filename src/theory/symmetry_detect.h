...
class SymmetryDetect {
 ...
 public:
   ...
   * d_var_to_subvar is the mapping from the variable to the substitution variable,
   * and d_subvar_to_vars is the mapping from the substitution variable to a list
   * of variables forming a region of a partition.
   */
  class Partition {
  public:
    /** Term corresponding to the partition, e.g., x + y = 0 */
    Node d_term;
    /** Mapping between the variable and the substitute variable x -> w, y -> w, z -> w */
  ...
  /**
   * We build the partition trie indexed by parts[0].var_to_subvar[v]....parts[n].var_to_subvar[v].
   * The leaves of a partition trie is the new regions of a partition
   */
  class PartitionTrie
  {
  public:
    ...
    /** Add variable v to the trie, indexed by parts[0].var_to_subvar[v]....parts[n].var_to_subvar[v]. */
 ...
 private:
   ...
   *  We might need optimizations here, such as rewriting the input to negation normal form.
   * */
  ...
  void getVariables( vector< Partition >& partitions, unordered_set< Node, NodeHashFunction >& vars );

  /** Start processing singleton partitions and add all variables to vars */
  void processSingletonPartitions( vector< Partition >& partitions, unordered_set< Node, NodeHashFunction >& vars );

  /** Do matches on singleton partitions */
  void matches( vector< Partition >& partitions, map< Node, Node >& subvar_to_var, map< Node, Node >& subvar_to_expr );
