                             ...
                             bool& useBuffer)
{
  ...
  switch (infer)
  {
    ...
    case InferenceId::STRINGS_EXTF_EQ_REW:
    {
      ...
      Trace("strings-ipc-core") << "Generate proof for STRINGS_EXTF_EQ_REW, starting with " << src << std::endl;
      Node mainEqSRew = psb.applyPredElim(src, expe);
      Trace("strings-ipc-core") << "...after pred elim: " << mainEqSRew << std::endl;
      if (mainEqSRew == conc)
      {
      ...
      else if (mainEqSRew.getKind()!=EQUAL)
      {
        ...
        Trace("strings-ipc-core") << "...failed, not equality after rewriting" << std::endl;
        break;
      }
      ...
      Trace("strings-ipc-core") << "...after extended equality rewrite: " << mainEqSRew2 << std::endl;
      if (mainEqSRew2 == conc)
      {
        useBuffer = true;
        break;
