...
bool TheoryStrings::processLoop( std::vector< std::vector< Node > > &normal_forms, std::vector< Node > &normal_form_src,
  ...
  if( s_zy == t_yz &&
    ...
    //special case
    str_in_re = nm->mkNode( kind::STRING_IN_REGEXP, normal_forms[other_n_index][index],
    ...
    for(unsigned len=1; len<=size; len++) {
      ...
      Node conc2 = nm->mkNode(kind::STRING_IN_REGEXP, normal_forms[other_n_index][index],
    ...
    conc = vconc.size()==0 ? Node::null() : vconc.size()==1 ? vconc[0] : nm->mkNode(kind::OR, vconc);
  } else {
    ...
    str_in_re = nm->mkNode( kind::STRING_IN_REGEXP, sk_w,
            nm->mkNode( kind::REGEXP_STAR,
              nm->mkNode( kind::STRING_TO_REGEXP, restr ) ) );
