...
TheoryStrings::TheoryStrings(Env& env, OutputChannel& out, Valuation valuation)
    : Theory(THEORY_STRINGS, env, out, valuation),
      ...
      d_regexp_elim(options().strings.regExpElim==options::RegExpElimMode::AGG, d_pnm, userContext()),
...
TrustNode TheoryStrings::ppRewrite(TNode atom, std::vector<SkolemLemma>& lems)
{
  ...
  TrustNode ret;
  Node atomRet = atom;
  if (options().strings.regExpElim!=options::RegExpElimMode::OFF && atom.getKind() == STRING_IN_REGEXP)
  {
