...
Node StringsPreprocess::simplify( Node t, std::vector< Node > &new_nodes ) {
  ...
  else if (t.getKind() == STRING_TOLOWER || t.getKind()==STRING_TOUPPER )
  {
    ...
    Node ci = nm->mkNode(STRING_CODE,nm->mkNode(STRING_SUBSTR,t,i,d_one));
    ...
    Node offset = nm->mkConst(Rational(t.getKind()==STRING_TOUPPER ? -32 : 32));
    
    Node res = nm->mkNode( ITE, nm->mkNode( AND, nm->mkNode(LEQ, lb, ci ), nm->mkNode( ci, ub ) ), nm->mkNode(PLUS, ci, offset), ci );
    
    Node bound = nm->mkNode( AND, nm->mkNode( LEQ, d_zero, i ), nm->mkNode( LEQ, i, lenr ) );
    Node rangeA = nm->mkNode(FORALL,bvi,nm->mkNode(OR, bound.negate(), ri.eqNode(res)));
    ...
    // where ci = str.code( str.substr(t,i,1) )
