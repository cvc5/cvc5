...
enum class Inference : uint32_t
{
  ...
  //   x1 = "" ^ ... ^ x_{i-1} = "" ^ x_{i+1} = "" ^ ... ^ xn = "" =>
  //   x1 ++ ... ++ xn = xi
  // Proof: equal by substitution+rewriting
  I_NORM_S,
  ...
  // where c1 != c2.
  // Proof: equal by substitution+rewriting, rewrite constant equality to false
  I_CONST_CONFLICT,
  // initial normalize
  ...
  // Proof: x in (re.complement R2), RE_INTER, substitution
  // (re.inter R1 (re.complement R2)) -> re.none, rewrite
  RE_INTER_INCLUDE,
  // intersection conflict, using regexp intersection computation
  //   (x in R1 ^ x in R2) => false   where [[intersect(R1, R2) = empty]]
  // Proof: RE_INTER, rewrite
  RE_INTER_CONF,
  ...
  // the current context.
  // Proof: substitution+rewriting, Boolean entailment
  EXTF_D,
  // Same as above, for normal form substitutions.
  // Proof: substitution+rewriting, Boolean entailment
  EXTF_D_N,
