...
void InferenceManager::sendLemma(Node ant, Node conc, Inference infer)
{
  if (conc.isNull() || conc == d_false)
  {
    ...
    ProvenNode conf = ProvenNode::mkProvenNodeConflict(ant, nullptr);
    d_poc.conflict(conf);
    d_state.setConflict();
    return;
  }
...
void InferenceManager::doPendingLemmas()
{
  if (!d_state.isInConflict())
  {
    for (const Node& lc : d_pendingLem)
    {
      ...
      ProvenNode plem = ProvenNode::mkProvenNodeLemma(lc, nullptr);
      d_poc.lemma(plem);
    }
...
void InferenceManager::assertPendingFact(Node atom, bool polarity, Node exp)
{
  ...
  if (!d_state.isInConflict())
  {
    ...
    if (!pc.isNull())
    {
      ...
      ProvenNode pconf = ProvenNode::mkProvenNodeConflict(cnode, nullptr);
