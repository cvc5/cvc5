...
{
  return k==STRING_STOI || k==STRING_ITOS || k==STRING_TOLOWER || k==STRING_TOUPPER || k==STRING_LEQ || k==STRING_LT || k==STRING_FROM_CODE || k==STRING_TO_CODE;
}
...
TypeNode getOwnerStringType(Node n)
{
  TypeNode tn;
  Kind k = n.getKind();
  if (k==STRING_STRIDOF || k==STRING_LENGTH || k==STRING_STRCTN || k==STRING_PREFIX || k==STRING_SUFFIX)
  {
  ...
  AlwaysAssert(tn.isStringLike()) << "Unexpected term in getOwnerStringType : " << n << ", type " << tn;
  return tn;
}
