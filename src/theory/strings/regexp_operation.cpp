...
bool RegExpOpr::containC2(unsigned cnt, Node n) {
  if(n.getKind() == kind::REGEXP_RV) {
    Assert(n[0].getConst<Rational>() <= d_rMaxInt, "Exceeded UINT32_MAX in RegExpOpr::containC2");
...
void RegExpOpr::convert2(unsigned cnt, Node n, Node &r1, Node &r2) {
  if(n == d_emptyRegexp) {
    ...
    Assert(n[0].getConst<Rational>() <= d_rMaxInt, "Exceeded UINT32_MAX in RegExpOpr::convert2");
    unsigned y = n[0].getConst<Rational>().getNumerator().toUnsignedInt();
    r1 = d_emptySingleton;
    if(cnt == y) {
      r2 = d_emptyRegexp;
