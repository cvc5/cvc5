...
WordIter::WordIter(uint32_t startLength)
    : d_hasEndLength(false), d_endLength(0)
{
...
SEnumLen::SEnumLen(TypeNode tn, uint32_t startLength) : d_type(tn), 
  d_witer(new WordIter(startLength))
{
  
}
SEnumLen::SEnumLen(TypeNode tn, uint32_t startLength, uint32_t endLength) : d_type(tn), 
  d_witer(new WordIter(startLength, endLength))
{
...
d_type(e.d_type),
d_witer(new WordIter(*e.d_witer)),
d_curr(e.d_curr)
{
...
StringEnumLen::StringEnumLen(uint32_t startLength, uint32_t endLength, uint32_t card)
    : SEnumLen(NodeManager::currentNM()->stringType(), startLength, endLength), d_cardinality(card)
{
  mkCurr();
}

StringEnumLen::StringEnumLen(uint32_t startLength, uint32_t card)
    : SEnumLen(NodeManager::currentNM()->stringType(), startLength), d_cardinality(card)
{
  mkCurr();
}
...
StringEnumerator::StringEnumerator(TypeNode type, TypeEnumeratorProperties* tep)
    : TypeEnumeratorBase<StringEnumerator>(type), d_wenum(0, utils::getAlphabetCardinality())
{
...
TypeEnumeratorBase<StringEnumerator>(enumerator.getType()), d_wenum(enumerator.d_wenum)
{
  
}

Node StringEnumerator::operator*() 
{
...
bool StringEnumerator::isFinished() 
{
