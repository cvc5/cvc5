...
Node RegExpElimination::eliminateConcat(Node atom)
{
  ...
  // if it can be reduced to memberships in 
  ...
  for (unsigned i=0, size = children.size(); i<size; i++)
  {
    ...
    if( fl.isNull() )
    {
      if (!hasPivotIndex && c.getKind() == REGEXP_STAR && c[0].getKind() == REGEXP_SIGMA)
      {
        hasPivotIndex = true;
        pivotIndex = i;
  ...
  if( hasFixedLength )
  {
    ...
    for( unsigned i=0, size = childLengths.size(); i<size; i++ )
    {
      ...
      {
        Node ppSum = childLengthsPostPivot.size()==1 ? childLengthsPostPivot[0] : nm->mkNode( PLUS, childLengthsPostPivot );
