        ...
        if (itsv != d_cache[a].d_search_val[tn].end())
        {
          ...
          if (sample_ret != prev_sample_ret)
          {
            ...
            if( Trace.isOn("sygus-rr-debug") )
            {
              ...
              if( pt_index>=0 )
              {
                Trace("sygus-rr-debug") << "; both ext-rewrite to : " << bvr << std::endl;
                Trace("sygus-rr-debug") << "; but are not equivalent for : " << std::endl;
                ...
                {
                  Trace("sygus-rr-debug") << ";   " << vars[i] << " -> " << pt[i] << std::endl;
                }
                Node bv_e = its->second.evaluate(bv,pt_index);
                Node pbv_e = its->second.evaluate(prev_bv,pt_index);
                Assert( bv_e!=pbv_e );
                Trace("sygus-rr-debug") << "; where they evaluate to " << pbv_e << " and " << bv_e << std::endl;
              }
              else
              {
                Assert( false );
