...
SygusSimpleSymBreak::SygusSimpleSymBreak(QuantifiersEngine * qe) : d_tds(qe->getTermDatabaseSygus()), d_tutil(qe->getTermUtil()){
...
bool SygusSimpleSymBreak::considerArgKind( TypeNode tn, TypeNode tnp, Kind k, Kind pk, int arg ) {
  ...
  if( pk==NOT || pk==BITVECTOR_NOT || pk==UMINUS || pk==BITVECTOR_NEG ){
    ...
    if( pk==k ){
      ...
      if( pk==NOT ){
        if( k==AND ) {
          ...
          rt.d_children[0].d_children[0].d_req_type = TypeNode::fromType( dt[c].getArgType( 1 ) );
          ...
          rt.d_children[1].d_children[0].d_req_type = TypeNode::fromType( dt[c].getArgType( 0 ) );
          ...
          if( pdt[pcr].getNumArgs()==dt[c].getNumArgs() ){
            for( unsigned i=0; i<pdt[pcr].getNumArgs(); i++ ){
              ...
              if( rk!=UNDEFINED_KIND ){
                ...
                rt.d_children[i].d_children[0].d_req_type =TypeNode::fromType(  dt[c].getArgType( i ));
              }
            }
          }
        }
    ...
    if( pk==EQUAL || 
      ...
      rt.d_children[oarg].d_children[0].d_req_type = TypeNode::fromType( pdt[pc].getArgType( oarg ) );
      rt.d_children[oarg].d_children[1].d_req_type = TypeNode::fromType( dt[c].getArgType( 1 ) );
      ...
      rt.d_children[0].d_children[0].d_req_type = TypeNode::fromType( pdt[pc].getArgType( oarg  ));
      rt.d_children[0].d_children[1].d_req_type = TypeNode::fromType( dt[c].getArgType(  0 ) );
    ...
    if( pk!=ITE ){
      ...
      for( unsigned r=1; r<=2; r++ ){
        ...
        for( unsigned q=0; q<n_args; q++ ){
          if( (int)q==arg ){
            rt.d_children[r].d_children[q].d_req_type = TypeNode::fromType( dt[c].getArgType( r ) );
          }else{
            rt.d_children[r].d_children[q].d_req_type = TypeNode::fromType( pdt[pc].getArgType( q ) );
          }
        }
      }
...
bool SygusSimpleSymBreak::considerConst( TypeNode tn, TypeNode tnp, Node c, Kind pk, int arg ) {
  const Datatype& pdt = ((DatatypeType)(tnp).toType()).getDatatype();
    ...
    if (d_tutil->hasOffsetArg(pk, arg, offset, ok))
    {
      ...
      if( ok_arg!=-1 ){
        ...
        if( d_tds->isTypeMatch( pdt[ok_arg], pdt[arg] ) ){
          int status;
          Node co = d_tutil->getTypeValueOffset(
...
bool SygusSimpleSymBreak::considerConst( const Datatype& pdt, TypeNode tnp, Node c, Kind pk, int arg ) {
  Assert( d_tds->hasKind( tnp, pk ) );
  int pc = d_tds->getKindConsNum( tnp, pk );
  bool ret = true;
...
int SygusSimpleSymBreak::solveForArgument( TypeNode tn, unsigned cindex, unsigned arg ) {
  // we currently do not solve for arguments
  return -1;
}

int SygusSimpleSymBreak::getFirstArgOccurrence( const DatatypeConstructor& c, TypeNode tn ) {
