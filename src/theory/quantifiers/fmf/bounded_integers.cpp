      ...
      d_ranges_proxied(userContext())
{
  ...
  if (!isProxy)
  {
    Trace("bound-int") << "Introduce proxy " << d_proxy_range << " for "
                       << d_range << std::endl;
  }
}
...
void BoundedIntegers::presolve() { d_bnd_it.clear(); }

bool BoundedIntegers::hasNonBoundVar(Node f,
                                     Node b,
...
bool BoundedIntegers::processEqDisjunct(Node q,
                                        Node n,
                                        Node& v,
  ...
  if (n.getKind() == Kind::EQUAL)
  {
    for (unsigned i = 0; i < 2; i++)
    {
      ...
      if (!hasNonBoundVar(q, n[1 - i]))
      {
        ...
        else if (v.isNull() && t.getKind() == Kind::BOUND_VARIABLE)
        {
          v = t;
          v_cases.push_back(n[1 - i]);
          return true;
...
void BoundedIntegers::processMatchBoundVars(Node q,
                                            Node n,
                                            std::vector<Node>& bvs,
                                            std::map<Node, bool>& visited)
{
  ...
  {
    visited[n] = true;
    if (n.getKind() == Kind::BOUND_VARIABLE && !isBound(q, n))
    {
      bvs.push_back(n);
      // injective operators
    }
    ...
    std::map<Node, std::vector<Node> >& bound_fixed_set)
{
  if (n.getKind() == Kind::OR || n.getKind() == Kind::AND)
  {
    ...
    else
    {
      ...
      Trace("bound-int-debug")
          << "Process possible finite disequality conjunction : " << conj
          << std::endl;
      ...
      {
        if (conj[i].getKind() == Kind::NOT
            && processEqDisjunct(q, conj[i][0], v, v_cases))
        {
          // continue
        }
        else
      ...
      if (success && !isBound(q, v))
      {
        ...
        bound_fixed_set[v].insert(
            bound_fixed_set[v].end(), v_cases.begin(), v_cases.end());
      }
    }
  }
  ...
  else if (n.getKind() == Kind::GEQ)
  {
    if (n[0].getType().isInteger())
    {
      ...
      {
        NodeManager* nm = nodeManager();
        Trace("bound-int-debug") << "literal (polarity = " << pol << ") " << n
        ...
        for (std::map<Node, Node>::iterator it = msum.begin(); it != msum.end();
             ++it)
        {
          if (!it->first.isNull() && it->first.getKind() == Kind::BOUND_VARIABLE
              && !isBound(q, it->first))
          {
            // if not bound in another way
            if (bound_lit_type_map.find(it->first) == bound_lit_type_map.end()
                || bound_lit_type_map[it->first] == BOUND_INT_RANGE)
            {
              Node veq;
              if (ArithMSum::isolate(it->first, msum, veq, Kind::GEQ) != 0)
              {
                Node n1 = veq[0];
                Node n2 = veq[1];
                if (pol)
                {
                  ...
                  n1 = veq[1];
                  n2 = veq[0];
                  if (n1.getKind() == Kind::BOUND_VARIABLE)
                  {
                    n2 = nm->mkNode(Kind::ADD, n2, nm->mkConstInt(Rational(1)));
                  }
                  else
                  {
                    n1 =
                        nm->mkNode(Kind::ADD, n1, nm->mkConstInt(Rational(-1)));
                  }
                  veq = nm->mkNode(Kind::GEQ, n1, n2);
                }
                Trace("bound-int-debug")
                    << "Isolated for " << it->first << " : (" << n1
                    << " >= " << n2 << ")" << std::endl;
                Node t = n1 == it->first ? n2 : n1;
                if (!hasNonBoundVar(q, t))
                {
                  Trace("bound-int-debug")
                      << "The bound is relevant." << std::endl;
                ...
                {
                  Trace("bound-int-debug")
                      << "The term " << t << " has non-bound variable."
                      << std::endl;
                }
              }
            }
  ...
  {
    Assert(n.getKind() != Kind::LEQ && n.getKind() != Kind::LT
           && n.getKind() != Kind::GT);
  }
}
...
{
  // this needs to be done at preregister since it affects e.g. QuantDSplit's
  // preregister
  ...
  do
  {
    ...
    process(f,
            ...
            bound_int_range_term,
            bound_fixed_set);
    // for( std::map< Node, Node >::iterator it = d_bounds[0][f].begin(); it !=
    // d_bounds[0][f].end(); ++it ){
    for (std::map<Node, unsigned>::iterator it = bound_lit_type_map.begin();
         it != bound_lit_type_map.end();
         ++it)
    {
      Node v = it->first;
      if (!isBound(f, v))
      {
        ...
        if (it->second == BOUND_INT_RANGE)
        {
          ...
          if (blm0.find(v) != blm0.end() && blm1.find(v) != blm1.end())
          {
            ...
            Node r =
                nm->mkNode(Kind::SUB, d_bounds[1][f][v], d_bounds[0][f][v]);
            d_range[f][v] = rewrite(r);
            Trace("bound-int") << "Variable " << v
                               << " is bound because of int range literals "
                               << bound_lit_map[0][v] << " and "
                               << bound_lit_map[1][v] << std::endl;
          }
        }
  ...
  if (TraceIsOn("bound-int"))
  {
    ...
    for (unsigned i = 0; i < f[0].getNumChildren(); i++)
    {
      ...
      if (std::find(d_set[f].begin(), d_set[f].end(), v) != d_set[f].end())
      {
        ...
        {
          Trace("bound-int") << "  " << d_bounds[0][f][v] << " <= " << v
                             << " <= " << d_bounds[1][f][v] << " (range is "
                             << d_range[f][v] << ")" << std::endl;
        }
        else if (d_bound_type[f][v] == BOUND_SET_MEMBER)
        {
          ...
          {
            Trace("bound-int")
                << "  " << v << " in " << d_setm_range[f][v] << std::endl;
          }
          else
          {
            Trace("bound-int") << "  " << v << " unifiable in "
                               << d_setm_range_lit[f][v] << std::endl;
          }
        }
        ...
        {
          Trace("bound-int")
              << "  " << v << " has small finite type." << std::endl;
        }
        else
        {
      ...
      {
        Trace("bound-int") << "  " << "*** " << v << " is unbounded."
                           << std::endl;
      }
    }
  }
  ...
  for (unsigned i = 0; i < f[0].getNumChildren(); i++)
  {
    if (d_bound_type[f].find(f[0][i]) == d_bound_type[f].end())
    {
      Trace("bound-int-warn")
          << "Warning : Bounded Integers : Due to quantification on " << f[0][i]
          << ", could not find bounds for " << f << std::endl;
      bound_success = false;
      break;
    }
  }
...
void BoundedIntegers::getBounds(
    Node f, Node v, RepSetIterator* rsi, Node& l, Node& u)
{
  l = d_bounds[0][f][v];
  u = d_bounds[1][f][v];
...
void BoundedIntegers::getBoundValues(
    Node f, Node v, RepSetIterator* rsi, Node& l, Node& u)
{
  getBounds(f, v, rsi, l, u);
  Trace("bound-int-rsi") << "Get value in model for..." << l << " and " << u
                         << std::endl;
  if (!l.isNull())
  {
...
bool BoundedIntegers::getRsiSubsitution(Node q,
                                        Node v,
                                        std::vector<Node>& vars,
                                        std::vector<Node>& subs,
                                        RepSetIterator* rsi)
{
  Trace("bound-int-rsi") << "Get bound value in model of variable " << v
                         << std::endl;
  ...
  for (int i = 0; i < vindex; i++)
  {
    ...
    Trace("bound-int-rsi") << "Look up the value for " << d_set[q][i] << " "
                           << i << std::endl;
  ...
  {
    if (d_bound_type[q][v] == BOUND_INT_RANGE
        || d_bound_type[q][v] == BOUND_SET_MEMBER)
    {
      // must add the lemma
      ...
      Trace("bound-int-lemma")
          << "*** Add lemma to minimize instantiated non-ground term " << lem
          << std::endl;
...
bool BoundedIntegers::getBoundElements(RepSetIterator* rsi,
                                       bool initial,
                                       Node q,
                                       Node v,
                                       std::vector<Node>& elements)
{
  if (initial || !isGroundRange(q, v))
  {
    ...
    if (bvt == BOUND_INT_RANGE)
    {
      ...
      {
        Trace("bound-int-warn")
            << "WARNING: Could not find integer bounds in model for " << v
            << " in " << q << std::endl;
      ...
      {
        NodeManager* nm = nodeManager();
        Trace("bound-int-rsi") << "Can limit bounds of " << v << " to " << l
                               << "..." << u << std::endl;
        ...
        {
          Trace("fmf-incomplete") << "Incomplete because of integer "
                                     "quantification, bounds are too big for "
                                  << v << "." << std::endl;
          return false;
        }
      }
    ...
    else if (bvt == BOUND_SET_MEMBER)
    {
      ...
      {
        Trace("bound-int-warn")
            << "WARNING: Could not find set bound in model for " << v << " in "
            << q << std::endl;
        return false;
      }
      else
      {
        Trace("bound-int-rsi")
            << "Bounded by set membership : " << srv << std::endl;
        if (srv.getKind() != Kind::SET_EMPTY)
        {
          // collect the elements
          while (srv.getKind() == Kind::SET_UNION)
          {
          ...
          // check if we need to do matching, for literals like ( tuple( v ) in
          // S )
          ...
          if (t != v)
          {
            ...
            elements_tmp.insert(
                elements_tmp.end(), elements.begin(), elements.end());
            elements.clear();
