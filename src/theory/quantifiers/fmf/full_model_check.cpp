...
int FullModelChecker::doExhaustiveInstantiation( FirstOrderModel * fm, Node f, int effort ) {
  ...
  if (effort == 0)
  {
    if (options::mbqiMode() == options::MbqiMode::NONE)
    {
      ...
      if (!exhaustiveInstantiate(fmfmc, f, c, InferenceId::QUANTIFIERS_INST_EXH_FMF))
      {
        return 0;
      }
      return 1;
    ...
    for (unsigned i = 0, msize = mcond.size(); i < msize; i++)
    {
      ...
      d_triedLemmas++;
      if (instq->addInstantiation(f, inst, InferenceId::QUANTIFIERS_INST_FMF, true))
      {
...
bool FullModelChecker::exhaustiveInstantiate(FirstOrderModelFmc * fm, Node f, Node c) {
  ...
  if (riter.setQuantifier(f))
  {
    ...
    while( !riter.isFinished() ){
      ...
      if (ev!=d_true) {
        ...
        if (d_qe->getInstantiate()->addInstantiation(f, inst, InferenceId::QUANTIFIERS_INST_FMF_EXH, true))
        {
