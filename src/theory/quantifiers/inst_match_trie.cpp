...
bool InstMatchTrie::removeInstMatch( Node q, std::vector< Node >& m, ImtIndexOrder* imtio, int index ) {
...
void InstMatchTrie::getExplanationForInstLemmas( Node q, std::vector< Node >& terms, const std::vector< Node >& lems, std::map< Node, Node >& quant, std::map< Node, std::vector< Node > >& tvec ) const {
  if( terms.size()==q[0].getNumChildren() ){
    if( hasInstLemma() ){
      Node lem = getInstLemma();
...
void CDInstMatchTrie::getExplanationForInstLemmas( Node q, std::vector< Node >& terms, const std::vector< Node >& lems, std::map< Node, Node >& quant, std::map< Node, std::vector< Node > >& tvec ) const {
  if( d_valid.get() ){
    if( terms.size()==q[0].getNumChildren() ){
      if( hasInstLemma() ){
        Node lem;
