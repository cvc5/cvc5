                                              ...
                                              unsigned iindex)
{
  if( iindex==vars.size() ){
    ...
    CVC4_DCHECK(pindex < q[2][0].getNumChildren());
    Node pat = q[2][0][pindex];
    CVC4_DCHECK(pat.getNumChildren() == 0 || paindex <= pat.getNumChildren());
    if( pat.getKind()==APPLY_UF ){
      CVC4_DCHECK(paindex <= pat.getNumChildren());
      if( paindex==pat.getNumChildren() ){
        getPartialInstantiations( conj, q, bvl, vars, terms, types, NULL, pindex+1, 0, iindex );
      }else{
        if( !curr ){
          CVC4_DCHECK(paindex == 0);
