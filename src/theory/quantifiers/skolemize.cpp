...
bool Skolemize::getSkolemConstants( Node q, std::vector< Node >& skolems ) {
  std::unordered_map< Node, std::vector< Node >, NodeHashFunction >::iterator it = d_skolem_constants.find(q);
...
void Skolemize::getSelfSel( const Datatype& dt, const DatatypeConstructor& dc, Node n, TypeNode ntn, std::vector< Node >& selfSel ){
  TypeNode tspec;
  if( dt.isParametric() ){
...
Node Skolemize::mkSkolemizedBody( Node f, Node n, std::vector< TypeNode >& argTypes, std::vector< TNode >& fvs,
