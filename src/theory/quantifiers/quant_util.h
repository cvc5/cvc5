 ...
 *   (b) v is null.
 * 
 *   For mconstant or mvariable t, we write [t] to denote 1 if t.isNull() and t otherwise.
 * 
 *   A monomial m is a pair ( mvariable, mconstant ) of the form ( v, c ), which
 *   is interpreted as [c]*[v].
 * 
 *   A {monmoial sum} msum is represented by a std::map< Node, Node > having key-value
 *   pairs of the form ( mvariable, mconstant ).  It is interpreted as:
...
public:
   ...
   * If literal lit is in rewritten form, this function should always return true.
   */
