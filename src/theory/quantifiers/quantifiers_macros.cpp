...
QuantifiersMacros::QuantifiersMacros(Env& env, QuantifiersRegistry& qr) : EnvObj(env), d_qreg(qr) {}
  ...
  else if (pol && n.getKind() == EQUAL)
  {
    ...
    for (const Node& m : candidates)
    {
      ...
      if (!containsBadOp(n_def, op, reqGround))
      {
        ...
        if (options().quantifiers.macrosQuantMode != options::MacrosQuantMode::GROUND_UF
...
bool QuantifiersMacros::preservesTriggerVariables(Node q, Node n)
{
  ...
  inst::PatternTermSelector::getTriggerVariables(d_env.getOptions(), icn, q, trigger_var);
