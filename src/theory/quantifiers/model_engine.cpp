...
int ModelEngine::checkModel(){
  ...
  //d_quantEngine->getEqualityQuery()->flattenRepresentatives( fm->getRepSet()->d_type_reps );

  //for debugging, setup
  for( std::map< TypeNode, std::vector< Node > >::iterator it = fm->getRepSetPtr()->d_type_reps.begin();
       it != fm->getRepSetPtr()->d_type_reps.end(); ++it ){
    if( it->first.isSort() ){
      ...
      if( d_quantEngine->getModel()->isQuantifierActive( f ) && d_quantEngine->hasOwnership( f, this ) ){
        ...
        for( unsigned j=0; j<f[0].getNumChildren(); j++ ){
          ...
          if( fm->getRepSet()->hasType( tn ) ){
            totalInst = totalInst * (int)fm->getRepSet()->getNumRepresentatives( tn );
          }
        }
...
void ModelEngine::exhaustiveInstantiate( Node f, int effort ){
  ...
  if( retEi!=0 ){
    ...
    RepSetIterator riter( d_quantEngine, d_quantEngine->getModel()->getRepSetPtr() );
