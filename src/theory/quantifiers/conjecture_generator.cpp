...
void ConjectureGenerator::generateConjectures()
{
  ...
  for (unsigned depth = 1; depth <= maxDepth; depth++)
  {
    ...
    // Trace("conjecture-count") << "Total LHS of depth " << depth << " : " <<
    // conj_lhs[depth].size() << std::endl;

    /* test...
    for( unsigned i=0; i<rt_types.size(); i++ ){
      Trace("sg-term-enum") << "Term enumeration for " << rt_types[i] << " : "
    << std::endl; Trace("sg-term-enum") << "Ground term : " <<
    rt_types[i].mkGroundTerm() << std::endl; for( unsigned j=0; j<150; j++ ){
        Trace("sg-term-enum") << "  " << getEnumerateTerm( rt_types[i], j ) <<
    std::endl;
      }
    }
    */
