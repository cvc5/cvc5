...
Node OpArgIndex::getGroundTerm(ConjectureGenerator* s, std::vector<TNode>& args)
{
  if (d_ops.empty())
  {
    for (std::map<TNode, OpArgIndex>::iterator it = d_child.begin();
         it != d_child.end();
         ++it)
    {
...
void OpArgIndex::getGroundTerms(ConjectureGenerator* s,
...
void ConjectureGenerator::eqNotifyMerge(TNode t1, TNode t2)
{
  ...
  Trace("thm-ee-debug") << "UEE : equality holds : " << t1 << " == " << t2
                        << std::endl;
  Trace("thm-ee-debug") << "      ureps : " << rt1 << " == " << rt2
                        << std::endl;
  Trace("thm-ee-debug") << "      relevant : " << d_pattern_is_relevant[rt1]
                        << " " << d_pattern_is_relevant[rt2] << std::endl;
  Trace("thm-ee-debug") << "      normal : " << d_pattern_is_normal[rt1] << " "
                        << d_pattern_is_normal[rt2] << std::endl;
  Trace("thm-ee-debug") << "      size :   " << d_pattern_fun_sum[rt1] << " "
                        << d_pattern_fun_sum[rt2] << std::endl;
...
ConjectureGenerator::EqcInfo* ConjectureGenerator::getOrMakeEqcInfo(TNode n,
                                                                    bool doMake)
{
...
Node ConjectureGenerator::getUniversalRepresentative(TNode n, bool add)
{
  if (add)
  {
    if (d_urelevant_terms.find(n) == d_urelevant_terms.end())
    {
      ...
      // addding this term to equality engine will lead to a set of new terms
      // (the new subterms of n) now, do instantiation-based merging for each of
      // these terms
      Trace("thm-ee-debug")
          << "Merge equivalence classes based on instantiations of terms..."
          << std::endl;
      // merge all pending equalities
      EntailmentCheck* echeck = d_treg.getEntailmentCheck();
      while (!d_upendingAdds.empty())
      {
        Trace("sg-pending") << "Add " << d_upendingAdds.size()
                            << " pending terms..." << std::endl;
        std::vector<Node> pending;
        pending.insert(
            pending.end(), d_upendingAdds.begin(), d_upendingAdds.end());
        ...
        for (unsigned i = 0; i < pending.size(); i++)
        {
          ...
          // if occurs modulo equality at ground level, it is equivalent to
          // representative of ground equality engine
          ...
          {
            Trace("thm-ee-add")
                << "UEE : Based on ground EE/theorem DB, it is equivalent to "
                << eq_terms.size() << " terms : " << std::endl;
...
{
  return std::find(d_ground_terms.begin(), d_ground_terms.end(), n)
         != d_ground_terms.end();
}
...
void ConjectureGenerator::check(CVC5_UNUSED Theory::Effort e, QEffort quant_e)
{
  if (quant_e == QEFFORT_STANDARD)
  {
    ...
    if (d_fullEffortCount % optFullCheckFrequency() == 0)
    {
      ...
      Trace("sg-stats") << "Total conjectures considered : " << d_conj_count
                        << std::endl;
      if (TraceIsOn("thm-ee"))
      {
        ...
        while (!ueqcs_i.isFinished())
        {
          ...
          eq::EqClassIterator ueqc_i =
              eq::EqClassIterator(r, &d_uequalityEngine);
    ...
    const std::vector<Node>& provenConj)
{
  if (TraceIsOn("sg-conjecture"))
  {
    ...
    for (unsigned i = 0; i < d_conjectures.size(); i++)
    {
      ...
      if (std::find(provenConj.begin(), provenConj.end(), q)
          == provenConj.end())
      {
...
unsigned ConjectureGenerator::flushWaitingConjectures(unsigned& addedLemmas,
                                                      int ldepth,
                                                      int rdepth)
{
  if (!d_waiting_conjectures_lhs.empty())
  {
    Trace("sg-proc") << "Generated " << d_waiting_conjectures_lhs.size()
                     << " conjectures at depth " << ldepth << "/" << rdepth
                     << "." << std::endl;
    if ((int)addedLemmas < options().quantifiers.conjectureGenPerRound)
    {
      /*
      ...
      if( doSort ){
        ...
        scs.d_scores.insert( scs.d_scores.begin(),
      d_waiting_conjectures_score.begin(), d_waiting_conjectures_score.end() );
        std::sort( indices.begin(), indices.end(), scs );
      }
      //if( doSort &&
      d_waiting_conjectures_score[indices[0]]<optFilterScoreThreshold() ){
      */
      ...
      for (unsigned i = 0; i < d_waiting_conjectures_lhs.size(); i++)
      {
        if (d_waiting_conjectures_score[i] >= optFilterScoreThreshold())
        {
          ...
          {
            Trace("sg-engine") << "*** Consider conjecture : " << lhs
                               << " == " << rhs << std::endl;
            Trace("sg-engine-debug")
                << "      score : " << d_waiting_conjectures_score[i]
                << std::endl;
            if (optStatsOnly())
            {
              d_conj_count++;
      ...
      Trace("sg-proc") << "...have now added " << addedLemmas
                       << " conjecture lemmas." << std::endl;
      if (optStatsOnly())
      {
        Trace("sg-stats") << "Generated " << (d_conj_count - prevCount)
                          << " conjectures at depth " << ldepth << "/" << rdepth
                          << "." << std::endl;
      }
    }
...
bool ConjectureGenerator::considerTermCanon(Node ln, bool genRelevant)
{
  if (!ln.isNull())
  {
    ...
    {
      Trace("sg-gen-consider-term")
          << "Do not consider term, " << ln
...
unsigned ConjectureGenerator::collectFunctions(
    TNode opat,
    TNode pat,
...
{
  if (std::find(d_patterns[tpat].begin(), d_patterns[tpat].end(), pat)
      == d_patterns[tpat].end())
  {
    ...
    // collect functions
    std::map<TypeNode, unsigned> mnvn;
    d_pattern_fun_sum[pat] = collectFunctions(
        pat, pat, d_pattern_fun_id[pat], mnvn, d_pattern_var_id[pat]);
...
bool ConjectureGenerator::isGeneralization(TNode patg,
                                           TNode pat,
...
int ConjectureGenerator::calculateGeneralizationDepth(TNode n,
                                                      std::vector<TNode>& fv)
{
...
void ConjectureGenerator::getEnumerateUfTerm(Node n,
                                             unsigned num,
                                             std::vector<Node>& terms)
{
  if (n.getNumChildren() > 0)
  {
    Trace("sg-gt-enum-debug")
        << "Get enumerate uf terms " << n << " (" << num << ")" << std::endl;
      ...
      if (index < vec.size())
      {
        ...
        if (vec.size() == n.getNumChildren())
        {
          ...
          if (!lc.isNull())
          {
            ...
            Node nenum = nodeManager()->mkNode(Kind::APPLY_UF, children);
            Trace("sg-gt-enum")
...
void ConjectureGenerator::getEnumeratePredUfTerm(Node n,
                                                 unsigned num,
                                                 std::vector<Node>& terms)
{
...
void ConjectureGenerator::processCandidateConjecture(TNode lhs,
                                                     TNode rhs,
                                                     unsigned lhs_depth,
                                                     unsigned rhs_depth)
{
  ...
  if (score > 0)
  {
    Trace("sg-conjecture") << "* Candidate conjecture : " << lhs
                           << " == " << rhs << std::endl;
    Trace("sg-conjecture-debug")
        << "     LHS, RHS generalization depth : " << lhs_depth << ", "
        << rhs_depth << std::endl;
    Trace("sg-conjecture-debug")
        << "     confirmed = " << d_subs_confirmCount
        << ", #witnesses range = " << d_subs_confirmWitnessRange.size() << "."
        << std::endl;
    Trace("sg-conjecture-debug") << "     #witnesses for ";
    bool firstTime = true;
    for (std::map<TNode, std::vector<TNode> >::iterator it =
             d_subs_confirmWitnessDomain.begin();
         it != d_subs_confirmWitnessDomain.end();
         ++it)
    {
      if (!firstTime)
      {
    ...
    Trace("sg-conjecture-debug")
        << "     unknown = " << d_subs_unkCount << std::endl;
...
int ConjectureGenerator::considerCandidateConjecture(TNode lhs, TNode rhs)
{
  ...
  Trace("sg-cconj-debug") << "Consider candidate conjecture : " << lhs
                          << " == " << rhs << "?" << std::endl;
  if (lhs == rhs)
  {
...
bool ConjectureGenerator::notifySubstitution(TNode glhs,
                                             std::map<TNode, TNode>& subs,
                                             TNode rhs)
{
  if (TraceIsOn("sg-cconj-debug"))
  {
    Trace("sg-cconj-debug") << "Ground eqc for LHS : " << glhs
                            << ", based on substitution: " << std::endl;
    for (std::map<TNode, TNode>::iterator it = subs.begin(); it != subs.end();
         ++it)
    {
  ...
  Trace("sg-cconj-debug") << "...done evaluating term, got : " << grhs
                          << std::endl;
  if (!grhs.isNull())
  {
    if (glhs != grhs)
    {
      ...
      if (itl != d_ground_eqc_map.end())
      {
        ...
        {
          Trace("sg-cconj-debug") << "We have ground terms " << itl->second
                                  << " and " << itr->second << "." << std::endl;
          if (itl->second.isConst() && itr->second.isConst())
          {
            Trace("sg-cconj-debug") << "...disequal constants." << std::endl;
            Trace("sg-cconj-witness")
                << "  Witness of falsification : " << itl->second
                << " != " << itr->second << ", substutition is : " << std::endl;
            for (std::map<TNode, TNode>::iterator it = subs.begin();
                 it != subs.end();
                 ++it)
            {
              Trace("sg-cconj-witness")
    ...
    if (isGroundSubs)
    {
      if (glhs == grhs)
      {
        Trace("sg-cconj-witness") << "  Witnessed " << glhs << " == " << grhs
             ...
             ++it)
        {
          Trace("sg-cconj-witness")
              << "    " << it->first << " -> " << it->second << std::endl;
          if (std::find(d_subs_confirmWitnessDomain[it->first].begin(),
                        d_subs_confirmWitnessDomain[it->first].end(),
                        it->second)
              == d_subs_confirmWitnessDomain[it->first].end())
          {
        ...
        d_subs_confirmCount++;
        if (std::find(d_subs_confirmWitnessRange.begin(),
                      d_subs_confirmWitnessRange.end(),
                      glhs)
            == d_subs_confirmWitnessRange.end())
        {
      ...
      else
      {
        if (optFilterUnknown())
        {
          Trace("sg-cconj-debug") << "...ground substitution giving terms that "
                                     "are neither equal nor disequal."
                                  << std::endl;
          return false;
        }
      }
...
bool TermGenerator::getNextTerm(TermGenEnv* s, unsigned depth)
{
  if (TraceIsOn("sg-gen-tg-debug2"))
  {
    Trace("sg-gen-tg-debug2")
        << this << " getNextTerm depth " << depth << " : status = " << d_status
        << ", num = " << d_status_num;
    if (d_status == 5)
    {
  ...
  if (d_status == 0)
  {
    ...
    if (!d_typ.isNull())
    {
      if (s->allowVar(d_typ))
      {
        ...
        s->addVar(d_typ);
        Trace("sg-gen-tg-debug2")
            << this << " ...return unique var #" << d_status_num << std::endl;
  ...
  else if (d_status == 2)
  {
    ...
    if ((d_status_num + 1) < (int)s->getNumTgVars(d_typ))
    {
      ...
      Trace("sg-gen-tg-debug2")
          << this << "...consider other var #" << d_status_num << std::endl;
      return s->considerCurrentTerm() ? true : getNextTerm(s, depth);
    }
    else
    {
  ...
  else if (d_status == 4)
  {
    ...
    if (depth > 0 && (d_status_num + 1) < (int)s->getNumTgFuncs(d_typ))
    {
      ...
      Trace("sg-gen-tg-debug2")
          ...
          << s->getTgFunc(d_typ, d_status_num) << std::endl;
      s->d_tg_gdepth++;
  ...
  else if (d_status == 5)
  {
    ...
    else
    {
      ...
      if (d_status_child_num == (int)d_children.size())
      {
        ...
        s->d_tg_alloc[d_children[d_status_child_num]].reset(
            s, s->d_func_args[f][d_status_child_num]);
        return getNextTerm(s, depth);
      }
      else
      {
        Assert(d_status_child_num + 1 == (int)d_children.size());
        if (s->d_tg_alloc[d_children[d_status_child_num]].getNextTerm(
                s, depth - 1))
        {
          d_status_child_num++;
          return getNextTerm(s, depth);
  ...
  else if (d_status == 1 || d_status == 3)
  {
    if (d_status == 1)
    {
      ...
      // check if there is only one feasible equivalence class.  if so, don't
      // make pattern any more specific. unsigned i =
      // s->d_ccand_eqc[0].size()-1; if( s->d_ccand_eqc[0][i].size()==1 &&
      // s->d_ccand_eqc[1][i].empty() ){
      //   d_status = 6;
      //   return getNextTerm( s, depth );
      // }
      s->d_tg_gdepth++;
...
bool TermGenerator::getNextMatch(
    TermGenEnv* s,
    TNode eqc,
    std::map<TypeNode, std::map<unsigned, TNode> >& subs,
    std::map<TNode, bool>& rev_subs)
{
  ...
  if (TraceIsOn("sg-gen-tg-match"))
  {
    ...
    Trace("sg-gen-tg-match")
        << " with eqc e" << s->d_cg->d_em[eqc] << "..." << std::endl;
    ...
    for (std::map<TypeNode, std::map<unsigned, TNode> >::iterator itt =
             subs.begin();
         itt != subs.end();
         ++itt)
    {
      for (std::map<unsigned, TNode>::iterator it = itt->second.begin();
           it != itt->second.end();
           ++it)
      {
        Trace("sg-gen-tg-debug2")
            << " " << it->first << " -> e" << s->d_cg->d_em[it->second];
      }
    }
    Trace("sg-gen-tg-debug2") << " } " << std::endl;
  }
  ...
  else if (d_status == 5)
  {
    ...
    else if (d_match_status == 0)
    {
      ...
      if (d_match_status_child_num == (int)d_match_children.size())
      {
        ...
        {
          d_match_children.push_back(
              d_match_children[d_match_status_child_num - 1]
                  ->second.d_data.begin());
          d_match_children_end.push_back(
              d_match_children[d_match_status_child_num - 1]
                  ->second.d_data.end());
        }
      }
      ...
      if (d_match_children[d_match_status_child_num]
      ...
      else
      {
        if (d_match_status_child_num == (int)d_children.size())
        {
          ...
          d_match_status_child_num--;
          return true;  // return
                        // d_match_children[d_match_status]!=d_match_children_end[d_match_status];
        }
        else
        {
          // do next binding
    ...
    else
    {
      ...
      if (s->d_tg_alloc[d_children[d_match_status_child_num]].getNextMatch(
              s,
              d_match_children[d_match_status_child_num]->first,
              subs,
              rev_subs))
      {
        d_match_status_child_num++;
    ...
    TermGenEnv* s, std::map<TypeNode, std::vector<int> >& fvs)
{
  ...
  else
  {
    ...
    if (it != fvs.end())
    {
      if (std::find(it->second.begin(), it->second.end(), d_status_num)
          != it->second.end())
      {
        return 1;
      }
...
void TermGenEnv::collectSignatureInformation()
{
  ...
  for (size_t i = 0, nops = tdb->getNumOperators(); i < nops; i++)
  {
    ...
    if (!dbl->d_list.empty())
    {
      ...
      Trace("sg-rel-sig-debug")
          << "Done check in signature : " << nn << std::endl;
    }
  }
  // shuffle functions
...
// get term
Node TermGenEnv::getTerm() { return d_tg_alloc[0].getTerm(this); }
...
unsigned TermGenEnv::getNumTgVars(TypeNode tn) { return d_var_id[tn]; }
...
void TermGenEnv::addVar(TypeNode tn) { d_var_id[tn]++; }
...
bool TermGenEnv::considerCurrentTerm()
{
  ...
  Trace("sg-gen-tg-debug") << "?  curr term size = " << d_tg_alloc.size()
                           << ", last status = " << d_tg_alloc[i - 1].d_status;
  Trace("sg-gen-tg-debug") << std::endl;

  if (d_tg_gdepth_limit >= 0
      && d_tg_alloc[0].getGeneralizationDepth(this)
             > (unsigned)d_tg_gdepth_limit)
  {
    ...
    Trace("sg-gen-consider-term") << " is too high " << d_tg_gdepth << " "
                                  << d_tg_alloc[0].getGeneralizationDepth(this)
                                  << ", do not consider." << std::endl;
    return false;
  }
  ...
  */
  ...
  if (d_gen_relevant_terms)
  {
    ...
    Trace("sg-gen-tg-debug")
        << ", #eqc to try = " << d_ccand_eqc[0][i - 1].size() << "/"
        << d_ccand_eqc[1][i - 1].size() << std::endl;
    ...
    for (unsigned r = 0; r < 2; r++)
    {
      for (unsigned j = 0; j < d_ccand_eqc[r][i - 1].size(); j++)
      {
        ...
        d_tg_alloc[0].resetMatching(mode);
        if (d_tg_alloc[0].getNextMatch(
                this, d_ccand_eqc[r][i - 1][j], subs, rev_subs))
        {
    ...
    if (d_ccand_eqc[0][i].empty())
    {
      ...
      d_tg_alloc[0].debugPrint(
          this, "sg-gen-consider-term", "sg-gen-consider-term-debug");
      Trace("sg-gen-consider-term")
          << " since no relevant EQC matches it." << std::endl;
      return false;
    }
    ...
    if (d_ccand_eqc[1][i].empty())
    {
      ...
      d_tg_alloc[0].debugPrint(
          this, "sg-gen-consider-term", "sg-gen-consider-term-debug");
      Trace("sg-gen-consider-term")
          << " since no ground EQC matches it." << std::endl;
      return false;
    }
  }
...
TermDb* TermGenEnv::getTermDatabase() { return d_cg->getTermDatabase(); }
Node TermGenEnv::getGroundEqc(TNode r) { return d_cg->getGroundEqc(r); }
bool TermGenEnv::isGroundEqc(TNode r) { return d_cg->isGroundEqc(r); }
bool TermGenEnv::isGroundTerm(TNode n) { return d_cg->isGroundTerm(n); }

void SubstitutionIndex::addSubstitution(TNode eqc,
...
bool SubstitutionIndex::notifySubstitutions(ConjectureGenerator* s,
                                            std::map<TNode, TNode>& subs,
                                            TNode rhs,
                                            unsigned numVars,
                                            unsigned i)
{
  ...
  else
  {
    ...
    for (std::map<TNode, SubstitutionIndex>::iterator it = d_children.begin();
         it != d_children.end();
         ++it)
    {
      Trace("sg-cconj-debug2") << "Try " << d_var << " -> " << it->first << " ("
                               << i << "/" << numVars << ")" << std::endl;
      subs[d_var] = it->first;
...
void TheoremIndex::addTheorem(std::vector<TNode>& lhs_v,
...
void TheoremIndex::addTheoremNode(TNode curr,
                                  std::vector<TNode>& lhs_v,
                                  std::vector<unsigned>& lhs_arg,
                                  TNode rhs)
{
  Trace("thm-db-debug") << "Adding conjecture for subterm " << curr << "..."
                        << std::endl;
  if (curr.hasOperator())
  {
...
void TheoremIndex::getEquivalentTerms(std::vector<TNode>& n_v,
                                      ...
                                      std::vector<Node>& terms)
{
  Trace("thm-db-debug") << "Get equivalent terms " << n_v.size() << " "
                        << n_arg.size() << std::endl;
  if (n_v.empty())
  {
    Trace("thm-db-debug") << "Number of terms : " << d_terms.size()
                          << std::endl;
    // apply substutitions to RHS's
    for (unsigned i = 0; i < d_terms.size(); i++)
    {
      Node n = d_terms[i].substitute(
  ...
  else
  {
    ...
    {
      n_arg[index]++;
      getEquivalentTermsNode(
          n_v[index][n_arg[index] - 1], n_v, n_arg, smap, vars, subs, terms);
    }
  }
}

void TheoremIndex::getEquivalentTermsNode(Node curr,
                                          ...
                                          std::vector<Node>& terms)
{
  Trace("thm-db-debug") << "Get equivalent based on subterm " << curr << "..."
                        << std::endl;
  if (curr.hasOperator())
  {
    Trace("thm-db-debug") << "Check based on operator..." << std::endl;
    std::map<TNode, TheoremIndex>::iterator it =
        d_children.find(curr.getOperator());
  ...
  if (itt != d_var.end())
  {
    Trace("thm-db-debug") << "Check for substitution with " << itt->second
                          << "..." << std::endl;
    ...
    if (success)
    {
      d_children[itt->second].getEquivalentTerms(
          n_v, n_arg, smap, vars, subs, terms);
    }
  }
}
...
{
  for (std::map<TNode, TheoremIndex>::iterator it = d_children.begin();
       it != d_children.end();
       ++it)
  {
