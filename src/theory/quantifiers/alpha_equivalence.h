...
class AlphaEquivalence : protected EnvObj
{
 ...
 private:
  /**
   * Add the alpha equivalence step f = f { vars -> subs } to cdp, where we ensure that
   * f does not contain vars. Return the conclusion of this step.
   */
  Node addAlphaEquivStep(CDProof& cdp, const Node& f, const std::vector<Node>& vars, const std::vector<Node>& subs);
  /** a term canonizer */
