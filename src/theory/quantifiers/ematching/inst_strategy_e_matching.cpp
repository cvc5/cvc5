...
void InstStrategyAutoGenTriggers::processResetInstantiationRound( Theory::Effort effort ){
  ...
  for( unsigned r=0; r<2; r++ ){
    ...
    for( std::pair< const Node, std::map< inst::Trigger*, bool > >& agt : agts){
      for( std::map< inst::Trigger*, bool >::iterator it = agt.second.begin(); it != agt.second.end(); ++it){
...
int InstStrategyAutoGenTriggers::process( Node f, Theory::Effort effort, int e ){
  ...
  if (options::triggerActiveSelMode() != options::TriggerActiveSelMode::ALL)
  {
    ...
    for( std::map< Trigger*, bool >::iterator it = agt.begin(); it != agt.end(); ++it ){
      Trigger* t = it->first;
...
void InstStrategyAutoGenTriggers::generateTriggers( Node f ){
  ...
  {
    Trace("auto-gen-trigger-debug") << "...failed to generate pattern terms" << std::endl;
    return;
  }
...
bool InstStrategyAutoGenTriggers::generatePatternTerms(Node f)
{
  ...
  for (const Node& p : patTermsF){
    ...
    if (ntrivTriggers && !newVar && last_weight != -1 && curr_w > last_weight
        && curr_w >= 2)
    {
      Trace("auto-gen-trigger-debug") << "...exclude expendible non-trivial trigger : " << p << std::endl;
      rmPatTermsF[p] = true;
    }else{
      last_weight = curr_w;
    }
