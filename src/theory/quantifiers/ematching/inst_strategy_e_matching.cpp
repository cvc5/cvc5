...
void InstStrategyUserPatterns::addUserPattern( Node q, Node pat ){
  CVC4_DCHECK(pat.getKind() == INST_PATTERN);
  //add to generators
  bool usable = true;
...
void InstStrategyAutoGenTriggers::generateTriggers( Node f ){
  ...
  if( d_patTerms[0].find( f )==d_patTerms[0].end() ){
    ...
    if( patTermsF.empty() ){
      ...
      if( Trace.isOn("auto-gen-trigger-debug") ){
        ...
        for( unsigned i=0; i<patTermsF.size(); i++ ){
          CVC4_DCHECK(tinfo.find(patTermsF[i]) != tinfo.end());
    ...
    int last_weight = -1;
    for( unsigned i=0; i<patTermsF.size(); i++ ){
      CVC4_DCHECK(patTermsF[i].getKind() != NOT);
      bool newVar = false;
    ...
    for( unsigned i=0; i<patTermsF.size(); i++ ){
      ...
      if( rmPatTermsF.find( pat )==rmPatTermsF.end() ){
        ...
        CVC4_DCHECK(tinfo.find(pat) != tinfo.end());
        ...
        if( rpol!=0 ){
          CVC4_DCHECK(rpol == 1 || rpol == -1);
          if( Trigger::isRelationalTrigger( pat ) ){
            pat = rpol==-1 ? pat.negate() : pat;
          }else{
            CVC4_DCHECK(Trigger::isAtomicTrigger(pat));
  ...
  for( unsigned r=rmin; r<=rmax; r++ ){
    ...
    if( !patTerms.empty() ){
      ...
      if( options::relevantTriggers() ){
        CVC4_DCHECK(d_quant_rel);
        sortQuantifiersForSymbol sqfs;
        sqfs.d_quant_rel = d_quant_rel;
        for( unsigned i=0; i<patTerms.size(); i++ ){
          CVC4_DCHECK(d_pat_to_mpat.find(patTerms[i]) != d_pat_to_mpat.end());
          CVC4_DCHECK(d_pat_to_mpat[patTerms[i]].hasOperator());
...
void InstStrategyAutoGenTriggers::addUserNoPattern( Node q, Node pat ) {
  CVC4_DCHECK(pat.getKind() == INST_NO_PATTERN && pat.getNumChildren() == 1);
