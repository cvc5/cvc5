...
RelationalMatchGenerator::RelationalMatchGenerator(Trigger* tparent, Node rtrigger, bool hasPol, bool pol) : 
InstMatchGenerator(tparent, Node::null()),
d_relTrigger(rtrigger), d_hasPol(hasPol), d_pol(pol), d_counter(0){
  Assert ((rtrigger.getKind()==EQUAL && rtrigger[0].getType().isReal()) || rtrigger.getKind()==GEQ);
  ...
  AlwaysAssert(!d_var.isNull()) << "Failed to initialize RelationalMatchGenerator";
}
