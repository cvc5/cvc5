...
int InstMatchGenerator::getMatch(Node f, Node t, InstMatch& m)
{
  ...
  if (!d_eq_class_rel.isNull() && d_eq_class_rel.getKind() == INST_CONSTANT)
  {
    ...
    if (pol)
    {
      ...
      {
        t_match =
            nm->mkNode(MINUS, t, nm->mkConstRealOrInt(t.getType(), Rational(1)));
      }else{
        t_match = t;
      }
    }
    else
    {
      if (pat.getKind() == EQUAL)
      {
        ...
        else
        {
          ...
          t_match =
              nm->mkNode(PLUS, t, nm->mkConstRealOrInt(tn, Rational(1)));
        }
      }
      else if (pat.getKind() == GEQ)
      {
        t_match = nm->mkNode(PLUS, t, nm->mkConstRealOrInt(t.getType(), Rational(1)));
      }
