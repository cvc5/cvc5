...
bool IMGenerator::sendInstantiation(InstMatch& m,
                                    InferenceId id)
{
      ...
      if (!qa.isNull())
      {
        ...
        else
        {
          InstMatchGenerator* cimg =
              getInstMatchGenerator(d_tparent, q, pat);
          if (cimg)
          {
  ...
  if (success)
  {
    ...
    if (success)
    {
      ...
      ret_val = continueNextMatch(
          f, m, InferenceId::QUANTIFIERS_INST_E_MATCHING);
    }
  }
  if (ret_val < 0)
  {
