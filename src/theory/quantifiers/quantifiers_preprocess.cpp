...
bool QuantifiersRewriter::isLiteral(Node n)
{
  switch (n.getKind())
  {
    case NOT: return n[0].getKind() != NOT && isLiteral(n[0]); break;
    case OR:
    case AND:
    case IMPLIES:
    case XOR:
    case ITE: return false; break;
    case EQUAL:
      // for boolean terms
...
RewriteResponse QuantifiersRewriter::preRewrite(TNode in)
{
  if (in.getKind() == kind::EXISTS || in.getKind() == kind::FORALL)
  {
    ...
    if (doRewrite)
    {
      ...
      if (in != n)
      {
        Trace("quantifiers-pre-rewrite")
            << "*** pre-rewrite " << in << std::endl;
...
RewriteResponse QuantifiersRewriter::postRewrite(TNode in)
{
  ...
  if (in != ret)
  {
    Trace("quantifiers-rewrite")
        << "*** rewrite (op=" << rew_op << ") " << in << std::endl;
...
// eliminates IMPLIES/XOR, removes duplicates/infers tautologies of AND/OR, and
// computes NNF
Node QuantifiersRewriter::computeElimSymbols(Node body)
{
  ...
  else if (ok == NOT)
  {
    ...
    else if (body[0].getKind() == AND)
    {
      ...
      body = body[0];
             ...
             || (body[0].getKind() == EQUAL
                 && body[0][0].getType().isBoolean()))
    {
      k = EQUAL;
  ...
  {
    Node c = computeElimSymbols(
        (i == 0 && negCh1) != negAllCh ? body[i].negate() : body[i]);
    bool success = true;
...
{
  if (n.getKind() == ITE && n[0].getKind() == APPLY_TESTER
      && n[1].getType().isBoolean())
  {
    Trace("quantifiers-rewrite-ite-debug")
        << "Split tester condition : " << n << std::endl;
    Node x = n[0][0];
    std::map<Node, Node>::iterator itp = pcons.find(x);
    if (itp != pcons.end())
    {
      Trace("quantifiers-rewrite-ite-debug")
          << "...condition already set " << itp->second << std::endl;
      computeDtTesterIteSplit(
          n[itp->second == n[0] ? 1 : 2], pcons, ncons, conj);
    }
    else
    {
      ...
      {
        Trace("quantifiers-rewrite-ite-debug")
            << "...condition negated " << itn->second << std::endl;
  ...
  else
  {
    ...
    Trace("quantifiers-rewrite-ite-debug")
        << "Return value : " << n << std::endl;
    std::vector<Node> children;
    children.push_back(n);
    std::vector<Node> vars;
    // add all positive testers
    for (std::map<Node, Node>::iterator it = pcons.begin(); it != pcons.end();
         ++it)
    {
    ...
    for (std::map<Node, std::map<int, Node>>::iterator it = ncons.begin();
         it != ncons.end();
         ++it)
    {
      Node x = it->first;
                                              ...
                                              QAttributes& qa)
{
  ...
  if (qa.isFunDef())
  {
    ...
    Trace("quantifiers-rewrite-debug")
        << "Decompose " << h << " / " << fbody << " as function definition for "
        << q << "." << std::endl;
    if (!fbody.isNull())
    {
