...
void BvInstantiatorPreprocess::registerCounterexampleLemma( std::vector< Node >& lems, std::vector< Node >& ce_vars ) 
{
  d_extract_map.clear();
  std::unordered_set<TNode, TNodeHashFunction> visited;
  for( unsigned i=0; i<lems.size(); i++ ){
    Trace("cegqi-bv-pp") << "Register ce lemma # " << i << " : " << lems[i] << std::endl;
  ...
  for( std::pair< Node, std::vector< Node > > es : d_extract_map ){
    if( es.second.size()>1 ){
      ...
      for( unsigned i=0; i<curr_vec.size(); i++ ){
        ...
        BitVectorExtract e = curr_vec[i].getOperator().getConst<BitVectorExtract>();
      ...
      do
      {
        ...
        for( unsigned i=0; i<curr_vec.size(); i++ ){
          ...
          if( bounds[0][n]<curr_index ){
            ...
            if( diff>1 ){
              Node ex = bv::utils::mkExtract(es.first,curr_index-1,bounds[0][n]);
              children.push_back(ex);
            }
            
            // make a variable corresponding to this segment
            Node var = NodeManager::currentNM()->mkSkolem("ek",n.getType(),"variable to represent disjoint extract region");
        ...
        if( tmp_vars.size()>1 ){
          ...
          success = true;
          Node conc = NodeManager::currentNM()->mkNode( kind::BITVECTOR_CONCAT, children );
      ...
      Trace("cegqi-bv-pp") << "...finished processing extracts for term " << es.first << std::endl;
    }
  }
  if( !vars.empty() ){
    // do subs -> vars 
    for( unsigned i=0; i<lems.size(); i++ ){
      Node slem = lems[i].substitute( subs.begin(), subs.end(), vars.begin(), vars.end() );
      if( slem!=lems[i] ){
        Trace("cegqi-bv-pp-debug") << "Substitution applied to lemma # " << i << " : " << std::endl;
        Trace("cegqi-bv-pp-debug") << "   " << lems[i] << std::endl;
        Trace("cegqi-bv-pp-debug") << "-> " << slem << std::endl;
        lems[i] = slem;
      }
    }
    Trace("cegqi-bv-pp-debug") << "Adding " <<  new_lems.size() << " lemmas..." << std::endl;
    lems.insert( lems.end(), new_lems.begin(), new_lems.end() );
    Trace("cegqi-bv-pp-debug") << "Adding " <<  vars.size() << " variables..." << std::endl;
    ce_vars.insert( ce_vars.end(), vars.begin(), vars.end() );
  }
}

void BvInstantiatorPreprocess::process( Node lem, std::unordered_set<TNode, TNodeHashFunction>& visited ) 
{
