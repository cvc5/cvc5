...
/** CegConjecturePbe
*
* This class implements optimizations that target Programming-By-Examples (PBE) synthesis conjectures.
* [EX#1] An example of a PBE synthesis conjecture is:
*
* exists f. forall x. ( x = 0 => f( x ) = 2 ) ^ ( x = 5 => f( x ) = 7 ) ^ ( x = 6 => f( x ) = 8 )
*
...
private:
  ...
  /** for each candidate variable f (a function-to-synthesize), whether the conjecture is purely PBE for that variable
  * In other words, all occurrences of f are guarded by equalities that constraint its arguments to constants.
  */
  std::map< Node, bool > d_examples_invalid;
  /** for each candidate variable (function-to-synthesize), whether the conjecture is purely PBE for that variable.
  * An example of a conjecture for which d_examples_invalid is false but d_examples_out_invalid is true is:
  * exists f. forall x. ( x = 0 => f( x ) > 2 )
  */
  std::map< Node, bool > d_examples_out_invalid;
  /** for each candidate variable (function-to-synthesize), input of I/O examples */
  std::map< Node, std::vector< std::vector< Node > > > d_examples;
  /** for each candidate variable (function-to-synthesize), output of I/O examples */
  std::map< Node, std::vector< Node > > d_examples_out;
  /** the list of example terms (for the example [EX#1] above, this is f( 0 ), f( 5 ), f( 6 ) */
  std::map< Node, std::vector< Node > > d_examples_term;
  /** map from enumerators to candidate varaibles (function-to-synthesize). An enumerator may not be equivalent
  * to the candidate variable it maps so in synthesis-through-unification approaches (e.g. decision tree construction).
  */
