  ...
  if( q.getNumChildren()==3 ){
    ...
    for( unsigned i=0; i<q[2].getNumChildren(); i++ ){
      ...
      if( q[2][i].getKind()==INST_PATTERN || q[2][i].getKind()==INST_NO_PATTERN ){
        ...
        {
          qa.d_sygusSideCondition = avar.getAttribute(SygusSideConditionAttribute());
          Trace("quant-attr") << "Attribute : sygus side condition : " << qa.d_sygusSideCondition << " : " << q << std::endl;
        }
