...
public:
   ...
   */
  bool existsInstMatch( QuantifiersEngine* qe, Node q, InstMatch& m, bool modEq = false,
                        ImtIndexOrder* imtio = NULL, int index = 0 ) {
    return !addInstMatch( qe, q, m, modEq, imtio, true, index );
  }
  /** exists inst match, vector version */
  bool existsInstMatch( QuantifiersEngine* qe, Node q, std::vector< Node >& m, bool modEq = false,
                        ImtIndexOrder* imtio = NULL, int index = 0 ) {
    return !addInstMatch( qe, q, m, modEq, imtio, true, index );
  }
  /** add inst match 
   ...
   * equalities in the active equality engine of qe.
   */
  bool addInstMatch( QuantifiersEngine* qe, Node q, InstMatch& m, bool modEq = false,
   ...
   * The domain of m is the bound variables of quantified formula q.
   */
  bool removeInstMatch( Node f, std::vector< Node >& m, ImtIndexOrder* imtio = NULL, int index = 0 );
   ...
   */  
  void getExplanationForInstLemmas( Node q, const std::vector< Node >& lems, std::map< Node, Node >& quant, std::map< Node, std::vector< Node > >& tvec ) const {
    std::vector< Node > terms;
...
private:
   ...
   * terms accumulates the path we are on in the trie.
   */
  void getExplanationForInstLemmas( Node q, std::vector< Node >& terms, const std::vector< Node >& lems, std::map< Node, Node >& quant, std::map< Node, std::vector< Node > >& tvec ) const;
