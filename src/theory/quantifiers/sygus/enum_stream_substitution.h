...
class EnumStreamPermutation
{
 public:
   ...
   */
  EnumStreamPermutation(Node value, quantifiers::TermDbSygus* tds);
...
class EnumStreamSubstitution
{
 public:
   ...
   * variables in the value's type). For each variable subclass of this initial
   * set, a subset is chosen with as many variables as in the same variable
   * subclass of value's variables.
   *
   ...
   *
   */
  EnumStreamSubstitution(Node value, quantifiers::TermDbSygus* tds);
...
{
 public:
  EnumStreamConcrete(quantifiers::TermDbSygus* tds) : d_tds(tds){}
  ...
  {
    d_ess = new EnumStreamSubstitution(v,d_tds);
  }
