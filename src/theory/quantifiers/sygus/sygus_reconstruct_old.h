...
rule in T...
  ...
  while true
    // enumeration phase
    for each T
      s[z] = rewrite(nextEnum(T_{z}))
      if (s[z] is ground)
        let k be a new variable of type : typeOf(s[z])
    ...
    while Obs' != {}
      Obs'' = {}
      for each T, for each (k, t, s) in Obs'<T>
        if there exists a (k', t', s') in Obs<T> such that |=_X t' = t
          mergeObs(k', k) // need to implement
        else
          Obs<T> += (k, t, s)
...
matchNewObs(k, s[z]) : Obs'
{
  ...
  if u = t // u is a ground term (only concrete variables)
    ...
    CandSol<k> += s[k']
    Obs'<typeOf(z)> += (k', t', null) // could be duplicate
}
...
{
  if s is not ground
    instantiate with arbitrary sygus datatype values
  Sol<k> = s
