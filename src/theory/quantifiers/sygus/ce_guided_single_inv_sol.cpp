...
bool doCompare(Node a, Node b, Kind k)
{
  ...
  CVC4_DCHECK(com.getType().isBoolean());
    ...
    if( carg!=-1 ){
      ...
      if( status!=0 ){
        ...
        if( status!=0 ){
          ...
          {
            CVC4_DCHECK(dt[ii].getNumArgs() == 1);
...
bool CegSingleInvSol::collectReconstructNodes(int pid,
                                              ...
                                              int& status)
{
  CVC4_DCHECK(dtc.getNumArgs() == ts.size());
        ...
        if (rcons_depth < 1000)
        {
          ...
          if (pk != UNDEFINED_KIND)
          {
            ...
            if (arg != -1)
            {
              ...
              // get types
              CVC4_DCHECK(dt[arg].getNumArgs() == 2);
...
Node CegSingleInvSol::getGenericBase(TypeNode tn, const Datatype& dt, int c)
{
  ...
  TermDbSygus* tds = d_qe->getTermDatabaseSygus();
  CVC4_DCHECK(tds->isRegistered(tn));
