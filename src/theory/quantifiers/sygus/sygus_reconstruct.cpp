                                           ...
                                           int enumLimit)
{
  ...
  d_info.emplace(mainOb, ObligationInfo(sol));
  d_ob[stn][sol] = mainOb;
         ...
         d_unsolvedObs)
    {
      ...
      if (sz.isConst())
      {
        ...
        if (k == Node::null())
        {
          ...
          d_info.emplace(k, ObligationInfo(builtin));
                  ...
                  candObs))
  {
    ...
    for (const std::pair<const Node, Node>& candOb : candObs)
    {
      ...
      else
      {
        ...
        d_info.emplace(newVar, candOb.second);
    ...
    else
    {
      ...
      d_info[k].addCandidateSolution(sz);
      d_parentOb[sz] = k;
      d_info[d_subObs[sz].back()].addCandidateSolutionToWatchSet(sz);
    }
  }
...
void SygusReconstruct::markSolved(Node k, Node s)
{
  ...
  while (!stack.empty())
  {
    ...
    for (Node parent : d_info[curr].getWatchSet())
    {
      ...
      if (d_subObs[parent].empty())
      {
        ...
        {
          d_info[parentOb].addCandidateSolution(parentSol);
      ...
      else
      {
        ...
        d_info[d_subObs[parent].back()].addCandidateSolutionToWatchSet(parent);
      }
    }
  }
}
...
{
  return "ob<" + d_info.at(k).getBuiltin().toString() + ", "
         + k.getType().toString() + ">";
}
...
void SygusReconstruct::printCandSols(const Node& mainOb) const
{
  ...
  while (!stack.empty())
  {
    ...
    for (const Node& j : d_info.at(k).getCandidateSolutions())
    {
