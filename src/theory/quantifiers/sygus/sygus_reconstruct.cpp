                  ...
                  candObs))
  {
    ...
    for (const std::pair<const Node, Node>& candOb : candObs)
    {
      ...
      if (!repOb.isNull())
      {
        ...
        newVar = repOb;
        // the builtin term corresponding to the original obligation may not be
        // equivalent to this new one (up to rewriting)
        if (candOb.second != rep) {
          // in that case, add candOb to the set of builtin terms corresponding
          // to original obligation
