...
class SygusUnifRl : public SygusUnif
{
 public:
   ...
   * of the lemma based on the method purifyLemma below. The method adds
   * "evaluation points" to the map eval_pts, which are terms of the form:
   *   ev( e1, c1...cn )
   * where ev is an evaluation function for the sygus deep embedding, e1 is of
   * sygus datatype type, and c1...cn are constants. If ev( e1, c1...cn ) is the
   * purified form of ev( e, t1...tn ), then it is added to eval_pts[e]. We add
   * evaluation points to eval_pts only for those terms that are newly
   * generated by this call (and have not been returned by a previous call to
   * this method).
   */
  Node addRefLemma(Node lemma, std::map<Node, std::vector<Node>>& eval_pts);
  /**
 ...
 protected:
  ...
  /*
    ...
    --------------------------------------------------------------
  */
  /* Maps unif candidates to their evaluation points */
