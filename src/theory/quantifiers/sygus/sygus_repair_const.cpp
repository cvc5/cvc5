                                      ...
                                      std::vector<Node>& repair_cv)
{
  ...
  {
    Trace("sygus-repair-const") << "...all skeleton variables lead to undecidability." << std::endl;
    return false;
  }  
...
Node SygusRepairConst::getSkeleton( Node n, std::map< TypeNode, int >& free_var_count, std::vector< Node >& sk_vars, std::map< Node, Node >& sk_vars_to_subs )
{
...
Node SygusRepairConst::getFoQuery( const std::vector< Node >& candidates, const std::vector< Node >& candidate_skeletons, const std::vector< Node >& sk_vars )
{
  NodeManager * nm = NodeManager::currentNM();
