...
void SygusEvalUnfold::registerEvalTerm( Node n ) {
  ...
  Trace("sygus-eval-unfold") << "SygusEvalUnfold: register eval term : " << n << std::endl;
...
void SygusEvalUnfold::registerModelValue( Node a, Node v, std::vector< Node >& terms, std::vector< Node >& vals, std::vector< Node >& exps ) {
  std::map< Node, std::map< Node, bool > >::iterator its = d_subterms.find( a );
  if( its==d_subterms.end() ){
    return;
  }
  SygusExplain * sy_exp = d_tds->getExplain();
  Trace("sygus-eval-unfold") << "registerModelValue : " << a << ", has " << its->second.size() << " registered subterms." << std::endl;
  for( std::map< Node, bool >::iterator itss = its->second.begin(); itss != its->second.end(); ++itss ){
    ...
    if( it!=d_eval_args.end() && !it->second.empty() ){
      ...
      Assert( dt.isSygus() );
      Trace("sygus-eval-unfold") << "TermDbSygus::eager: Register model value : " << vn << " for " << n << std::endl;
      Trace("sygus-eval-unfold") << "...it has " << it->second.size() << " evaluations, already processed " << start << "." << std::endl;
      ...
      for( unsigned i=start; i<it->second.size(); i++ ){
        ...
        vals.push_back( res );
        exps.push_back( expn );
        Trace("sygus-eval-unfold") << "Conclude : " << d_evals[n][i] << " == " << res << ", given model value = " << d_eval_args_const[n][i] << std::endl;
