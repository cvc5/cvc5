                                     ...
                                     bool isVarAgnostic)
{
  ...
  if (isVarAgnostic)
  {
    ...
    for( std::pair<const unsigned, std::vector<Node> >& p : d_var_subclass_list[et] )
    {
      if( p.second.size()>1 )
      {
        useVarAgnostic = true;
    ...
    if( !useVarAgnostic )
    {
      Trace("sygus-db") << "...disabling variable agnostic for " << e << " since it has no subclass with more than one variable." << std::endl;
