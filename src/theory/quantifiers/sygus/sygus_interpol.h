...
#define CVC4__THEORY__QUANTIFIERS__SYGUS_INTERPOL_H
 ...
 class SygusInterpol
 {
  ...
  private:
	 /**
		* Collects symbols from axioms (axioms) and conjecture (conj), which stored in
		* d_syms. And computes the shared symbols between axioms and conjecture, stored in d_symsShared.
	  */
   void collectSymbols(const std::vector<Node>& axioms, const Node& conj);
 
	 /**
		* Creates free variables and shared free variables from d_syms and d_symsShared, which stored in
		* d_vars and d_varsShared. And also creates the same set of variables for formal argument list, which stored in
		* d_vlvs and d_vlvsShared. Extracts the types of shared variables, which stored in d_varTypesShared.
		* Creates the formal argument list of the interpol-to-synthesis, stored in d_abvlShared.
		*
		* The argument needsShared denotes if we want to restrict the argument list of the interpol-to-synthesis to be over shared variables or not.
		*/
    ...
    *
		* The user-defined grammar will be encoded by itpGType. In DEFAULT option, it will set up the grammar from itpGType. And if itpGType is null,
		* it will set up the default grammar. In ASSUMPTIONS option, it will set up the grammar by only using the operators from axioms.
		* In CONCLUSION option, it will set up the grammar by only using the operators from conj.
		* In SHARED option, it will set up the grammar by only using the operators shared by axioms and conj.
    * In ALL option, it will set up the grammar by only using the operators from axioms or conj.
		*/
...
*
   ...
   bool findInterpol(Expr& interpol, Node itp);
     ...
     *
     * This is a fresh copy of the SMT engine which is used for making calls especially for interpolation problem.
 		* In particular, consider the input:
     *   (assert A)
    *   (get-interpol s B)
