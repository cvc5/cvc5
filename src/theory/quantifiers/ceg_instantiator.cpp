...
void CegInstantiator::registerCounterexampleLemma( std::vector< Node >& lems, std::vector< Node >& ce_vars ) {
  ...
  Trace("cbqi-debug") << "Preprocess based on theory-specific methods..." << std::endl;
  ...
  Trace("cbqi-debug") << "Register variables from theory-specific methods " << d_num_input_variables << " " << d_vars.size() << " ..." << std::endl;
  for( unsigned i=d_num_input_variables; i<pvars.size(); i++ ){
    Trace("cbqi-debug") << "  register variable #" << i << " : " << pvars[i] << std::endl;
