...
class SubstitutionIndex
{
 public:
  ...
  // add substitution
  void addSubstitution(TNode eqc,
                       std::vector<TNode>& vars,
                       std::vector<TNode>& terms,
                       unsigned i = 0);
  // notify substitutions
  bool notifySubstitutions(ConjectureGenerator* s,
...
{
 private:
  unsigned calculateGeneralizationDepth(
      TermGenEnv* s, std::map<TypeNode, std::vector<int> >& fvs);
 ...
 public:
  ...
  bool getNextMatch(TermGenEnv* s,
                    TNode eqc,
...
class TermGenEnv
{
 public:
  ...
  // the equivalence classes (if applicable) that match the currently generated
  // term
...
{
 private:
  void addTheorem(std::vector<TNode>& lhs_v,
                  std::vector<unsigned>& lhs_arg,
                  TNode rhs);
  void addTheoremNode(TNode curr,
                      std::vector<TNode>& lhs_v,
                      std::vector<unsigned>& lhs_arg,
                      TNode rhs);
  void getEquivalentTerms(std::vector<TNode>& n_v,
                          std::vector<unsigned>& n_arg,
                          std::map<TNode, TNode>& smap,
                          std::vector<TNode>& vars,
                          std::vector<TNode>& subs,
                          std::vector<Node>& terms);
  void getEquivalentTermsNode(Node curr,
                              std::vector<TNode>& n_v,
                              std::vector<unsigned>& n_arg,
                              std::map<TNode, TNode>& smap,
                              std::vector<TNode>& vars,
                              std::vector<TNode>& subs,
...
class ConjectureGenerator : public QuantifiersModule
{
 ...
 private:  // information regarding the terms
  ...
  // collect functions
  unsigned collectFunctions(TNode opat,
                            TNode pat,
                            std::map<TNode, unsigned>& funcs,
                            std::map<TypeNode, unsigned>& mnvn,
                            std::map<TypeNode, unsigned>& mxvn);
  // add pattern
 ...
 public:  // for property enumeration
  ...
  void processCandidateConjecture(TNode lhs,
                                  TNode rhs,
                                  unsigned lhs_depth,
                                  unsigned rhs_depth);
 ...
 private:  // information about ground equivalence classes
  ...
  unsigned flushWaitingConjectures(unsigned& addedLemmas,
                                   int ldepth,
                                   int rdepth);
