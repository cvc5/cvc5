      ...
      d_ierCounterc(c)
{
  ...
  d_instWhenPhase = 1 + ( options::instWhenPhase()<1 ? 1 : options::instWhenPhase() );
}
...
void QuantifiersState::incrementInstRoundCounters(Theory::Effort e)
{
  if( e==Theory::EFFORT_FULL ){
    ...
    if( d_ierCounterLastLc!=d_ierCounterLc || !options::instWhenStrictInterleave() || d_ierCounter%d_instWhenPhase!=0 ){
...
bool QuantifiersState::getInstWhenNeedsCheck( Theory::Effort e ) {
  Trace("quant-engine-debug2") << "Get inst when needs check, counts=" << d_ierCounter << ", " << d_ierCounterLc << std::endl;
  ...
  {
    performCheck = ( ( e==Theory::EFFORT_FULL && d_ierCounter%d_instWhenPhase!=0 ) || e==Theory::EFFORT_LAST_CALL );
  }
