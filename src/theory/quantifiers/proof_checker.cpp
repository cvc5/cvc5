...
Node QuantifiersProofRuleChecker::checkInternal(PfRule id,
                                       ...
                                       const std::vector<Node>& args)
{
  ...
  if (id == PfRule::SKOLEMIZE)
  {
    ...
    echildren.insert(echildren.begin(), children[0][0].begin(), children[0][0].end());
  ...
  {
    Assert(children.size() == 1);
    if (children[0].getKind()!=FORALL || args.size()!=children[0][0].getNumChildren())
    {
      return Node::null();
    }
    ...
    Node inst = body.substitute(vars.begin(),vars.end(),subs.begin(),subs.end());
