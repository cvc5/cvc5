...
Node EqualityQueryInstProp::getRepresentativeExp( Node a, std::vector< Node >& exp ) {
  ...
  if( !ar.isNull() ){
    if( engine_has_a || getEngine()->hasTerm( ar ) ){
      ...
      CVC4_DCHECK(getEngine()->hasTerm(ar));
      CVC4_DCHECK(getEngine()->getRepresentative(ar) == ar);
      return ar;
    }
  }else{
    if( engine_has_a ){
...
Node EqualityQueryInstProp::getUfRepresentative( Node a, std::vector< Node >& exp ) {
  ...
  if( it!=d_uf.end() ){
    if( it->second==a ){
      CVC4_DCHECK(d_uf_exp[a].empty());
      return it->second;
    }else{
      Node m = getUfRepresentative( it->second, exp );
      CVC4_DCHECK(!m.isNull());
      if( m!=it->second ){
        //update union find
        d_uf[ a ] = m;
...
int EqualityQueryInstProp::setEqual( Node& a, Node& b, bool pol, std::vector< Node >& reason ) {
  ...
  CVC4_DCHECK(!getEngine()->hasTerm(ar) || getEngine()->hasTerm(br));
  CVC4_DCHECK(ar != br);
  ...
  if( areDisequalExp( ar, br, exp_d ) ){
    ...
    if( pol ){
      ...
      CVC4_DCHECK(d_uf_exp[ar].empty());
      CVC4_DCHECK(d_uf_exp[br].empty());
      ...
      CVC4_DCHECK(d_diseq_list[ar].find(br) == d_diseq_list[ar].end());
      CVC4_DCHECK(d_diseq_list[br].find(ar) == d_diseq_list[br].end());
        ...
        if( k!=FORALL ){
          ...
          if( !ret_set ){
            ...
            if( !f.isNull() && watch.empty() ){
              ...
              CVC4_DCHECK(args.size() == n.getNumChildren());
...
void EqualityQueryInstProp::merge_exp( std::vector< Node >& v, std::vector< Node >& v_to_merge, int up_to_size ) {
  //TODO : optimize
  if( v.empty() ){
    CVC4_DCHECK(up_to_size == -1 || up_to_size == (int)v_to_merge.size());
                                         ...
                                         Node body)
{
  if( !d_conflict ){
    ...
    if( cacheConclusion( id, body ) ){
      CVC4_DCHECK(d_update_list.empty());
  ...
  if( eval.isNull() ){
    ...
    if( eval==d_qy.d_false ){
      ...
      for( unsigned i=0; i<props.size(); i++ ){
        ...
        CVC4_DCHECK(d_qy.isPropagateLiteral(props[i]));
...
void InstPropagator::propagate( Node a, Node b, bool pol, std::vector< Node >& exp ) {
  ...
  if( pol ){
    if( status==EqualityQueryInstProp::STATUS_MERGED_KNOWN ){
      ...
      CVC4_DCHECK(d_qy.getEngine()->hasTerm(a));
      CVC4_DCHECK(d_qy.getEngine()->hasTerm(b));
...
bool InstPropagator::cacheConclusion( unsigned id, Node body, int prop_index ) {
  CVC4_DCHECK(prop_index == 0 || prop_index == 1);
...
void InstPropagator::addRelevantInstances( std::vector< Node >& exp, const char * c ) {
  for( unsigned i=0; i<exp.size(); i++ ){
    CVC4_DCHECK(d_conc_to_id[0].find(exp[i]) != d_conc_to_id[0].end());
