...
*
* This data structure stores a set expressions, indexed by representatives of their arguments.
*
...
* (1) Maintain a list of all ground terms that exist in the quantifier-free solvers,
*     as notified through the master equality engine.
* (2) Build TermArgTrie objects that index all ground terms, per operator. This is done
...
public:
  ...
  * Get the number of ground terms with operator f that have been added to the database
  */
  ...
  * withinInstClosure is whether n is within an inst-closure operator (see Bansal et al CAV 2015).
  */
  ...
  * (1) t is a term that is currently indexed by this database,
  * (2) t is of the form f( t1, ..., tk ) and ti is in the equivalence class of r. 
  */
  bool inRelevantDomain( TNode f, unsigned i, TNode r );
  /** evaluate term
  * Returns a term n' such that n = n' is entailed based on the equality information qy.
  * This function may generate new terms. In particular, we typically rewrite maximal
  * subterms of n to terms that exist in the equality engine specified by qy.
  * useEntailmentTests is whether to use the theory engine's entailmentCheck call,
  * for increased precision. This is not frequently used.
  */
  ...
  * Wrt evaluateTerm, this version does not construct new terms, is less aggressive 
  */
  TNode getEntailedTerm( TNode n, EqualityQuery * qy = NULL );
  /** get entailed term
  * If possible, returns a term n' such that:
  * (1) n' exists in the current equality engine (as specified by qy),
  * (2) n * subs = n' is entailed in the current context, where * is denotes substitution application.
  * It returns null if no such term can be found.
  * subsRep is whether the substitution maps to terms that are representatives according to qy.
  * Wrt evaluateTerm, this version does not construct new terms, is less aggressive 
  */
  TNode getEntailedTerm( TNode n, std::map< TNode, TNode >& subs, bool subsRep, EqualityQuery * qy = NULL );
  /** is entailed 
  * Checks whether the current context entails n with polarity pol, based on the equality information qy.
  * Returns true if the entailment can be successfully shown.
  */
  bool isEntailed( TNode n, bool pol, EqualityQuery * qy = NULL );
  /** is entailed  
  * Checks whether the current context entails ( n * subs ) with polarity pol, based on the equality information qy,
  ...
  * By default, all terms are active.
  * A term is inactive if:
  * (1) it is congruent to another term 
  * (2) it is irrelevant based on the term database mode. This includes terms that only appear in literals that are not relevant.
  * (3) it contains instantiation constants (used for CEGQI and cannot be used in instantiation).
  ...
  */
  ...
  /** has term current
  * This function is used in cases where we restrict which terms appear in the database.
  * It returns 
  */
