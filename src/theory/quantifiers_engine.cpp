      ...
      d_presolve_cache_wic(u)
{
  ...
  if (options::ceGuidedInst()) {
    d_sygus_tdb = std::unique_ptr<quantifiers::TermDbSygus>(
  ...
  if( options::instPropagate() ){
    ...
    d_inst_prop = std::unique_ptr<quantifiers::InstPropagator>(
  ...
  if( options::inferArithTriggerEq() ){
    d_eq_inference = std::unique_ptr<quantifiers::EqualityInference>(
  ...
  if( options::relevantTriggers() ){
    d_quant_rel = std::unique_ptr<quantifiers::QuantRelevance>(
  ...
  if( options::quantConflictFind() || options::quantRewriteRules() ){
    d_qcf = std::unique_ptr<quantifiers::QuantConflictFind>(
        new quantifiers::QuantConflictFind(this, c));
    d_modules.push_back(d_qcf.get());
  }
  if( options::conjectureGen() ){
    d_sg_gen = std::unique_ptr<quantifiers::ConjectureGenerator>(
        new quantifiers::ConjectureGenerator(this, c));
    d_modules.push_back(d_sg_gen.get());
  }
  if( !options::finiteModelFind() || options::fmfInstEngine() ){
    d_inst_engine = std::unique_ptr<quantifiers::InstantiationEngine>(
        new quantifiers::InstantiationEngine(this));
    d_modules.push_back(d_inst_engine.get());
  }
  if( options::cbqi() ){
    d_i_cbqi = std::unique_ptr<quantifiers::InstStrategyCegqi>(
        new quantifiers::InstStrategyCegqi(this));
    d_modules.push_back(d_i_cbqi.get());
    if( options::cbqiBv() ){
      // if doing instantiation for BV, need the inverter class
      d_bv_invert =
          std::unique_ptr<quantifiers::BvInverter>(new quantifiers::BvInverter);
    }
  }
  if( options::ceGuidedInst() ){
    d_ceg_inst = std::unique_ptr<quantifiers::CegInstantiation>(
  ...
  if( options::finiteModelFind() ){
    if( options::fmfBound() ){
      d_bint = std::unique_ptr<quantifiers::BoundedIntegers>(
          new quantifiers::BoundedIntegers(c, this));
      d_modules.push_back(d_bint.get());
    }
    d_model_engine = std::unique_ptr<quantifiers::ModelEngine>(
  ...
  if( options::quantRewriteRules() ){
    d_rr_engine = std::unique_ptr<quantifiers::RewriteEngine>(
        new quantifiers::RewriteEngine(c, this));
    d_modules.push_back(d_rr_engine.get());
  }
  if( options::ltePartialInst() ){
    d_lte_part_inst = std::unique_ptr<quantifiers::LtePartialInst>(
  ...
  if( needsRelDom ){
    d_rel_dom = std::unique_ptr<quantifiers::RelevantDomain>(
  ...
  if( needsBuilder ){
        ...
        || options::fmfBound())
    {
      ...
      d_model = std::unique_ptr<quantifiers::fmcheck::FirstOrderModelFmc>(
          new quantifiers::fmcheck::FirstOrderModelFmc(
              this, c, "FirstOrderModelFmc"));
      d_builder = std::unique_ptr<quantifiers::fmcheck::FullModelChecker>(
      ...
      d_model = std::unique_ptr<quantifiers::FirstOrderModelAbs>(
          new quantifiers::FirstOrderModelAbs(this, c, "FirstOrderModelAbs"));
      d_builder = std::unique_ptr<quantifiers::AbsMbqiBuilder>(
          new quantifiers::AbsMbqiBuilder(c, this));
    }else{
      Trace("quant-engine-debug") << "...make default model builder." << std::endl;
      d_model = std::unique_ptr<quantifiers::FirstOrderModelIG>(
          new quantifiers::FirstOrderModelIG(this, c, "FirstOrderModelIG"));
      d_builder = std::unique_ptr<quantifiers::QModelBuilderDefault>(
