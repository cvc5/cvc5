...
{
  clear();
  for (std::map<int, int>::iterator it = c.d_eqc.begin(); it != c.d_eqc.end();
       ++it)
  {
...
void SortInference::setEqual(int t1, int t2)
{
  if (t1 != t2)
  {
    ...
    if (rt1 != rt2)
    {
      ...
      /*
      d_type_eq_class[rt1].insert( d_type_eq_class[rt1].end(),
      d_type_eq_class[rt2].begin(), d_type_eq_class[rt2].end() );
      ...
      */
      ...
      if (it1 != d_type_types.end())
      {
        ...
        {
          Trace("sort-inference-debug")
              << "...fail : associated with types " << d_type_types[rt1]
              << " and " << d_type_types[rt2] << std::endl;
          return;
        }
      }
...
int SortInference::process(Node n,
                           std::map<Node, Node>& var_bound,
                           std::map<Node, int>& visited)
{
  ...
  else
  {
    ...
    for (size_t i = 0; i < n.getNumChildren(); i++)
    {
      ...
      if (processChild)
      {
        ...
        child_types.push_back(
            process(n[i], var_bound, use_new_visited ? new_visited : visited));
      }
    }
                                     ...
                                     false))
    {
      Trace("sort-inference-debug")
          << "For equality " << n
          << ", set equal types from : " << n[0].getType() << " "
          << n[1].getType() << std::endl;
    ...
    else if (isHandledApplyUf(n.getKind()))
    {
      ...
      for (size_t i = 0; i < n.getNumChildren(); i++)
      {
        // the argument of the operator must match the return type of the
        // subterm
        if (n[i].getType() != tn_op[i])
        {
          // if type mismatch, assume original types
          Trace("sort-inference-debug")
              << "Argument " << i << " of " << op << " " << n[i] << " has type "
              << n[i].getType();
          Trace("sort-inference-debug")
              << ", while operator arg has type " << tn_op[i] << std::endl;
    ...
    else
    {
      ...
      {
        Trace("sort-inference-debug")
            << n << " is a bound variable." << std::endl;
      ...
      {
        Trace("sort-inference-debug")
            << n << " is a interpreted symbol." << std::endl;
        // it is an interpreted term
        for (size_t i = 0; i < children.size(); i++)
        {
          Trace("sort-inference-debug") << children[i] << " forced to have "
                                        << children[i].getType() << std::endl;
...
void SortInference::processMonotonic(
    Node n,
    bool pol,
    bool hasPol,
    std::map<Node, Node>& var_bound,
    ...
    bool typeMode)
{
  ...
  if (visited[n].find(pindex) == visited[n].end())
  {
    ...
    Trace("sort-inference-debug") << "...Process monotonic " << pol << " "
                                  << hasPol << " " << n << std::endl;
    if (n.getKind() == Kind::FORALL)
    {
      // only consider variables universally if it is possible this quantified
      // formula is asserted positively
      if (!hasPol || pol)
      {
    ...
    std::map<Node, std::map<TypeNode, Node> >& visited)
{
  ...
  {
    NodeManager* nm = nodeManager();
    Trace("sort-inference-debug2")
        << "Simplify " << n << ", type context=" << tnn << std::endl;
    std::vector<Node> children;
    ...
    if (n.getKind() == Kind::FORALL || n.getKind() == Kind::EXISTS)
    {
      ...
      {
        TypeNode tn =
            getOrCreateTypeForId(d_var_types[n][n[0][i]], n[0][i].getType());
        Node v = getNewSymbol(n[0][i], tn);
        Trace("sort-inference-debug2")
            << "Map variable " << n[0][i] << " to " << v << std::endl;
    ...
    for (size_t i = 0; i < n.getNumChildren(); i++)
    {
      ...
      if (processChild)
      {
        ...
        Trace("sort-inference-debug2") << "Simplify " << i << " " << n[i]
                                       << " returned " << nc << std::endl;
    ...
    if (n.getKind() == Kind::FORALL || n.getKind() == Kind::EXISTS)
    {
      ...
      {
        Trace("sort-inference-debug2")
            << "Remove bound for " << n[0][i] << std::endl;
        var_bound.erase(n[0][i]);
      }
    ...
    else if (n.getKind() == Kind::EQUAL)
    {
      ...
      if (tn1 != tn2)
      {
        Trace("sort-inference-warn")
            << "Sort inference created bad equality: " << children[0] << " = "
            << children[1] << std::endl;
        Trace("sort-inference-warn")
            << "  Types : " << children[0].getType() << " "
            << children[1].getType() << std::endl;
        DebugUnhandled();
      }
    ...
    else if (isHandledApplyUf(n.getKind()))
    {
      if (d_symbol_map.find(op) == d_symbol_map.end())
      {
        ...
        {
          TypeNode tn =
              getOrCreateTypeForId(d_op_arg_types[op][i], n[i].getType());
        ...
        TypeNode retType =
            getOrCreateTypeForId(d_op_return_types[op], n.getType());
        ...
        if (opChanged)
        {
          ...
          Trace("setp-model") << "Function " << op << " is replaced with "
                              << d_symbol_map[op] << std::endl;
      ...
      for (size_t i = 0, size = n.getNumChildren(); i < size; i++)
      {
        ...
        {
          Trace("sort-inference-warn")
              << "Sort inference created bad child: " << n << " " << n[i] << " "
              << tn << " " << tna << std::endl;
          DebugUnhandled();
        }
      }
...
Node SortInference::mkInjection(TypeNode tn1, TypeNode tn2)
{
  ...
  Trace("sort-inference") << "-> Make injection " << f << " from " << tn1
                          << " to " << tn2 << std::endl;
...
{
  Trace("sort-inference-temp")
      << "Set skolem var for " << f << ", variable " << v << std::endl;
  ...
  Trace("sort-inference-temp") << "Set skolem sort id for " << sk << " to "
                               << d_op_return_types[sk] << std::endl;
}
  ...
  else
  {
    if (isHandledApplyUf(n.getKind()))
    {
      for (unsigned i = 0; i < n.getNumChildren(); i++)
      {
        ...
        int s2 = d_type_union_find.getRepresentative(
            d_op_arg_types[n.getOperator()][i]);
        if (s1 != s2)
        {
          return false;
        }
