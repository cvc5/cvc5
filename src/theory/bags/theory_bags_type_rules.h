...
/**
 * Table join operator is indexed by a list of indices (m_1, m_k, n_1, ..., n_k).
 * It ensures that it has 2 arguments:
 * - A table of type (Table X_1 ... X_i)
 * - A table of type (Table Y_1 ... Y_j)
 * such that indices has constraints 0 <= m_1, ..., mk, n_1, ..., n_k <= min(i,j)
 * and types has constraints X_{m_1} = Y_{n_1}, ..., X_{m_k} = Y_{n_k}.
