        ...
        if (!options::finiteModelFind())
        {
          while ((i = TheoryIdSetUtil::setPop(theories)) != THEORY_LAST)
          {
            if (!d_logicInfo.isTheoryEnabled(i))
            {
              ...
              newLogicInfo.lock();
              std::stringstream ss;
              ss << "The logic was specified as "
                 << d_logicInfo.getLogicString() << ", which doesn't include "
                 << i << ", but found a term in that theory." << std::endl
  ...
  try
  {
    ...
    while (d_factsAsserted && !d_inConflict && !d_lemmasAdded)
    {
      ...
      if (Theory::fullEffort(effort) && d_logicInfo.isSharingEnabled()
          && !d_factsAsserted && !d_lemmasAdded && !d_inConflict)
      {
        // Do the combination
        Debug("theory") << "TheoryEngine::check(" << effort
...
void TheoryEngine::printInstantiations(std::ostream& out)
{
  ...
  {
    out << "Internal error : instantiations not available when quantifiers are "
           "not present."
        << std::endl;
    Assert(false);
  }
...
void TheoryEngine::printSynthSolution(std::ostream& out)
{
  ...
  {
    out << "Internal error : synth solution not available when quantifiers are "
           "not present."
        << std::endl;
    Assert(false);
  }
...
void TheoryEngine::getInstantiationTermVectors(
                                        ...
                                        theory::TheoryId from)
{
  ...
  Debug("te-lemma") << "retLemma: " << retLemma << std::endl;
