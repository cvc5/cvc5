 ...
 * where t.getType()=atn.
 *
 * bvl should be a bound variable list whose variables correspond to in-order to
 * the index types of the
 * (curried) Array type. For example a bound variable list bvl whose variables
 * have types (Int, Real) would be a proper
 * input when paired with atn = (Array Int (Array Real Bool)), or (Array Int
 * (Array Real (Array Bool Bool))).
 * This function returns (-> Int Real Bool) and (-> Int Real (Array Bool Bool))
 * respectively in these cases.
 * Whereas bvl is not a proper input for atn = (Array Int (Array Bool Bool)) or
 * (Array Int Int).
 */
 static TypeNode getFunctionTypeForArrayType(TypeNode atn, Node bvl);
 /** Get array type for function type
 * This returns the array type of terms returned
 * getArrayRepresentationForLambda( t ),
 * where t.getType()=ftn.
 */
