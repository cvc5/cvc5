...
class BuiltinProofRuleChecker : public ProofRuleChecker
{
 public:
   ...
   * substitutions if exp is of kind AND and ids is SB_DEFAULT (the other
   * substitution types interpret AND as a formula). The vector from are the
   * literals from exp that each substitution in vars/subs are based on. For
   * example, if exp is (and (= x t) (= y s)), then vars = { x, y },
   * subs = { t, s }, from = { (= x y), (= y s) }.
   */
