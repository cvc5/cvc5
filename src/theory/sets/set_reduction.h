...
class SetReduction
{
 public:
  ...
  /**
   * @param node a term of the form (set.min r A i) where
   *        r: (-> T T Bool) is a binary operation
   *        A: (Set T) is a set
   *        i: T is the initial value
   * @param asserts a list of assertions generated by this reduction
   * @return the reduction term min with asserts:
   * - (or
   ...
   * where
   * min : T is a fresh value
   */
  static Node reduceMinOperator(Node node, std::vector<Node>& asserts);

  /**
   * @param node a term of the form (set.max r A i) where
   *        r: (-> T T Bool) is a binary operation
   *        A: (Set T) is a set
   *        i: T is the initial value
   * @param asserts a list of assertions generated by this reduction
   * @return the reduction term max with asserts:
   * - (or
   ...
   * where
   * max : T is a fresh value
   */
  static Node reduceMaxOperator(Node node, std::vector<Node>& asserts);
  /**
   * @param node a term of the form ((_ rel.min index) r A initial) where
   *        r: (-> T T Bool) is a binary operation
   *        A: (Set E) where E is (Tuple ... T ...)
   *        i: T is the initial value
   * @param asserts a list of assertions generated by this reduction
   * @return the reduction term min with asserts:
   * - (or
   *      (and (set.is_empty A) (= min i))
   *      (and (not (set.is_empty A))
   *           (set.some (lambda ((t E)) (= min ((_ tuple.select index) t)) A)
   *           (set.all  (lambda ((x E)) (r min ((_ tuple.select index) t)) A)))
   * where
   * min : T is a fresh value
   */
  static Node reduceRelationMinOperator(Node node, std::vector<Node>& asserts);
  /**
   * @param node a term of the form ((_ rel.max index) r A initial) where
   *        r: (-> T T Bool) is a binary operation
   *        A: (Set E) where E is (Tuple ... T ...)
   *        i: T is the initial value
   * @param asserts a list of assertions generated by this reduction
   ...
   *      (and (set.is_empty A) (= max i))
   *      (and (not (set.is_empty A))
   *           (set.some (lambda ((t E)) (= max ((_ tuple.select index) t)) A)
   *           (set.all  (lambda ((x E)) (r max ((_ tuple.select index) t)) A)))
   * where
   * max : T is a fresh value
   */
  static Node reduceRelationMaxOperator(Node node, std::vector<Node>& asserts);
