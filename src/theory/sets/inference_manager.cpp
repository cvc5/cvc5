...
bool InferenceManager::assertFactRec( Node fact, Node exp, std::vector< Node >& lemma, int inferType ) {
  ...
  else if( fact.getKind()==AND || ( fact.getKind()==NOT && fact[0].getKind()==OR ) ){
    bool ret = false;
  ...
  if( atom.getKind()==MEMBER || ( atom.getKind()==EQUAL && atom[0].getType().isSet() ) ){
...
void InferenceManager::assertInference( Node fact, Node exp, std::vector< Node >& lemmas, const char * c, int inferType ) {
  d_keep.insert( exp );
  d_keep.insert( fact );
...
void InferenceManager::assertInference( Node fact, std::vector< Node >& exp, std::vector< Node >& lemmas, const char * c, int inferType ){
...
void InferenceManager::assertInference( std::vector< Node >& conc, Node exp, std::vector< Node >& lemmas, const char * c, int inferType ){
  if( !conc.empty() ){
    Node fact = conc.size()==1 ? conc[0] : NodeManager::currentNM()->mkNode( AND, conc );
    assertInference( fact, exp, lemmas, c, inferType );
  }
}
void InferenceManager::assertInference( std::vector< Node >& conc, std::vector< Node >& exp, std::vector< Node >& lemmas, const char * c, int inferType ) {
  Node exp_n = exp.empty() ? d_true : ( exp.size()==1 ? exp[0] : NodeManager::currentNM()->mkNode( AND, exp ) );
