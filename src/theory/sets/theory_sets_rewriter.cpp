  ...
  case kind::RELATION_TRANSPOSE:
  {
    ...
    else if (node[0].isConst())
    {
      ...
      while(tuple_it != tuple_set.end()) {
        new_tuple_set.insert(DatatypesUtils::reverseTuple(*tuple_it));
        ++tuple_it;
      }
    ...
    else if (node[0].isConst() && node[1].isConst())
    {
      ...
      while(left_it != left.end()) {
        ...
        for(int i = 0; i < left_len; i++) {
          left_tuple.push_back(DatatypesUtils::nthElementOfTuple(*left_it,i));
        }
        ...
        while(right_it != right.end()) {
          ...
          for(int j = 0; j < right_len; j++) {
            right_tuple.push_back(DatatypesUtils::nthElementOfTuple(*right_it,j));
          }
  ...
  case kind::RELATION_JOIN:
  {
    ...
    else if (node[0].isConst() && node[1].isConst())
    {
      ...
      while(left_it != left.end()) {
        ...
        int right_len = (*right_it).getType().getTupleLength();
        while(right_it != right.end()) {
          if(DatatypesUtils::nthElementOfTuple(*left_it,left_len-1) == DatatypesUtils::nthElementOfTuple(*right_it,0)) {
            std::vector<Node> right_tuple;
  ...
  case kind::RELATION_IDEN:
  {
    ...
    else if (node[0].isConst())
    {
      ...
      while( rel_mems_it != rel_mems.end() ) {
        Node fst_mem = DatatypesUtils::nthElementOfTuple( *rel_mems_it, 0);
    ...
    else if (node[0].isConst())
    {
      ...
      while( rel_mems_it != rel_mems.end() ) {
        ...
        while( rel_mems_it_snd != rel_mems.end() ) {
          Node fst_mem_snd = DatatypesUtils::nthElementOfTuple( *rel_mems_it_snd, 0);
          if( fst_mem == fst_mem_snd ) {
            existing_mems.insert( DatatypesUtils::nthElementOfTuple( *rel_mems_it_snd, 1) );
          }
          ++rel_mems_it_snd;
        }
