          ...
          for (const std::pair<const Node, Node>& it2 : smem)
          {
            ...
            if (mem[1] != eq_set)
            {
              ...
              if (!options::setsProxyLemmas())
              {
                ...
                d_im.assertInference(nmem, InferenceId::UNKNOWN, exp, "downc");
                if (d_state.isInConflict())
                {
                  return;
                }
              ...
              for (const std::pair<const Node, Node>& itm1m : r1mem)
              {
                ...
                if (valid)
                {
                  ...
                  if (!d_state.isMember(x, rr))
                  {
                    ...
                    d_im.assertInference(fact, InferenceId::SETS_MEMBER, exp, "upc", inferType);
                    if (d_state.isInConflict())
                    {
                      return;
                    }
              ...
              if (!r2mem.empty())
              {
                ...
                for (const std::pair<const Node, Node>& itm2m : r2mem)
                {
                  ...
                  if (!d_state.isMember(x, rr))
                  {
                    ...
                    d_im.assertInference(fact, InferenceId::UNKNOWN, exp, "upc2");
                    if (d_state.isInConflict())
                    {
                      return;
                    }
        ...
        if (!v.isNull())
        {
          ...
          for (const std::pair<const Node, Node>& it2 : smems)
          {
            ...
            if (!u.isNull())
            {
              ...
              d_im.assertInference(fact, InferenceId::UNKNOWN, exp, "upuniv");
              if (d_state.isInConflict())
              {
                return;
              }
...
void TheorySetsPrivate::checkDisequalities()
{
  ...
  for (NodeBoolMap::const_iterator it = d_deq.begin(); it != d_deq.end(); ++it)
  {
    ...
    d_im.assertInference(lem, InferenceId::UNKNOWN, d_true, "diseq", 1);
    d_im.doPendingLemmas();
    if (d_im.hasSent())
    {
      return;
