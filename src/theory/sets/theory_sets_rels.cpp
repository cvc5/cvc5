  ...
  */
  ...
  void TheorySetsRels::computeMembersForJoinImageTerm( Node join_image_term ) {
    ...
    while( mem_rep_it != (*rel_mem_it).second.end() ) {
      ...
      Node reasons = d_trueNode;
      std::vector< Node > existing_members;
      std::vector< Node >::iterator mem_rep_it_snd = (*rel_mem_it).second.begin();
      std::vector< Node >::iterator mem_rep_exp_it_snd = (*rel_mem_exp_it).second.begin();

      while( mem_rep_it_snd != (*rel_mem_it).second.end() ) {
        Node fst_element_snd_mem = RelsUtils::nthElementOfTuple( *mem_rep_it_snd, 0 );

        if( areEqual( fst_mem_rep,  fst_element_snd_mem ) ) {
          bool notExist = true;
          std::vector< Node >::iterator existing_mem_it = existing_members.begin();
          Node snd_element_snd_mem = RelsUtils::nthElementOfTuple( *mem_rep_it_snd, 1 );
          ...
          if( notExist ) {
            ...
            reasons = NodeManager::currentNM()->mkNode( kind::AND, reasons, *mem_rep_exp_it_snd );

            if( join_image_rel != (*mem_rep_exp_it_snd)[1] ) {
              reasons = NodeManager::currentNM()->mkNode( kind::AND, reasons, NodeManager::currentNM()->mkNode( kind::EQUAL, (*mem_rep_exp_it_snd)[1], join_image_rel ));
            }
            Node fst_exp_mem = RelsUtils::nthElementOfTuple( (*mem_rep_exp_it_snd)[0], 0);
            if( fst_mem_rep !=  fst_exp_mem ) {
              reasons = NodeManager::currentNM()->mkNode( kind::AND, reasons, NodeManager::currentNM()->mkNode( kind::EQUAL, fst_mem_rep, fst_exp_mem ));
            }
            if( count == min_card ) {
              if( min_card >= 2) {
              ...
              sendInfer( new_membership, reasons, "JOIN-IMAGE UP" );
              break;
            }
          }
        }
