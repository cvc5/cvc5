...
void TheorySetsRels::check(Theory::Effort level)
{
    ...
    while( !eqcs_i.isFinished() ){
      ...
      while( !eqc_i.isFinished() ){
        ...
        else if (erType.isTuple() && !eqc_node.isConst() && !eqc_node.isVar())
        {
          ...
          {
            Node element = DatatypesUtils::nthElementOfTuple(eqc_node, i);
            if (!element.isConst())
            {
  ...
  void TheorySetsRels::computeMembersForJoinImageTerm( Node join_image_term ) {
    ...
    while( mem_rep_it != (*rel_mem_it).second.end() ) {
      Node fst_mem_rep = DatatypesUtils::nthElementOfTuple( *mem_rep_it, 0 );
      ...
      while( mem_rep_exp_it_snd != (*rel_mem_exp_it).second.end() ) {
        Node fst_element_snd_mem = DatatypesUtils::nthElementOfTuple( (*mem_rep_exp_it_snd)[0], 0 );

        if( areEqual( fst_mem_rep,  fst_element_snd_mem ) ) {
          bool notExist = true;
          std::vector< Node >::iterator existing_mem_it = existing_members.begin();
          Node snd_element_snd_mem = DatatypesUtils::nthElementOfTuple( (*mem_rep_exp_it_snd)[0], 1 );
  ...
  void TheorySetsRels::applyJoinImageRule( Node mem_rep, Node join_image_term, Node exp ) {
    ...
    Node fst_mem_element = DatatypesUtils::nthElementOfTuple( exp[0], 0 );
  ...
  void TheorySetsRels::applyIdenRule( Node mem_rep, Node iden_term, Node exp) {
    ...
    Node reason = exp;
    Node fst_mem = DatatypesUtils::nthElementOfTuple( exp[0], 0 );
    Node snd_mem = DatatypesUtils::nthElementOfTuple( exp[0], 1 );
  ...
  void TheorySetsRels::computeMembersForIdenTerm( Node iden_term ) {
    ...
    while( mem_rep_exp_it != (*rel_mem_exp_it).second.end() ) {
      ...
      Node fst_exp_mem = DatatypesUtils::nthElementOfTuple( (*mem_rep_exp_it)[0], 0 );
  ...
  void TheorySetsRels::applyTCRule( Node mem_rep, Node tc_rel, Node tc_rel_rep, Node exp ) {
    ...
    Node mem_rep_fst = getRepresentative( DatatypesUtils::nthElementOfTuple( mem_rep, 0 ) );
    Node mem_rep_snd = getRepresentative( DatatypesUtils::nthElementOfTuple( mem_rep, 1 ) );
    ...
    Node fst_element = DatatypesUtils::nthElementOfTuple( exp[0], 0 );
    Node snd_element = DatatypesUtils::nthElementOfTuple( exp[0], 1 );
  ...
  bool TheorySetsRels::isTCReachable( Node mem_rep, Node tc_rel ) {
    ...
    if( tc_it != d_rRep_tcGraph.end() ) {
      ...
      isTCReachable( getRepresentative( DatatypesUtils::nthElementOfTuple(mem_rep, 0) ),
                     getRepresentative( DatatypesUtils::nthElementOfTuple(mem_rep, 1) ), seen, tc_it->second, isReachable );
      return isReachable;
    }
    return false;
  }
  ...
  void TheorySetsRels::buildTCGraphForRel( Node tc_rel ) {
    ...
    {
      Node fst_element_rep = getRepresentative( DatatypesUtils::nthElementOfTuple( members[i], 0 ));
      Node snd_element_rep = getRepresentative( DatatypesUtils::nthElementOfTuple( members[i], 1 ));
  ...
  {
    NodeManager* nm = NodeManager::currentNM();
    Node tc_mem = RelsUtils::constructPair( tc_rel, DatatypesUtils::nthElementOfTuple((reasons.front())[0], 0), DatatypesUtils::nthElementOfTuple((reasons.back())[0], 1) );
    std::vector< Node > all_reasons( reasons );

    for( unsigned int i = 0 ; i < reasons.size()-1; i++ ) {
      Node fst_element_end = DatatypesUtils::nthElementOfTuple( reasons[i][0], 1 );
      Node snd_element_begin = DatatypesUtils::nthElementOfTuple( reasons[i+1][0], 0 );
  ...
  void TheorySetsRels::applyProductRule( Node pt_rel, Node pt_rel_rep, Node exp ) {
    ...
    unsigned int i = 0;
    for(; i < s1_len; ++i) {
      r1_element.push_back(DatatypesUtils::nthElementOfTuple(mem, i));
    }
    const DType& dt2 = pt_rel[1].getType().getSetElementType().getDType();
    r2_element.push_back(dt2[0].getConstructor());
    for(; i < tup_len; ++i) {
      r2_element.push_back(DatatypesUtils::nthElementOfTuple(mem, i));
    }
    Node reason   = exp;
  ...
  void TheorySetsRels::applyTransposeRule( Node tp_rel, Node tp_rel_rep, Node exp ) {
    ...
    Node reason = exp;
    Node reversed_mem = DatatypesUtils::reverseTuple( exp[0] );
  ...
  void TheorySetsRels::computeMembersForUnaryOpRel(Node rel) {
    ...
    if (rel.getKind() == kind::RELATION_TRANSPOSE)
    {
      for (size_t i = 0, msize = members.size(); i < msize; i++)
      {
        ...
        sendInfer(
            nm->mkNode(SET_MEMBER, DatatypesUtils::reverseTuple(exps[i][0]), rel),
            InferenceId::SETS_RELS_TRANSPOSE_REV,
            reason);
      }
    }
  ...
  void TheorySetsRels::composeMembersForRels( Node rel ) {
    ...
    for( unsigned int i = 0; i < r1_rep_exps.size(); i++ ) {
      for( unsigned int j = 0; j < r2_rep_exps.size(); j++ ) {
        ...
        {
          Node r1_rmost =
              DatatypesUtils::nthElementOfTuple(r1_rep_exps[i][0], r1_tuple_len - 1);
          Node r2_lmost = DatatypesUtils::nthElementOfTuple(r2_rep_exps[j][0], 0);
        ...
        if (rk == kind::RELATION_PRODUCT || rk == kind::RELATION_JOIN)
        {
          ...
          for( ; k < r1_tuple_len - 1; ++k ) {
            tuple_elements.push_back( DatatypesUtils::nthElementOfTuple( r1_rep_exps[i][0], k ) );
          }
          if(isProduct) {
            tuple_elements.push_back( DatatypesUtils::nthElementOfTuple( r1_rep_exps[i][0], k ) );
            tuple_elements.push_back( DatatypesUtils::nthElementOfTuple( r2_rep_exps[j][0], 0 ) );
          }
          for( ; l < r2_tuple_len; ++l ) {
            tuple_elements.push_back( DatatypesUtils::nthElementOfTuple( r2_rep_exps[j][0], l ) );
          }
  ...
  bool TheorySetsRels::areEqual( Node a, Node b ){
    ...
    if (atn.isTuple())
    {
      ...
      {
        if (!areEqual(DatatypesUtils::nthElementOfTuple(a, i),
                      DatatypesUtils::nthElementOfTuple(b, i)))
        {
          return false;
        }
      }
  ...
  void TheorySetsRels::computeTupleReps( Node n ) {
    if( d_tuple_reps.find( n ) == d_tuple_reps.end() ){
      for( unsigned i = 0; i < n.getType().getTupleLength(); i++ ){
        d_tuple_reps[n].push_back( getRepresentative( DatatypesUtils::nthElementOfTuple(n, i) ) );
      }
    }
  }
  ...
  void TheorySetsRels::reduceTupleVar(Node n) {
    if(d_symbolic_tuples.find(n) == d_symbolic_tuples.end()) {
      ...
      {
        Node element = DatatypesUtils::nthElementOfTuple(n[0], i);
        makeSharedTerm(element, tupleTypes[i]);
