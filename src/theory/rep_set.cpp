...
Node RepSet::getRepresentative(TypeNode tn, unsigned i) const
{
  std::map<TypeNode, std::vector<Node> >::const_iterator it =
      d_type_reps.find(tn);
  CVC4_DCHECK(it != d_type_reps.end());
...
void RepSet::add( TypeNode tn, Node n ){
  ...
  CVC4_DCHECK(n.getType().isSubtypeOf(tn));
...
bool RepSetIterator::initialize()
{
  ...
  if (d_rext)
  {
    ...
    if (d_rext->getVariableOrder(d_owner, varOrder))
    {
      ...
      for (unsigned i = 0; i < varOrder.size(); i++)
      {
        CVC4_DCHECK(varOrder[i] < indexOrder.size());
...
Node RepSetIterator::getCurrentTerm(unsigned v, bool valTerm) const
{
  ...
  CVC4_DCHECK(0 <= curr && curr < d_domain_elements[v].size());
