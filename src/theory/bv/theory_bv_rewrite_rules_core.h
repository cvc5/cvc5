...
template <>
inline bool RewriteRule<ConcatFlatten>::applies(TNode node)
{
  return (node.getKind() == Kind::BITVECTOR_CONCAT);
}

template <>
inline Node RewriteRule<ConcatFlatten>::apply(TNode node)
{
...
template <>
inline bool RewriteRule<ConcatExtractMerge>::applies(TNode node)
{
  return (node.getKind() == Kind::BITVECTOR_CONCAT);
}

template <>
inline Node RewriteRule<ConcatExtractMerge>::apply(TNode node)
{
  ...
  for (size_t i = 1, end = node.getNumChildren(); i < end; ++i)
  {
    ...
    // If we haven't merged anything, add the previous merge and continue with
    // the next
    if (!merged)
    {
      if (!mergeStarted)
        ...
        mergedExtracts.push_back(
            utils::mkExtract(current[0], currentHigh, currentLow));
      current = next;
      mergeStarted = false;
    }
  }
...
template <>
inline bool RewriteRule<ConcatConstantMerge>::applies(TNode node)
{
  return node.getKind() == Kind::BITVECTOR_CONCAT;
}

template <>
inline Node RewriteRule<ConcatConstantMerge>::apply(TNode node)
{
...
template <>
inline bool RewriteRule<ExtractWhole>::applies(TNode node)
{
...
template <>
inline Node RewriteRule<ExtractWhole>::apply(TNode node)
{
...
template <>
inline bool RewriteRule<ExtractConstant>::applies(TNode node)
{
  if (node.getKind() != Kind::BITVECTOR_EXTRACT) return false;
...
template <>
inline Node RewriteRule<ExtractConstant>::apply(TNode node)
{
...
template <>
inline bool RewriteRule<ExtractConcat>::applies(TNode node)
{
...
template <>
inline Node RewriteRule<ExtractConcat>::apply(TNode node)
{
  Trace("bv-rewrite") << "RewriteRule<ExtractConcat>(" << node << ")"
                      << std::endl;
  ...
  for (int i = concat.getNumChildren() - 1; i >= 0 && extract_high >= 0; i--)
  {
    ...
    if (extract_low < concatChildSize)
    {
      ...
      int extract_end =
          extract_high < concatChildSize ? extract_high : concatChildSize - 1;
      resultChildren.push_back(
          utils::mkExtract(concatChild, extract_end, extract_start));
    }
...
template <>
inline bool RewriteRule<ExtractExtract>::applies(TNode node)
{
  if (node.getKind() != Kind::BITVECTOR_EXTRACT) return false;
...
template <>
inline Node RewriteRule<ExtractExtract>::apply(TNode node)
{
...
template <>
inline bool RewriteRule<FailEq>::applies(TNode node)
{
...
template <>
inline Node RewriteRule<FailEq>::apply(TNode node)
{
...
template <>
inline bool RewriteRule<SimplifyEq>::applies(TNode node)
{
  if (node.getKind() != Kind::EQUAL) return false;
  return node[0] == node[1];
}
...
template <>
inline Node RewriteRule<SimplifyEq>::apply(TNode node)
{
...
template <>
inline bool RewriteRule<ReflexivityEq>::applies(TNode node)
{
  return (node.getKind() == Kind::EQUAL && node[0] > node[1]);
}

template <>
inline Node RewriteRule<ReflexivityEq>::apply(TNode node)
{
