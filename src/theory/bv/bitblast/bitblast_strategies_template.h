...
T DefaultUgtBB(TNode node, TBitblaster<T>* bb){
  Trace("bitvector-bb") << "Bitblasting node " << node  << "\n";
  Assert(node.getKind() == Kind::BITVECTOR_UGT);
  // should be rewritten
  Unimplemented();
}
...
T DefaultUgeBB(TNode node, TBitblaster<T>* bb){
  Trace("bitvector-bb") << "Bitblasting node " << node  << "\n";
  Assert(node.getKind() == Kind::BITVECTOR_UGE);
  // should be rewritten
  Unimplemented();
}
...
T DefaultSleBB(TNode node, TBitblaster<T>* bb){
  Trace("bitvector-bb") << "Bitblasting node " << node  << "\n";
  Assert(node.getKind() == Kind::BITVECTOR_SLE);
  std::vector<T> a, b;
  bb->bbTerm(node[0], a);
  bb->bbTerm(node[1], b);
  Assert(a.size() == b.size());
...
T DefaultSgtBB(TNode node, TBitblaster<T>* bb){
  Trace("bitvector-bb") << "Bitblasting node " << node  << "\n";
  Assert(node.getKind() == Kind::BITVECTOR_SGT);
  // should be rewritten
  Unimplemented();
}
...
T DefaultSgeBB(TNode node, TBitblaster<T>* bb){
  Trace("bitvector-bb") << "Bitblasting node " << node  << "\n";
  Assert(node.getKind() == Kind::BITVECTOR_SGE);
  // should be rewritten
  Unimplemented();
}
...
void DefaultXnorBB (TNode node, std::vector<T>& bits, TBitblaster<T>* bb) {
  ...
  Assert(bits.empty());
  Assert(node.getKind() == Kind::BITVECTOR_XNOR);
...
void DefaultNandBB (TNode node, std::vector<T>& bits, TBitblaster<T>* bb) {
  Trace("bitvector") << "theory::bv:: Unimplemented kind "
                     << node.getKind() << "\n";
  Assert(node.getKind() == Kind::BITVECTOR_NAND);
...
void DefaultNorBB (TNode node, std::vector<T>& bits, TBitblaster<T>* bb) {
  Trace("bitvector") << "theory::bv:: Unimplemented kind "
                     << node.getKind() << "\n";
  Assert(node.getKind() == Kind::BITVECTOR_NOR);
...
void DefaultCompBB (TNode node, std::vector<T>& bits, TBitblaster<T>* bb) {
  ...
  Assert(bits.empty());
  Assert(node.getKind() == Kind::BITVECTOR_COMP);
  Assert(node.getNumChildren() == 2);
  Assert(utils::getSize(node) == 1);
...
template <class T>
void DefaultMultBB (TNode node, std::vector<T>& bits, TBitblaster<T>* bb) {
  ...
  for(unsigned i = 1; i < node.getNumChildren(); ++i) {
    ...
    // in res
    shiftAddMultiplier(nm, bits, current, newres);
    bits = newres;
  }
  if(TraceIsOn("bitvector-bb")) {
    Trace("bitvector-bb") << "with bits: " << toString(bits)  << "\n";
...
template <class T>
void DefaultAddBB(TNode node, std::vector<T>& bits, TBitblaster<T>* bb) {
  ...
  for(unsigned i = 1; i < node.getNumChildren(); ++i) {
    ...
    bb->bbTerm(node[i], current);
    new_bits.clear();
    rippleCarryAdder(bits, current, new_bits, mkFalse<T>(nm));
    bits = new_bits;
  }
...
template <class T>
void DefaultUdivBB(TNode node, std::vector<T>& bits, TBitblaster<T>* bb)
{
  Trace("bitvector-bb") << "theory::bv::DefaultUdivBB bitblasting " << node
                        << "\n";
  Assert(bits.empty());
  ...
  std::vector<T> r;
  UdivUremBB(node, bits, r, bb);
}

template <class T>
void DefaultUremBB(TNode node, std::vector<T>& bits, TBitblaster<T>* bb)
{
...
void DefaultSdivBB (TNode node, std::vector<T>& bits, TBitblaster<T>* bb) {
  Trace("bitvector") << "theory::bv:: Unimplemented kind "
                     << node.getKind() << "\n";
  Assert(bits.empty());
  Assert(node.getKind() == Kind::BITVECTOR_SDIV);
  Unimplemented();
}
...
void DefaultSremBB (TNode node, std::vector<T>& bits, TBitblaster<T>* bb) {
  Trace("bitvector") << "theory::bv:: Unimplemented kind "
                     << node.getKind() << "\n";
  Assert(bits.empty());
  Assert(node.getKind() == Kind::BITVECTOR_SREM);
  Unimplemented();
}
...
void DefaultSmodBB (TNode node, std::vector<T>& bits, TBitblaster<T>* bb) {
  Trace("bitvector") << "theory::bv:: Unimplemented kind "
                     << node.getKind() << "\n";
  Assert(bits.empty());
  Assert(node.getKind() == Kind::BITVECTOR_SMOD);
  Unimplemented();
}

template <class T>
void DefaultShlBB(TNode node, std::vector<T>& bits, TBitblaster<T>* bb)
{
  ...
  for (unsigned s = 0; s < log2_size; ++s)
  {
    ...
    for (unsigned i = 0; i < a.size(); ++i)
    {
      if (i < threshold)
      {
        ...
        // value
        bits[i] = mkIte(b[s], mkFalse<T>(nm), prev_res[i]);
      }
      else
      {
        // if b[s]= 0, use previous value, otherwise shift by threshold  bits
        Assert(i >= threshold);
        bits[i] = mkIte(b[s], prev_res[i - threshold], prev_res[i]);
      }
    }
  }
  prev_res = bits;
  for (unsigned i = 0; i < b.size(); ++i)
  {
    ...
    bits[i] = mkIte(b_ult_a_size, prev_res[i], mkFalse<T>(nm));
  }
...
template <class T>
void DefaultLshrBB(TNode node, std::vector<T>& bits, TBitblaster<T>* bb)
{
  ...
  for (unsigned s = 0; s < log2_size; ++s)
  {
    ...
    for (unsigned i = 0; i < a.size(); ++i)
    {
      ...
      else
      {
        ...
        bits[i] = mkIte(mkNot(b[s]), prev_res[i], prev_res[i + threshold]);
      }
    }
  }
...
template <class T>
void DefaultAshrBB(TNode node, std::vector<T>& bits, TBitblaster<T>* bb)
{
  ...
  for (unsigned s = 0; s < log2_size; ++s)
  {
    ...
    for (unsigned i = 0; i < a.size(); ++i)
    {
      if (i + threshold >= a.size())
      {
        ...
        // shift value
        bits[i] = mkIte(b[s], sign_bit, prev_res[i]);
      }
      else
      {
        // if b[s]= 0, use previous value, otherwise shift by threshold  bits
  ...
  {
    // this is fine  because b_ult_a_size has been bit-blasted
    bits[i] = mkIte(b_ult_a_size, prev_res[i], sign_bit);
  }
...
template <class T>
void DefaultUltbvBB (TNode node, std::vector<T>& bits, TBitblaster<T>* bb) {
  Trace("bitvector-bb") << "Bitblasting node " << node  << "\n";
  Assert(bits.empty());
  ...
  // construct bitwise comparison 
  bits.push_back(uLessThanBB(a, b, false));
}

template <class T>
void DefaultSltbvBB (TNode node, std::vector<T>& bits, TBitblaster<T>* bb) {
  Trace("bitvector-bb") << "Bitblasting node " << node  << "\n";
  Assert(bits.empty());
  ...
  // construct bitwise comparison 
  bits.push_back(sLessThanBB(a, b, false));
}

template <class T>
void DefaultIteBB (TNode node, std::vector<T>& bits, TBitblaster<T>* bb) {
  Trace("bitvector-bb") << "Bitblasting node " << node  << "\n";
  Assert(bits.empty());
  ...
  for (unsigned i = 0; i < thenpart.size(); ++i) {
    ...
    bits.push_back(mkAnd(mkOr(mkNot(cond[0]),thenpart[i]),mkOr(cond[0],elsepart[i])));
  }
}
...
void DefaultRepeatBB (TNode node, std::vector<T>& bits, TBitblaster<T>* bb) {
  Trace("bitvector") << "theory::bv:: Unimplemented kind "
                     << node.getKind() << "\n";
  Assert(bits.empty());
  Assert(node.getKind() == Kind::BITVECTOR_REPEAT);
  // this should be rewritten
  Unimplemented();
}

template <class T>
void DefaultZeroExtendBB (TNode node, std::vector<T>& bits, TBitblaster<T>* bb) {
...
template <class T>
void DefaultSignExtendBB (TNode node, std::vector<T>& bits, TBitblaster<T>* bb) {
  Trace("bitvector-bb") << "theory::bv::DefaultSignExtendBB bitblasting " << node  << "\n";
  Assert(bits.empty());
  Assert(node.getKind() == Kind::BITVECTOR_SIGN_EXTEND);

  std::vector<T> small_bits;
  bb->bbTerm(node[0], small_bits);

  T sign_bit = small_bits.back();
  unsigned amount =
      node.getOperator().getConst<BitVectorSignExtend>().d_signExtendAmount;

  for (unsigned i = 0; i < small_bits.size(); ++i ) {
    bits.push_back(small_bits[i]);
  }
  ...
  Assert(bits.size() == amount + small_bits.size());
}

template <class T>
void DefaultRotateRightBB (TNode node, std::vector<T>& bits, TBitblaster<T>* bb) {
  Trace("bitvector") << "theory::bv:: Unimplemented kind "
                     << node.getKind() << "\n";
  Assert(bits.empty());
  Assert(node.getKind() == Kind::BITVECTOR_ROTATE_RIGHT);
  Unimplemented();
}
...
void DefaultRotateLeftBB (TNode node, std::vector<T>& bits, TBitblaster<T>* bb) {
  Trace("bitvector") << "theory::bv:: Unimplemented kind "
                     << node.getKind() << "\n";
  Assert(bits.empty());
  Assert(node.getKind() == Kind::BITVECTOR_ROTATE_LEFT);
  Unimplemented();
}
