...
TrustNode TheoryBVRewriter::expandDefinition(Node node)
{
  ...
  switch (node.getKind())
  {
    ...
    case kind::BITVECTOR_TO_NAT: if (!options::bvLazyReduceExtf()) { ret = utils::eliminateBv2Nat(node); }break;
    case kind::INT_TO_BITVECTOR: if (!options::bvLazyReduceExtf()) { ret = utils::eliminateInt2Bv(node); } break;
    default: break;
  }
