...
enum class LemmaKind : uint32_t
{
  ...
  MUL5_REF1,      //  5: (not (= s (bvnot (bvor t (bvand 1 (bvor x s))))))
  MUL6_REF3,      //  6: (not (= (bvand x t) (bvor s (bvnot t))))
  MUL7_REFN3,     //  7: (not (= t (bvshl (bvor s 1) (bvshl t x))))
  MUL8_REFN14,    //  8: (= s (bvshl s (bvand x (bvlshr 1 t))))
  MUL9_REFN5,     //  9: (bvuge t (bvand 1 (bvlshr (bvand x s) 1)))
  MUL10_REFN6,    // 10: (not (= x (bvxor 1 (bvshl x (bvxor s t)))))
  MUL11_REF14,    // 11: (not (= t (bvor 1 (bvnot (bvxor x s)))))
  MUL12_REF15,    // 12: (not (= t (bvor (bvnot 1) (bvxor x s))))
  MUL13_REFN9,    // 13: (not (= x (bvsub (bvshl x (bvadd s t)) 1)))
  MUL14_REF18,    // 14: (not (= x (bvsub 1 (bvshl x (bvsub s t)))))
  MUL15_REFN11,   // 15: (not (= s (bvadd 1 (bvshl s (bvsub t x)))))
  MUL16_REFN12,   // 16: (not (= s (bvsub 1 (bvshl s (bvsub t x)))))
  MUL17_REFN13,   // 17: (not (= s (bvadd 1 (bvshl s (bvsub x t)))))
  MUL18_REF13,    // 18: (not (= t (bvor 1 (bvadd x s))))
  MUL19_REF12,    // 19: (not (= x (bvnot (bvshl x (bvadd s t)))))

  // Unsigned division (x udiv s = t)
  UDIV1_POW2,    // 1*: (=> (= s 2^i) (= t (bvlshr x i)))
  UDIV__REF1,    //  -: (=> (= s 1) (= t x))
  UDIV2_REF2,    // 2*: (=> (and (= s x) (distinct s 0)) (= t 1))
  UDIV3_REF3,    // 3*: (=> (= s 0) (= t (bvnot 0)))
  UDIV4_REF4,    // 4*: (=> (and (= x 0) (distinct s 0)) (= t 0))
  UDIV5_REF5,    // 5*: (=> (distinct s 0) (bvule t x))
  UDIV6_REF6,    // 6*: (=> (and (= s ~0) (distinct x ~0)) (= t 0))
  UDIV7_REF7,    //  7: (not (bvult x (bvneg (bvand (bvneg s) (bvneg t)))))
  UDIV8_REF8,    //  8: (not (bvult (bvneg (bvor s 1)) t))
  UDIV9_REF9,    //  9: (not (= t (bvneg (bvand s (bvnot x)))))
  UDIV10_REF10,  // 10: (not (= (bvor s t) (bvand x (bvnot 1))))
  UDIV11_REF11,  // 11: (not (= (bvor s 1) (bvand x (bvnot t))))
  UDIV12_REF12,  // 12: (not (bvult (bvand x (bvneg t)) (bvand s t)))
  UDIV13_REF13,  // 13: (not (bvult s (bvlshr x t)))
  UDIV14_REF14,  // 14: (not (bvult x (bvshl (bvlshr s (bvshl s t)) 1)))
  UDIV15_REF15,  // 15: (not (bvult x (bvlshr (bvshl t 1) (bvshl t s))))
  UDIV16_REF16,  // 16: (not (bvult t (bvshl (bvlshr x s) 1)))
  UDIV17_REF17,  // 17: (not (bvult x (bvand (bvor x t) (bvshl s 1))))
  UDIV18_REF18,  // 18: (not (bvult x (bvand (bvor x s) (bvshl t 1))))
  UDIV19_REF19,  // 19: (not (= (bvlshr x t) (bvor s t)))
  UDIV20_REF20,  // 20: (not (= s (bvnot (bvlshr s (bvlshr t 1)))))
  UDIV21_REF21,  // 21: (not (= x (bvnot (bvand x (bvshl t 1)))))
  UDIV22_REF23,  // 22: (not (bvult t (bvlshr (bvshl x 1) s)))
  UDIV23_REF24,  // 23: (not (bvult x (bvshl s (bvnot (bvor x t)))))
  UDIV24_REF25,  // 24: (not (bvult x (bvshl t (bvnot (bvor x s)))))
  UDIV25_REF26,  // 25: (not (bvult x (bvxor t (bvlshr t (bvlshr s 1)))))
  UDIV26_REF27,  // 26: (not (bvult x (bvxor s (bvlshr s (bvlshr t 1)))))
  UDIV27_REF28,  // 27: (not (bvult x (bvshl s (bvnot (bvxor x t)))))
  UDIV28_REF29,  // 28: (not (bvult x (bvshl t (bvnot (bvxor x s)))))
  UDIV29_REF30,  // 29: (not (= x (bvadd t (bvor s (bvadd x s)))))
  UDIV30_REF31,  // 30: (not (= x (bvadd t (bvadd 1 (bvshl 1 x)))))
  UDIV31_REF32,  // 31: (not (bvult s (bvlshr (bvadd x t) t)))
  UDIV32_REF33,  // 32: (not (= x (bvadd t (bvadd t (bvor x s)))))
  UDIV33_REF34,  // 33: (not (bvult (bvxor s (bvor x t)) (bvxor t 1)))
  UDIV34_REF36,  // 34: (not (bvult t (bvlshr x (bvsub s 1))))
  UDIV35_REF37,  // 35: (not (bvult (bvsub s 1) (bvlshr x t)))
  UDIV36_REF38,  // 36: (not (= x (bvsub 1 (bvshl x (bvsub x t)))))

  // Unsigned remainder (x urem s = t)
  UREM1_POW2,    // 1*: (=> (= s 2^i) (= t (concat 0[w-i] x[i-1:0])))
  UREM2_REF1,    // 2*: (=> (distinct s 0) (bvult t s))
  UREM3_REF2,    // 3*: (=> (= x 0) (= t 0))
  UREM4_REF3,    // 4*: (=> (= s 0) (= t x))
  UREM5_REF4,    // 5*: (=> (= s x) (= t 0))
  UREM6_REF5,    // 6*: (=> (bvult x s) (= t x))
  UREM7_REF6,    // 7*: (bvuge (bvnot (bvneg s)) t)
  UREM8_REF7,    //  8: (= x (bvand x (bvor s (bvor t (bvneg s)))))
  UREM9_REF8,    //  9: (not (bvult x (bvor t (bvand x s))))
  UREM10_REF9,   // 10: (not (= 1 (bvand t (bvnot (bvor x s)))))
  UREM11_REF10,  // 11: (not (= t (bvor (bvnot x) (bvneg s))))
  UREM12_REF11,  // 12: (not (bvult (bvand t (bvor x s)) (bvand t 1)))
  UREM13_REF12,  // 13: (not (= x (bvor (bvneg x) (bvneg (bvnot t)))))
  UREM14_REF13,  // 14: (not (bvult (bvadd x (bvneg s)) t))
  UREM15_REF14,  // 15: (not (bvult (bvxor (bvneg s) (bvor x s)) t))
};
