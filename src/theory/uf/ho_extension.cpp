 ...
 **
 ** \brief This is the interface to HoExtension implementations
 **
 **/
...
HoExtension::HoExtension(TheoryUF& p, context::Context* c,
      ...
      for( unsigned k=(j+1), sizek = itf->second.size(); k<sizek; k++ ){ 
             ...
             itf->second[j], itf->second[k], false))
        {
          ...
          if (isCollectModel)
          {
            ...
            Assert(edeq.getKind() == NOT
                   && edeq[0].getKind() == EQUAL);
