...
void Region::takeNode( Region* r, Node n ){
  CVC4_DCHECK(!hasRep(n));
  CVC4_DCHECK(r->hasRep(n));
  //add representative
  setRep( n, true );
    ...
    if( d_regions_map.find( n )==d_regions_map.end() ){
      ...
      if( options::ufssTotality() ){
        ...
        if( d_regions_index<d_regions.size() ){
          ...
          CVC4_DCHECK(d_regions[d_regions_index]->getNumReps() == 0);
        }else{
  ...
  if( !d_conflict ){
    if( options::ufssTotality() ){
      ...
      Debug("uf-ss") << "CardinalityExtension: Merging " << a << " = " << b
                     << "..." << std::endl;
      if( a!=b ){
        CVC4_DCHECK(d_regions_map.find(a) != d_regions_map.end());
        CVC4_DCHECK(d_regions_map.find(b) != d_regions_map.end());
...
bool SortModel::areDisequal( Node a, Node b ) {
  CVC4_DCHECK(
      a == d_thss->getTheory()->getEqualityEngine()->getRepresentative(a));
  CVC4_DCHECK(
      b == d_thss->getTheory()->getEqualityEngine()->getRepresentative(b));
...
void SortModel::check( Theory::Effort level, OutputChannel* out ){
  CVC4_DCHECK(options::ufssMode() == UF_SS_FULL);
                                          ...
                                          std::map< int, int >& regions_diseq)
{
  ...
  for(Region::iterator it = region->begin(); it != region->end(); ++it ){
    if( it->second->valid() ){
      ...
      for( DiseqList::iterator it2 = del->begin(); it2 != del->end(); ++it2 ){
        if( (*it2).second ){
          CVC4_DCHECK(isValid(d_regions_map[(*it2).first]));
...
int SortModel::forceCombineRegion( int ri, bool useDensity ){
  if( !useDensity ){
    ...
    for( std::map< int, int >::iterator it = regions_diseq.begin(); it != regions_diseq.end(); ++it ){
      CVC4_DCHECK(it->first != ri);
      CVC4_DCHECK(isValid(it->first));
      CVC4_DCHECK(d_regions[it->first]->getNumReps() > 0);
...
int SortModel::combineRegions( int ai, int bi ){
  ...
  CVC4_DCHECK(isValid(ai) && isValid(bi));
...
void SortModel::moveNode( Node n, int ri ){
  ...
  CVC4_DCHECK(isValid(d_regions_map[n]));
  CVC4_DCHECK(isValid(ri));
...
int SortModel::addSplit( Region* r, OutputChannel* out ){
  ...
  if( r->hasSplits() ){
    ...
    CVC4_DCHECK(s != Node::null());
  }
  if (!s.isNull() ){
    //add lemma to output channel
    CVC4_DCHECK(s.getKind() == EQUAL);
...
void CardinalityExtension::assertNode(Node n, bool isDecision)
{
  ...
  if( options::ufssMode()==UF_SS_FULL ){
    if( lit.getKind()==CARDINALITY_CONSTRAINT ){
      ...
      CVC4_DCHECK(tn.isSort());
      CVC4_DCHECK(d_rep_model[tn]);
