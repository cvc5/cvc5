...
void Region::addRep(Node n) { setRep(n, true); }
...
void Region::setDisequal(Node n1, Node n2, int type, bool valid)
{
  ...
  if (isDisequal(n1, n2, type) != valid)
    ...
    else
    {
      ...
      if (valid)
      {
            ...
            && d_testClique.find(n2) != d_testClique.end() && d_testClique[n2])
        {
          ...
          {
            Trace("uf-ss-debug")
                << "removing split for " << n1 << " " << n2 << std::endl;
            d_splits[eq] = false;
...
bool Region::check(Theory::Effort level,
                   int cardinality,
    ...
    else if (level == Theory::EFFORT_FULL)
    {
      ...
      if (d_testCliqueSize <= unsigned(cardinality))
      {
        ...
        else
        {
          ...
          int maxDeg = -1;
          Node maxNode;
          for (std::map<Node, RegionNodeInfo*>::iterator it = d_nodes.begin();
               it != d_nodes.end();
               ++it)
          {
            // if not in the test clique, add it to the set of new members
        ...
        {
          Trace("uf-ss-debug")
              << "Choose to add clique member " << newClique[j] << std::endl;
...
void SortModel::presolve() { d_initialized = false; }
...
int SortModel::forceCombineRegion(int ri, bool useDensity)
{
  ...
  else
  {
    ...
    {
      Trace("uf-ss-check-region")
          << "We must combine Region #" << ri << ". " << std::endl;
    ...
    for (std::map<int, int>::iterator it = regions_diseq.begin();
         it != regions_diseq.end();
         ++it)
    {
      Trace("uf-ss-check-region")
         ...
         ++it)
    {
      ...
      double tempScore =
          double(it->second) / double(d_regions[it->first]->getNumReps());
    ...
    if (maxRegion != -1)
    {
      ...
      {
        Trace("uf-ss-check-region")
            << "Combine with region #" << maxRegion << ":" << std::endl;
...
{
  Trace("uf-ss-region") << "uf-ss: Combine Region #" << bi << " with Region #"
                        << ai << std::endl;
...
{
  Trace("uf-ss-region") << "uf-ss: Move node " << n << " to Region #" << ri
                        << std::endl;
...
int SortModel::addSplit(Region* r)
{
  ...
  if (r->hasSplits())
  {
    ...
    for (Region::split_iterator it = r->begin_splits(); it != r->end_splits();
         ++it)
    {
      if ((*it).second)
      {
  ...
  if (!s.isNull())
  {
    ...
    if (ss.getKind() != Kind::EQUAL)
    {
      ...
      if (ss == b_f)
      {
        Trace("uf-ss-lemma") << "....Assert disequal directly : " << s[0] << " "
                             << s[1] << std::endl;
    ...
    // << " "; Trace("uf-ss-lemma") << d_th->getEqualityEngine()->areDisequal(
    // s[0], s[1] ) << std::endl; Trace("uf-ss-lemma") << s[0].getType() << " "
    // << s[1].getType() << std::endl; split on the equality s
...
{
  if (d_maxNegCard.get() != 0 && d_hasCard.get()
      && d_cardinality.get() < d_maxNegCard.get())
  {
    Node lem = NodeManager::mkNode(
        Kind::AND,
        {getCardinalityLiteral(d_cardinality.get()),
         getCardinalityLiteral(d_maxNegCard.get()).negate()});
    Trace("uf-ss-lemma") << "*** Simple cardinality conflict : " << lem
                         << std::endl;
    ...
    while (!eqcs_i.isFinished())
    {
      ...
      if (eqc.getType() == d_type)
      {
        if (std::find(eqcs.begin(), eqcs.end(), eqc) == eqcs.end())
        {
          ...
          {
            Trace("uf-ss-warn") << "WARNING : equivalence class " << eqc
  ...
  if (nReps != d_maxNegCard + 1)
  {
    ...
    if (d_maxNegCard >= nReps)
    {
      ...
      else
      {
        ...
        Trace("uf-ss-lemma")
            << "*** Enforce negative cardinality constraint lemma : " << lem
            << std::endl;
...
void CardinalityExtension::assertNode(Node n, bool isDecision)
{
  ...
  if (options().uf.ufssMode == options::UfssMode::FULL)
  {
    if (lit.getKind() == Kind::CARDINALITY_CONSTRAINT)
    {
    ...
    else if (lit.getKind() == Kind::COMBINED_CARDINALITY_CONSTRAINT)
    {
      if (polarity)
      {
      ...
      if (TraceIsOn("uf-ss-warn"))
      {
        ...
        if (isDecision)
        {
               ...
               ++it)
          {
            ...
            {
              Trace("uf-ss-warn") << "WARNING: Assert " << n
                                  << " as a decision before cardinality for "
                                  << it->first << "." << std::endl;
            }
          }
        }
  ...
  {
    if (lit.getKind() == Kind::CARDINALITY_CONSTRAINT
        || lit.getKind() == Kind::COMBINED_CARDINALITY_CONSTRAINT)
    {
      // cardinality constraint from user input, set incomplete
      Trace("uf-ss")
          << "Literal " << lit
          << " not handled when uf ss mode is not FULL, set incomplete."
          << std::endl;
...
void CardinalityExtension::check(Theory::Effort level)
{
  ...
  if (!d_state.isInConflict())
  {
    ...
    else if (options().uf.ufssMode == options::UfssMode::NO_MINIMAL)
    {
      if (level == Theory::EFFORT_FULL)
      {
        // split on an equality between two equivalence classes (at most one per
        // type)
        ...
        while (!eqcs_i.isFinished())
        {
          ...
          if (tn.isUninterpretedSort())
          {
            ...
            {
              std::map<TypeNode, std::vector<Node> >::iterator itel =
                  eqc_list.find(tn);
...
void CardinalityExtension::preRegisterTerm(TNode n)
{
  ...
  // quantifiers over tn)
  TypeNode tn;
  if (n.getKind() == Kind::CARDINALITY_CONSTRAINT)
  {
    const CardinalityConstraint& cc =
...
void CardinalityExtension::debugPrint(const char* c)
{
  for (std::map<TypeNode, SortModel*>::iterator it = d_rep_model.begin();
       ...
       ++it)
  {
    Trace(c) << "Conflict find structure for " << it->first << ": "
             << std::endl;
  ...
  if (options().uf.ufssFairness)
  {
    ...
    if (d_min_pos_com_card_set.get() && totalCombinedCard > cc)
    {
      ...
      Trace("uf-ss-lemma") << "*** Combined cardinality conflict : " << cf
                           << std::endl;
      Trace("uf-ss-com-card")
          << "*** Combined cardinality conflict : " << cf << std::endl;
