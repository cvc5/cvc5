...
TheoryFp::TheoryFp(context::Context *c, context::UserContext *u,
    ...
    : Theory(THEORY_FP, c, u, out, valuation, logicInfo),
      d_notification(*this),
      d_d_equalityEngine(d_notification, c, "theory::fp::TheoryFp", true),
      d_registeredTerms(u),
      d_conv(u),
...
void TheoryFp::convertAndEquateTerm(TNode node) {
  ...
  size_t oldAdditionalAssertions = d_conv.additionalAssertions.size();
  ...
  size_t newAdditionalAssertions = d_conv.additionalAssertions.size();
  Assert(oldAdditionalAssertions <= newAdditionalAssertions);

  while (oldAdditionalAssertions < newAdditionalAssertions) {
    Node addA = d_conv.additionalAssertions[oldAdditionalAssertions];
