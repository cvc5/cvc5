...
TheoryFpRewriter::TheoryFpRewriter()
{
  ...
  RewriteResponse TheoryFpRewriter::preRewrite(TNode node) {
    ...
    if (res.d_node != node) {
      ...
      Debug("fp-rewrite") << "TheoryFpRewriter::preRewrite(): after  " << res.d_node << std::endl;
    }
    return res;
  }
  ...
  RewriteResponse TheoryFpRewriter::postRewrite(TNode node) {
    ...
    if (res.d_node != node) {
      ...
      Debug("fp-rewrite") << "TheoryFpRewriter::postRewrite(): after  " << res.d_node << std::endl;
    }
    ...
    if (res.d_status == REWRITE_DONE) {
      ...
      for (Node::const_iterator i = res.d_node.begin();
	   i != res.d_node.end();
	   ++i) {
	...
	if (constRes.d_node != res.d_node) {
	  Debug("fp-rewrite") << "TheoryFpRewriter::postRewrite(): before constant fold " << res.d_node << std::endl;
	  Debug("fp-rewrite") << "TheoryFpRewriter::postRewrite(): after constant fold " << constRes.d_node << std::endl;
	}
