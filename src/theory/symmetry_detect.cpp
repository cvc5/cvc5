...
SymmetryDetect::Partition SymmetryDetect::detect( vector< Node >& assertions )
{
  ...
  Trace("sym-dt") << endl << "------------------------------ The Final Partition ------------------------------" << std::endl;
  printPartition( p );
  return p;
}

void SymmetryDetect::getPartition( vector< vector< Node > >& parts, vector< Node >& assertions)
{
  Partition p = detect( assertions );
  map< Node, vector< Node > >::iterator subvar_to_vars_it = p.d_subvar_to_vars.begin();
...
SymmetryDetect::Partition SymmetryDetect::findPartitions( Node node )
{
  ...
  {
    Trace("sym-dt") << "[sym-dt] We have seen the node " << node << " before, thus we return its partition from cache."<< endl;
  ...
  Trace("sym-dt") << "[sym-dt] collectChildren for: " << node << " with operator " << node.getKind() << std::endl;
  collectChildren( node, children );
  Trace("sym-dt") << "[sym-dt] children: " << printNodeVector( children ) << std::endl;
  ...
  Trace("sym-dt") << "----------------------------- Start processing partitions -------------------------------" << endl;
...
void SymmetryDetect::matches( vector< Partition >& partitions, map< Node, Node >& subvar_to_var, map< Node, Node >& subvar_to_expr)
{
  Trace("sym-dt") << "[sym-dt] Start testing if singleton partitions can be merged!"<< endl;
  ...
  if( !subvar_to_expr.empty() )
  {
    ...
    for( map< Node, Node >::iterator expr_it1 = subvar_to_expr.begin(); expr_it1 != subvar_to_expr.end(); ++expr_it1 )
    {
      ...
      Node sub_expr1 = (expr_it1->second).substitute( subs.begin(), subs.end(), repls.begin(), repls.end() );

      for( map< Node, Node >::iterator expr_it2 = subvar_to_expr.begin(); expr_it2 != subvar_to_expr.end(); ++expr_it2 )
      {
        ...
        Node sub_expr2 = (expr_it2->second).substitute( subs2.begin(), subs2.end(), repls.begin(), repls.end() );
        Trace("sym-dt") << "[sym-dt] Testing if " << sub_expr1 << " is equivalent to " << sub_expr2 << endl;

        if( sub_expr1 == sub_expr2 )
        {
          Trace("sym-dt") << "[sym-dt] Merge variable " << fst_var << " with variable " << snd_var << endl;
...
void SymmetryDetect::processSingletonPartitions( vector< Partition >& partitions, unordered_set< Node, NodeHashFunction >& vars )
{
  Trace("sym-dt") << "[sym-dt] Start post-processing partitions with size = " << partitions.size() << endl;
  ...
  {
    if( !(*part_it).d_var_to_subvar.empty() && (*part_it).d_var_to_subvar.size() == 1 )
    {
      vars.insert( ((*part_it).d_var_to_subvar.begin())->first );
      subvar_to_expr[((*part_it).d_var_to_subvar.begin())->second] = (*part_it).d_term;
      subvar_to_var[((*part_it).d_var_to_subvar.begin())->second] = ((*part_it).d_var_to_subvar.begin())->first;
    }
    else if( !(*part_it).d_var_to_subvar.empty() && (*part_it).d_var_to_subvar.size() >= 2 )
    {
      map< Node, Node >::iterator var_to_subvar_it = (*part_it).d_var_to_subvar.begin();
...
{
  Trace("sym-dt") << "[sym-dt] Getting the new partitions from the leaves of the partition trie:" << std::endl;
  if( !d_variables.empty() )
  {
    vector< Node > vars;
    vector< Node >::iterator var_it = d_variables.begin();
    Node fresh_var = NodeManager::currentNM()->mkSkolem("sym_bk", d_variables[0].getType());
...
void SymmetryDetect::PartitionTrie::getNewPartition( Partition& part, PartitionTrie& pt )
{
  if( !pt.d_variables.empty() )
  {
    vector< Node > vars;
    vector< Node >::iterator var_it = pt.d_variables.begin();
    Node fresh_var = NodeManager::currentNM()->mkSkolem("sym_bk", pt.d_variables[0].getType());
    Trace("sym-dt") << "[sym-dt] A partition from leaves of the partition trie:{";
...
void SymmetryDetect::getVariables(vector< Partition >& partitions, unordered_set< Node, NodeHashFunction >& vars)
{
  vector< Partition >::iterator part_it = partitions.begin();

  while( part_it != partitions.end() )
  {
    map< Node, vector< Node > >::iterator sub_var_it = (*part_it).d_subvar_to_vars.begin();
...
void SymmetryDetect::PartitionTrie::addNode( Node v, vector< Partition >& partitions )
{
  Trace("sym-dt") << "[sym-dt] Add a variable {" << v << "} to the partition trie..." << endl;
  vector< Node > subvars;
  vector< Partition >::iterator part_it = partitions.begin();
  ...
  while( part_it != partitions.end() )
  {
    ...
    {
      map< Node, vector< Node > >::iterator sub_vars_it = (*part_it).d_subvar_to_vars.begin();
  ...
  Trace("sym-dt") << "[sym-dt] Substitution variables for the variable " << v << ": " << SymmetryDetect::printNodeVector( subvars ) << endl;
