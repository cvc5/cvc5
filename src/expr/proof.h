...
class CDProof : public ProofGenerator
{
 public:
  ...
  /** As above, but stores steps in a proof step buffer. */
  static Node factorReorderElimDoubleNeg(Node n, ProofStepBuffer& psb);

  /**
   * Eliminate double negation of a literal if it has the form (not (not t)). If
   * the elimination happens, a step is added to the given proof, if any.
   *
  ...
  /** As above, but stores steps in a proof step buffer. */
  static Node elimDoubleNegLit(Node n, ProofStepBuffer& psb);
