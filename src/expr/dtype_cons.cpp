...
void DTypeConstructor::setSygus(Node op)
{
  ...
  if (op.isConst())
  {
    ...
    if (nk != ok)
    {
      ...
      d_sygusOp = NodeManager::currentNM()->operatorOf(nk);
    }
  }
  else
  {
...
Kind DTypeConstructor::getEliminateKind(Kind ok)
{
  Kind nk = ok;
...
Node DTypeConstructor::eliminatePartialOperators(Node n)
{
  NodeManager* nm = NodeManager::currentNM();
