...
ArrayStoreAll::ArrayStoreAll(const ArrayType& type,
  ...
  PrettyCheckArgument(type.isArray(), type) 
      << "array store-all constants can only be created for array types, not `"
      << type.toString() << "'" << std::endl;

  PrettyCheckArgument(expr.getType().isComparableTo(type.getConstituentType()), expr)
      << "expr type `" << expr.getType().toString() 
      << "' does not match constituent type of array type `" << type.toString() 
      << "'" << std::endl;

  PrettyCheckArgument(expr.isConst(), expr) << "ArrayStoreAll requires a constant expression" << std::endl;
