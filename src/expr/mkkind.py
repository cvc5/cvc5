...
class CodeGenerator:
    ...
    def register_kind(self, kinds, theory_id, filename, name_property="name", comment_property="comment"):
        ...
        for kind in kinds:
            ...
            if "well-founded" in kind:
                self.register_wellfounded(kind["well-founded"], filename, kind_name)
    ...
    def register_wellfounded(self, wellfounded, filename, kind_name):
        ...
        if computer != 'false':
            if not groundterm_computer:
                print(f"{filename}: error: ground-term computer missing in command \"well-founded\"")
                exit(1)
        else:
            if groundterm_computer:
                print(f"{filename}: error: ground-term computer specified for not-well-founded type")
                exit(1)
        
        if groundterm_computer:
            groundterm_computer = groundterm_computer.replace('%TYPE%', 'typeNode')
    ...
    def register_cardinality(self, cardinality, kind_name):
        ...
        cardinality_computer = cardinality["computer"].replace('%TYPE%', 'typeNode')
    ...
    def fill_register_kind_template_data(self):
        self.fill_template(self.kinds_enum_header_replacement_pattern, self.kinds_enum_header)
        self.fill_template(self.kind_printers_replacement_pattern, self.kind_printers)
        self.fill_template(self.kind_to_theory_id_replacement_pattern, self.kind_to_theory_id)

    def fill_register_sort_template_data(self):
        self.fill_template(self.type_constant_descriptions_replacement_pattern, self.type_constant_descriptions)
        self.fill_template(self.type_constant_to_theory_id_replacement_pattern, self.type_constant_to_theory_id)
        self.fill_template(self.type_constant_wellfoundednesses_replacement_pattern, self.type_constant_wellfoundednesses)
        self.fill_template(self.type_constant_cardinalities_replacement_pattern, self.type_constant_cardinalities)
        self.fill_template(self.type_constant_groundterms_replacement_pattern, self.type_constant_groundterms)
        
    def fill_register_cardinality_template_data(self):
        self.fill_template(self.type_properties_includes_replacement_pattern, self.type_properties_includes)
        self.fill_template(self.type_cardinalities_replacement_pattern, self.type_cardinalities)
    
    def fill_register_wellfoundedness_template_data(self):
        self.fill_template(self.type_wellfoundednesses_replacement_pattern, self.type_wellfoundednesses)
        self.fill_template(self.type_groundterms_replacement_pattern, self.type_groundterms)
    
    def fill_constant_list_header_template_data(self):
        self.fill_template(self.type_constant_list_replacement_pattern, self.type_constant_list)
