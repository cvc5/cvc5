...
void nodeTriePathCompareInternal(const TNodeTrie* t1,
                            const TNodeTrie* t2,
                            size_t arity,
                            size_t depth, NodeTriePathCompareCallback& ntpc)
{
  if( depth==arity ){
    if( t2!=nullptr ){
      Node f1 = t1->getData();
    ...
    if( depth<(arity-1) ){
      ...
      {
        nodeTriePathCompareInternal(&tt.second, nullptr, arity, depth + 1, ntpc);
      }
    }
          ...
          ++it)
    {
      ...
      for( ; it2 != t1->d_data.end(); ++it2 ){
        ...
        {
          nodeTriePathCompareInternal( &it->second, &it2->second, arity, depth+1, ntpc );
        }
      }
    }
  }else{
    ...
    for (const std::pair<const TNode, TNodeTrie>& tt1 : t1->d_data)
    {
      for (const std::pair<const TNode, TNodeTrie>& tt2 : t2->d_data)
      {
        ...
        {
          nodeTriePathCompareInternal(&tt1.second, &tt2.second, arity, depth + 1);
        }
      }
    }
  }
