                                ...
                                std::vector<Node>& sols)
{
  if (assertions.empty())
  {
    ...
    Warning() << "Cannot convert to sygus since there are no assertions." << std::endl;
    return false;
  }
  ...
  for (const Node& as : eassertions)
  {
    ...
    if (pas.getKind() == Kind::FORALL)
    {
      ...
      if (!qa.isStandard())
      {
        ...
        Warning() << "Cannot convert to sygus since there is a non-standard top-level quantified formula: " << pas << std::endl;
        return false;
      }
      // infer prefix
    ...
    do
    {
      ...
      if (visited.find(cur) == visited.end())
      {
        ...
        else if (cur.isClosure())
        {
          ...
          Warning() << "Cannot convert to sygus since there is a non-top-level quantified formula: " << cur << std::endl;
          return false;
        }
  ...
  {
    Warning() << "Cannot convert to sygus since there are no free function symbols." << std::endl;
  ...
  if (!rrSygus->getSubsolverSynthSolutions(synth_sols))
  {
    ...
    std::stringstream ss;
    ss << "Translated to sygus, but failed to show problem to be satisfiable with --sygus-inference.";
    throw LogicException(ss.str());
    return true;
  }
