...
size_t removeFromConjunction(Node& n, const std::unordered_set<unsigned long>& toRemove) {
   ...
   * BackEdgesMap.
   */
  void traceBackToAssertions(booleans::CircuitPropagator* propagator, const std::vector<Node>& nodes, std::vector<TNode>& assertions) {
    const booleans::CircuitPropagator::BackEdgesMap& backEdges = propagator->getBackEdges();
    ...
    AssertionPipeline* assertionsToPreprocess)
{
  ...
  booleans::CircuitPropagator* propagator = d_preprocContext->getCircuitPropagator();
  const booleans::CircuitPropagator::BackEdgesMap& backEdges = propagator->getBackEdges();
  unordered_set<unsigned long> removeAssertions;
  SubstitutionMap& top_level_substs = assertionsToPreprocess->getTopLevelSubstitutions();
  ...
  for(TNode v : d_boolVars) {
    if(propagator->isAssigned(v)) {
      Debug("miplib") << "ineligible: " << v << " because assigned " << propagator->getAssignment(v) << endl;
      continue;
    }

    vector<TNode> assertions;
    booleans::CircuitPropagator::BackEdgesMap::const_iterator j = backEdges.find(v);
    ...
    for(vector<TNode>::const_iterator j = assertions.begin(); j != assertions.end(); ++j) {
      ...
      if(conj.getKind() == kind::AND) {
        ...
        if(!found_x) {
          eligible = false;
          Debug("miplib") << "  --INELIGIBLE -- (couldn't find " << v << " in conjunction)" << endl;
          break;
        }
    ...
    if(eligible) {
      for(map<pair<Node, Node>, uint64_t>::const_iterator j = marks.begin(); j != marks.end(); ++j) {
        ...
        if(mark != expected) {
          ...
          Debug("miplib") << "  -- ELIGIBLE " << v << " , " << pos << " --" << endl;
          vector<Node> newVars;
  ...
  if(!removeAssertions.empty()) {
    ...
    for(size_t i = 0, size = assertionsToPreprocess->getRealAssertionsEnd(); i < size; ++i) {
      Node assertion = (*assertionsToPreprocess)[i];
      if(removeAssertions.find(assertion.getId()) != removeAssertions.end()) {
        Debug("miplib") << "SmtEnginePrivate::simplify(): - removing " << assertion << endl;
        assertionsToPreprocess->replace(i, trueNode);
        ++d_statistics.d_numMiplibAssertionsRemoved;
      } else if(assertion.getKind() == kind::AND) {
        size_t removals = removeFromConjunction(assertion, removeAssertions);
        if(removals > 0) {
          Debug("miplib") << "SmtEnginePrivate::simplify(): - reduced " << assertion << endl;
      ...
      assertionsToPreprocess->replace(i,
          Rewriter::rewrite(top_level_substs.apply(assertion)));
