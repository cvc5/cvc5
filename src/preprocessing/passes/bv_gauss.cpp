    ...
    AssertionPipeline* assertionsToPreprocess)
{
  ...
  for (const auto& eq : equations)
  {
    ...
    if (ret != BVGauss::Result::INVALID)
    {
      if (ret == BVGauss::Result::NONE)
      {
        ...
        assertionsToPreprocess->push_back(n, false, nullptr, TrustId::PREPROCESS_BV_GUASS_LEMMA);
      ...
      else
      {
        ...
        for (const auto& p : res)
        {
          ...
          // add new assertion
          assertionsToPreprocess->push_back(a, false, nullptr, TrustId::PREPROCESS_BV_GUASS_LEMMA);
        }
      }
    }
  }
  ...
  if (!subst.empty())
  {
    ...
    for (size_t i = 0, asize = aref.size(); i < asize; ++i)
    {
      ...
      assertionsToPreprocess->replace(i, as, nullptr, TrustId::PREPROCESS_BV_GUASS);
    }
  }
