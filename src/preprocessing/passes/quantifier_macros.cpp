...
bool QuantifierMacros::simplify(std::vector<Node>& assertions, bool doRewrite)
{
  ...
  {
    d_ground_macros = (r == 0);
    Trace("macros") << "Find macros, ground=" << d_ground_macros << "..."
                    << std::endl;
    // first, collect macro definitions
    std::vector<Node> macro_assertions;
    for (int i = 0; i < (int)assertions.size(); i++)
    {
      Trace("macros-debug") << "  process assertion " << assertions[i]
                            << std::endl;
      if (processAssertion(assertions[i]))
      {
        PROOF(if (std::find(macro_assertions.begin(),
    ...
    Trace("macros") << "...finished process, #new def = "
                    << d_macro_defs_new.size() << std::endl;
    if (doRewrite && !d_macro_defs_new.empty())
    {
      ...
      for (unsigned i = 0; i < assertions.size(); i++)
      {
        ...
        if (curr != assertions[i])
        {
          ...
          Trace("macros-rewrite") << "Rewrite " << assertions[i] << " to "
                                  << curr << std::endl;
          // for now, it is dependent upon all assertions involving macros, this
          // is an over-approximation.
          // a more fine-grained unsat core computation would require caching
          // dependencies for each subterm of the formula,
          // which is expensive.
    ...
    for (unsigned i = 0; i < n.getNumChildren(); i++)
    {
      if (processAssertion(n[i]))
      {
        return true;
           ...
           && d_quant_macros.find(n) == d_quant_macros.end())
  {
...
bool QuantifierMacros::containsBadOp(Node n,
                                     Node op,
                                     std::vector<Node>& opc,
...
bool QuantifierMacros::getFreeVariables(Node n,
...
bool QuantifierMacros::getSubstitution(std::vector<Node>& v_quant,
...
bool QuantifierMacros::process(Node n,
                               bool pol,
                               std::vector<Node>& args,
                               Node f)
{
    ...
    if (isMacroLiteral(n, pol))
    {
      ...
      for (size_t i = 0; i < candidates.size(); i++)
      {
        ...
        if (d_macro_defs.find(op) == d_macro_defs.end())
        {
          ...
          if (!n_def.isNull())
          {
            Trace("macros-debug") << m << " is possible macro in " << f
                                  << std::endl;
            Trace("macros-debug") << "  corresponding definition is : " << n_def
                                  << std::endl;
            visited.clear();
            // definition must exist and not contain any free variables apart
            // from fvs
            if (!getFreeVariables(n_def, args, fvs, true, visited))
            {
              Trace("macros-debug") << "...free variables are contained."
                                    << std::endl;
              visited.clear();
              // cannot contain a defined operator, opc is list of functions it
              // contains
              std::vector<Node> opc;
              if (!containsBadOp(n_def, op, opc, visited))
              {
                Trace("macros-debug")
                    << "...does not contain bad (recursive) operator."
                    << std::endl;
                // must be ground UF term if mode is GROUND_UF
                if (options::macrosQuantMode() != MACROS_QUANT_MODE_GROUND_UF
                    || isGroundUfTerm(f, n_def))
                {
                  Trace("macros-debug") << "...respects ground-uf constraint."
                                        << std::endl;
                  // now we must rewrite candidates[i] to a term of form g( x1,
                  // ..., xn ) where
      ...
      if (n.getKind() == APPLY_UF)
      {
        ...
        if (it != d_macro_defs.end() && !it->second.isNull())
        {
          ...
          if (success)
          {
            ...
            ret = it->second;
            std::map<Node, std::vector<Node> >::iterator itb =
                d_macro_basis.find(op);
            if (itb != d_macro_basis.end())
            {
              ret = ret.substitute(itb->second.begin(),
                                   itb->second.end(),
                                   children.begin(),
                                   children.end());
            }
            if (!cond.empty())
            {
              Node cc = cond.size() == 1
                            ? cond[0]
                            : NodeManager::currentNM()->mkNode(kind::AND, cond);
...
void QuantifierMacros::debugMacroDefinition(Node oo, Node n)
{
  ...
  if (n.getKind() == APPLY_UF)
  {
    ...
    if (d_macro_defs.find(op) != d_macro_defs.end())
    {
      ...
      {
        Trace("macros-warn") << "BAD DEFINITION for macro " << oo << " : "
                             << d_macro_defs[oo] << std::endl;
      }
      else
      {
        Trace("macros-warn") << "BAD ASSERTION " << oo << std::endl;
      }
      Trace("macros-warn") << "  contains defined function " << op << "!!!"
                           << std::endl;
    }
  }
  for (unsigned i = 0; i < n.getNumChildren(); i++)
...
void QuantifierMacros::finalizeDefinitions()
{
  ...
  if (options::incrementalSolving() || options::produceModels() || doDefs)
  {
    ...
    for (std::map<Node, Node>::iterator it = d_macro_defs.begin();
         it != d_macro_defs.end();
         ++it)
    {
      Trace("macros-def") << "Macro definition for " << it->first << " : "
                          << it->second << std::endl;
      ...
      {
        Node bv = NodeManager::currentNM()->mkBoundVar(
            d_macro_basis[it->first][i].getType());
        Trace("macros-def") << d_macro_basis[it->first][i] << " ";
        nargs.push_back(bv);
        args.push_back(bv.toExpr());
      }
      Trace("macros-def") << std::endl;
      Node sbody = it->second.substitute(d_macro_basis[it->first].begin(),
                                         d_macro_basis[it->first].end(),
                                         nargs.begin(),
                                         nargs.end());
      smt::currentSmtEngine()->defineFunction(
          it->first.toExpr(), args, sbody.toExpr());
...
{
  Trace("macros") << "* " << n << " is a macro for " << op
                  << ", #op contain = " << opc.size() << std::endl;
  ...
  Trace("macros-debug") << "...substitute into "
                        << d_macro_def_contains[op].size()
                        << " previous definitions." << std::endl;
  ...
  for (unsigned i = 0; i < opc.size(); i++)
  {
    ...
    {
      Node dop = dep_ops[j];
      if (std::find(d_macro_def_contains[opc[i]].begin(),
                    d_macro_def_contains[opc[i]].end(),
                    dop)
          == d_macro_def_contains[opc[i]].end())
      {
