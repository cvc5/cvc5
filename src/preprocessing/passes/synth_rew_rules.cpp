...
{
  Trace("synth-rr-pass") << "Synthesize rewrite rules from assertions..." << std::endl;
  ...
  // two passes: the first collects the variables, the second registers the terms
  for( unsigned r=0; r<2; r++ )
  {
    ...
    if( r==0 )
    {
      Trace("synth-rr-pass-debug") << "Initialize with " << nsamples << " samples and variables : " << vars << std::endl;
      crdg = std::unique_ptr<theory::quantifiers::CandidateRewriteDatabaseGen>(new theory::quantifiers::CandidateRewriteDatabaseGen(vars,nsamples));
    }
  }
