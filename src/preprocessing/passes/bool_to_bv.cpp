...
Node BoolToBV::fromCache(TNode n)
{
...
bool BoolToBV::needToRebuild(TNode n)
{
...
Node BoolToBV::lowerAssertion(TNode a)
{
  bool optionITE = options::boolToBitvector() == preprocessing::passes::BOOL_TO_BV_ITE;
  NodeManager* nm = NodeManager::currentNM();
  std::vector<TNode> terms;
  terms.push_back(a);
  std::unordered_set<TNode, TNodeHashFunction> visited;
  // for optionITE=true, keeps track of whether you're in an ite condition
  ...
  while(!terms.empty())
  {
    ...
    {
      Debug("bool-to-bv") << "BoolToBV::lowerAssertion Skipping because don't need to rebuild: " << n << std::endl;
    ...
    else
    {
      ...
      if ((t.isBitVector() || t.isBoolean()) && (n.getNumChildren() == 0))
      {
        ...
        Debug("bool-to-bv") << "BoolToBV::lowerAssertion Skipping because no children: " << n << std::endl;
      }
      else
      {
...
BoolToBV::Statistics::Statistics()
  : d_numIteToBvite("preprocessing::passes::BoolToBV::NumIteToBvite", 0),
    ...
    d_numTermsForcedLowered("preprocessing::passes::BoolToBV::NumTermsForcedLowered", 0)
{
