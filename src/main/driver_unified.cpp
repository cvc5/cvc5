  ...
  *(opts.getOut()) << unitbuf;
  ...
  int returnValue = 0;
  {
    ...
    if(opts.getInteractive() && inputFromStdin) {
      ...
      InteractiveShell shell(pExecutor->getSolver(), pExecutor->getSymbolManager());
      ...
      ParserBuilder parserBuilder(pExecutor->getSolver(), pExecutor->getSymbolManager(), filename, opts);
