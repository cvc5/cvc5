...
{
  if (input == "cnf") return TptpDialect::CNF;
  if (input == "fof") return TptpDialect::FOF;
  if (input == "tff") return TptpDialect::TFF;
  if (input == "thf") return TptpDialect::THF;
  return TptpDialect::AUTO;
}

TptpModel::TptpModel(bool isKnownSat,
                     const std::string& inputName,
                     bool tptpModelVerification,
                     TptpDialect tptpInputDialect)
    : smt::Model(isKnownSat, inputName),
      d_tptpModelVerification(tptpModelVerification),
      d_tptpInputDialect(tptpInputDialect)
{
}

Node TptpModel::getValueOrNull(TNode n) const
{
...
void TptpModel::setEvaluator(const std::function<Node(TNode)>& eval)
{
  d_evaluator = eval;
}
    ...
    size_t maxFuncSpace)
{
  ...
  if (!getFiniteDomainValues(
          tn.getRangeType(), nm, baseElems, cache, maxFuncSpace))
  {
    return false;
  }
  ...
  bool okPow = false;
  size_t funcCount =
      boundedPow(rangeVals.size(), tupleCount, maxFuncSpace, okPow);
...
bool tryGetModelValue(const TptpModel& m,
...
Node resolveModelValue(const TptpModel& m,
                       const std::map<Node, bool>& isDeclared,
                       Node n)
{
  Node cur = n;
...
Node evaluateAppValue(const TptpModel& m,
                      const std::map<Node, bool>& isDeclared,
                      Node fun,
...
std::string resolveElemName(const TptpModel& m,
                            const std::map<Node, bool>& isDeclared,
                            Node value,
                     ...
                     std::string& out)
{
  ...
  if (eit != elemNames.end() && n.getType().isUninterpretedSort())
  {
    ...
    out = mkApplyString(
        pit->second, std::vector<std::string>{eit->second}, useThfTuple);
    return true;
  }
  if (isDeclared.find(n) != isDeclared.end())
  {
...
void Smt2TptpPrinter::toStream(std::ostream& out, const smt::Model& m) const
{
  ...
  bool useThf = tm.useTptpModelVerification()
                && tm.getTptpInputDialect() == TptpDialect::THF;
  for (const Node& t : terms)
  {
  ...
  if (!useThf)
  {
    for (const std::pair<const TypeNode, std::vector<Node>>& p :
         finiteTypeElems)
    {
  ...
  for (const Node& t : terms)
  {
    ...
    Node tv = resolveModelValue(tm, isDeclared, t);
    if (tv.isNull())
    {
    ...
    std::string rhs;
    if (!modelNodeToTptp(
            tv, isDeclared, elemNames, promoteNames, useThf, bvarNames, rhs))
    {
      needHoFiniteDomain = true;
      continue;
    }
  ...
  for (const TypeNode& s : sorts)
  {
    ...
    std::string vS =
        sanitizeUpper(sN.substr(0, 1));  // variable of the original sort
  ...
  for (const Node& t : terms)
  {
    ...
    if (!tt.isFunction())
    {
      ...
      if (tt.isBoolean())
      {
        ...
        const bool hasFormulaValue = !v.isNull() && v != t
                                     && v.getKind() != Kind::CONST_BOOLEAN
                                     && modelNodeToTptp(v,
                                                        isDeclared,
                                                        elemNames,
                                                        promoteNames,
      ...
      std::string evn = resolveElemName(tm, isDeclared, v, elemNames);
      if (evn.empty())
      {
        continue;
      }
      ...
      ss << "( " << tn << " = "
         << mkApplyString(
                promoteNames[tt], std::vector<std::string>{evn}, useThf)
         << " )";
    ...
    for (const std::vector<Node>& tup : tuples)
    {
      ...
      Node val;
      if (!tryGetModelValue(tm, isDeclared, app, val))
      {
        val = evaluateAppValue(tm, isDeclared, t, tup);
        if (val.isNull())
        {
          std::map<Node, Node> emptySubst;
          val = evalFiniteTerm(app, tm, isDeclared, finiteTypeElems, emptySubst);
          if (val.isNull())
          {
            continue;
          }
      ...
      if (!isPred)
      {
        vname = resolveElemName(tm, isDeclared, val, elemNames);
        if (vname.empty())
        {
