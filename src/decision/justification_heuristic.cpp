...
CVC4::prop::SatLiteral JustificationHeuristic::getNextThresh(bool &stopSearch, DecisionWeight threshold) {
  ...
  // We have to make sure that we assign a value to all the Boolean term
  // variables. To illustrate why this is, consider the case where we have a
  // single assertion
  //
  // (or (f a) (f b))
  //
  ...
  for (const std::pair<const Node, Node>& skAssert : d_skolemAssertions)
  {
    if (skAssert.first.getKind() == kind::BOOLEAN_TERM_VARIABLE)
    {
      SatLiteral litDecision;
      litDecision = findSplitter(skAssert.second, SAT_VALUE_TRUE);
      if (litDecision != undefSatLiteral)
      {
        Trace("decision") << "jh: splitting on " << litDecision << std::endl;
        ++d_helpfulness;
