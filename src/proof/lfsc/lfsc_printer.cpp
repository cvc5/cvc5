...
void LfscPrinter::print(std::ostream& out, const std::vector<Node>& assertions, const ProofNode* pn)
{
  // closing parentheses
...
void LfscPrinter::printProofLetify(std::ostream& out, const ProofNode* pn, std::map<Node, uint32_t>& letMap, std::map<Node, uint32_t>& passumeMap)
{
  // closing parentheses
  std::stringstream cparen;
...
void LfscPrinter::printProofInternal(std::ostream& out, 
                                     const ProofNode* pn, 
                                     std::map<Node, uint32_t>& letMap,
                                     std::map<const ProofNode*, uint32_t>& pletMap, std::map<Node, uint32_t>& passumeMap)
{
  // the stack
...
void LfscPrinter::computeProofArgs(const ProofNode* pn, std::vector<PExpr>& pargs)
{
  // TODO
}
...
void LfscPrinter::printInternal(std::ostream& out, Node n, const std::map<Node, uint32_t>& letMap)
{
...
void LfscPrinter::computeLet(Node n, std::vector<Node>& letList, std::map<Node, uint32_t>& letMap, uint32_t& counter)
{
...
void LfscPrinter::computeCounts(Node n, std::vector<Node>& visitList, std::map<Node, uint32_t>& count)
{
  std::map<Node, uint32_t>::iterator it;
...
void LfscPrinter::convertCountToLet(const std::vector<Node>& visitList, const std::map<Node, uint32_t>& count, std::vector<Node>& letList, std::map<Node, uint32_t>& letMap, uint32_t& counter)
{
  Assert (letList.empty() && letMap.empty());
  ...
  for (std::vector<Node>::const_reverse_iterator it = visitList.rbegin();  it != visitList.rend(); ++it)
  {
    itc = count.find( *it);
...
void LfscPrinter::computeProofLet(const ProofNode* pn, std::vector<const ProofNode *>& pletList, std::map<const ProofNode*, uint32_t>& pletMap, uint32_t& pcounter)
{
...
void LfscPrinter::computeProofCounts(const ProofNode* pn, std::vector<const ProofNode *>& visitList, std::map<const ProofNode*, uint32_t>& pcount)
{
...
void LfscPrinter::convertProofCountToLet(const std::vector<const ProofNode *>& visitList, const std::map<const ProofNode*, uint32_t>& pcount, std::vector<const ProofNode *>& pletList, std::map<const ProofNode*, uint32_t>& pletMap, uint32_t& pcounter)
{
  Assert (pletList.empty() && pletMap.empty());
  // Assign ids for those whose count is > 1, traverse in reverse order
  // so that deeper proofs are assigned lower identifiers
  std::map<const ProofNode*, uint32_t>::const_iterator itc;
  for (std::vector<const ProofNode*>::const_reverse_iterator itp = visitList.rbegin();  itp != visitList.rend(); ++itp)
  {
    itc = pcount.find( *itp);
