...
Node ProofArray::toStreamRecLFSC(std::ostream& out,
  ...
  if(tb == 0) {
    ...
    if (options::eagerArrayProofs()) {
      // Eager proofs generated by conflicts are okay. Eager proofs generated by calls to explain() need some work.
      Assert((pf->d_node.getKind() == kind::IFF) ||
             (pf->d_node.getKind() == kind::EQUAL) ||
             ((pf->d_node.getKind() == kind::NOT) &&
              ((pf->d_node[0].getKind() == kind::IFF) ||
               (pf->d_node[0].getKind() == kind::EQUAL))));

      if (pf->d_node[0].getKind() == kind::NOT ||
          (!((pf->d_node[0] == NodeManager::currentNM()->mkConst(true) &&
        ...
        // The main proof is not true=false. This should only happen from calls to explain().
        // Transform the proof into the standard format
        Debug("pf::array") << "In eager mode, and proof was generated by explain(). Correcting it..." << std::endl;

        theory::eq::EqProof* newChild = new theory::eq::EqProof;
        newChild->d_id = theory::eq::MERGED_THROUGH_EQUALITY;
        bool haveExistingNegativeChild = false;

        // Need to negate and add the new child
        if (pf->d_node.getKind() == kind::NOT) {
          // If the root is (not (= a b)), then (= a b) is a pure equality that needs to be
          // added to the transitivity proof. It is not the negative child.
          newChild->d_node = pf->d_node.negate();
          haveExistingNegativeChild = true;
        } else if (pf->d_node[0] == NodeManager::currentNM()->mkConst(false)) {
          newChild->d_node = pf->d_node[1];
          haveExistingNegativeChild = true;
        } else if (pf->d_node[1] == NodeManager::currentNM()->mkConst(false)) {
          newChild->d_node = pf->d_node[0];
          haveExistingNegativeChild = true;
        } else {
          // pf->d_node is the negative child.
          if (pf->d_node[0] == NodeManager::currentNM()->mkConst(true)) {
            newChild->d_node = pf->d_node[1].negate();
          } else if (pf->d_node[1] == NodeManager::currentNM()->mkConst(true)) {
            newChild->d_node = pf->d_node[0].negate();
          } else {
            newChild->d_node = pf->d_node.negate();
          }
        }

        if (haveExistingNegativeChild) {
          std::vector<theory::eq::EqProof *> reorderedChildren;

          // 1. Find the real negative child at index i.
          // 2. Reverse the order of chidlren before i.
          // 3. Add d_node.
          // 4. Add children from n to i+1.
          bool negFound = false;
          int negIndex;

          for (unsigned i = 0; i < pf->d_children.size(); ++i) {
            // We can encounter nodes like (= (= a b) false). Transform to: (not (= a b)).
            if (!pf->d_children[i]->d_node.isNull() && pf->d_children[i]->d_node.getKind() == kind::IFF) {
              if ((pf->d_children[i]->d_node[0] == NodeManager::currentNM()->mkConst(false)) &&
                  (pf->d_children[i]->d_node[1].getKind() == kind::EQUAL)) {
                pf->d_children[i]->d_node = pf->d_children[i]->d_node[1].negate();
              } else if ((pf->d_children[i]->d_node[1] == NodeManager::currentNM()->mkConst(false)) &&
                         (pf->d_children[i]->d_node[0].getKind() == kind::EQUAL)) {
                pf->d_children[i]->d_node = pf->d_children[i]->d_node[0].negate();
              }
            }
          ...
          for (int i = negIndex; i >= 0; --i) {
            reorderedChildren.push_back(pf->d_children[i]);
          }

          reorderedChildren.push_back(newChild);

          for (int i = pf->d_children.size() - 1; i > negIndex; --i) {
            reorderedChildren.push_back(pf->d_children[i]);
          }

          pf->d_children = reorderedChildren;
          pf->d_node = eqNode(NodeManager::currentNM()->mkConst(false), NodeManager::currentNM()->mkConst(true));
        } else {
          pf->d_children.insert(pf->d_children.begin(), newChild);
          pf->d_node = eqNode(NodeManager::currentNM()->mkConst(false), NodeManager::currentNM()->mkConst(true));
        }

        Debug("pf::array") << "New proof: " << std::endl;
        pf->debug_print("pf::array");
        Debug("pf::array") << std::endl;
      }
    }
