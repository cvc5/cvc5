                                           ...
                                           bool& continueUpdate)
{
  ...
  switch (id)
  {
    ...
    //
    // E.g. (P1:(cl F1)) and (P2:(cl(not (or F1)))) cannot be resolved. But P1 will have a child (cl (or F1)) in this case and was generated by an application of the or rule. Therefore, this needs to be the child
    //
    ...
    case PfRule::RESOLUTION:{
      ...
      if (res.getKind() == kind::OR
          && ((num_children1 == 1 && num_children2 == 2)
              || (num_children1 == 2 && num_children2 == 1)))
      {
    ...
    // t1: (cl (or a (not a)))
    // t2: (cl a (not a)) :rule or :premises t1
    // t3: (cl (not (or a (not a))))
    // t4: (cl) :rule resolution :premises t2 t3
    //
    ...
    case PfRule::CHAIN_RESOLUTION: //TODO: This rule changed, also consider order, special case
    {
      ...
      for(int i = 0; i < children.size(); i++){ //
		...
		new_children.push_back(children[i][0]);
	}
	else{
    ...
    //
    //  If C1 = (or (or F1 ... Fn) ... (or F1 ... Fn)) and C2 = (or F1 ... Fn) then:
    //
    //  proof rule: duplicated_literals
    //  proof node: (VP:(cl C2))
    ...
    case PfRule::FACTORING:
    {
	 ...
	 if(*i != res){special_case=false;}
      }
      if(special_case){
	Node vp = d_nm->mkNode(kind::SEXPR,d_cl,res);
        return addVeritStepFromOr(vp,VeritRule::DUPLICATED_LITERALS,children,{},*cdp)
		&& addVeritStepFromOr(res,VeritRule::OR,{vp},{},*cdp);
      }
    ...
    //
    // TODO: ASK HANIEL IF THIS SHOULD BE DONE, MIGHT NOT MAKE SENSE TO STORE THIS INFORMATION
    ...
    case PfRule::SPLIT:
    {
      Node vp1 = d_nm->mkNode(kind::SEXPR, d_cl, args[0].notNode().notNode().notNode(), args[0]);
      Node vp2 = d_nm->mkNode(kind::SEXPR, d_cl, args[0].notNode().notNode().notNode().notNode(), args[0].notNode());
    ...
    {
      Node vp1 = d_nm->mkNode(kind::SEXPR, d_cl, children[1].negate(), res);
      std::vector<Node> new_children;
    ...
    case PfRule::MODUS_PONENS:
    {
      ...
      bool success = addVeritStep(vp1, VeritRule::IMPLIES, {children[1]}, {}, *cdp);
    ...
    // proof node: (VP1:(cl (or (not (forall ((x1 T1) ... (xn Tn)) F)) F*sigma)))
    // proof term: (cl (or (not (forall ((x1 T1) ... (xn Tn)) F)) F*sigma))
    // premises: ()
    // args: (= x1 t1) ... (= xn tn)
    //
    // proof rule: or
    ...
    // premises: (VP2:(cl (not (forall ((x1 T1) ... (xn Tn)) F)) F*sigma)) (P:(forall ((x1 T1) ... (xn Tn)) F))
    // args: ()
    //
    // proof rule: or
    // proof node: F*sigma
    ...
    case PfRule::INSTANTIATE:{
      ...
      for(int i=0;i<args.size();i++){
        new_args.push_back(d_nm->mkNode(kind::EQUAL,children[0][1][i],args[i]));
      }
      Node vp1 = d_nm->mkNode(kind::SEXPR,d_cl,d_nm->mkNode(kind::OR,children[0].negate(),res));
      bool success = addVeritStep(vp1,VeritRule::FORALL_INST,{},new_args,*cdp);
      Node vp2 = d_nm->mkNode(kind::SEXPR,d_cl,children[0].negate(),res);
...
void VeritProofPostprocess::processInternal(std::shared_ptr<ProofNode> pf,
                                            CDProof* cdp)
{
  for (const std::shared_ptr<ProofNode>& child : pf->getChildren()) //I stopped here yesterday, move this up?
  {
    if(!d_extended && (child->getRule() == PfRule::SYMM || child->getRule() == PfRule::REORDERING)){
       processInternal(child->getChildren()[0],cdp);
    }
    else{
  ...
  if (d_cb->shouldUpdate(pf, continueUpdate))
  {
    ...
    for (const std::shared_ptr<ProofNode>& child : pf->getChildren())
    {
	  ...
	  cdp->addProof(child->getChildren()[0]);
      }
      else
      {
