                                           ...
                                           bool& continueUpdate)
{
  ...
  switch (id)
  {
    ...
    //
    // E.g. (P1:(cl F1)) and (P2:(cl(not (or F1)))) cannot be resolved. But P1
    // will have a child (cl (or F1)) in this case and was generated by an
    ...
    case PfRule::CHAIN_RESOLUTION: //TODO: This rule changed, also consider order, special case
    {
      ...
      return addVeritStep(
          res, VeritRule::RESOLUTION, conclusion, new_children, {}, *cdp);
    }
    // ======== Factoring
    // Children: (P:C1)
    // Arguments: ()
    ...
    // F*sigma))) proof term: (cl (or (not (forall ((x1 T1) ... (xn Tn)) F))
    // F*sigma)) premises: () args: (= x1 t1) ... (= xn tn)
    //
    // proof rule: or
    ...
    case PfRule::INSTANTIATE:
    {
      ...
      {
        new_args.push_back(
            d_nm->mkNode(kind::EQUAL, children[0][1][i], args[i]));
      }
      Node vp1 = d_nm->mkNode(
...
void VeritProofPostprocess::processInternal(std::shared_ptr<ProofNode> pf,
                                            CDProof* cdp)
{
  for (const std::shared_ptr<ProofNode>& child :
       pf->getChildren())  // I stopped here yesterday, move this up?
  {
    if (!d_extended
        && (child->getRule() == PfRule::SYMM
            || child->getRule() == PfRule::REORDERING))
    {
