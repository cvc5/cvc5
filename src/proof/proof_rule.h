...
enum class PfRule : uint32_t
{
   ...
   *
   * We rewrite only on the Skolem form of :math:`F`, similar to :cpp:enumerator:`MACRO_SR_EQ_INTRO <cvc5::PfRule::MACRO_SR_EQ_INTRO>`.
   * \endverbatim
   */
  MACRO_SR_PRED_ELIM,
  /**
   ...
   *
   * where :math:`\texttt{Rewriter}_{idr}(F \circ \sigma_{ids, ida}(F_n) \circ \cdots \circ \sigma_{ids, ida}(F_1)) = \texttt{Rewriter}_{idr}(G \circ \sigma_{ids, ida}(F_n) \circ \cdots \circ \sigma_{ids, ida}(F_1))`.
   *
   * More generally, this rule also holds when:
   * :math:`\texttt{Rewriter::rewrite}(\texttt{toOriginal}(F')) = \texttt{Rewriter::rewrite}(\texttt{toOriginal}(G'))`
   * where :math:`F'` and :math:`G'` are the result of each side of the equation above. Here,
   * original forms are used in a similar manner to :cpp:enumerator:`MACRO_SR_PRED_INTRO <cvc5::PfRule::MACRO_SR_PRED_INTRO>` above.
   * \endverbatim
   */
   ...
   *
   * The terms :math:`a_1 \dots a_n` can be anything used to annotate the proof node, one
   * example is where :math:`a_1` is a theory::InferenceId.
   * \endverbatim
   */
  ANNOTATION,
  /**
   ...
   *
   * where :math:`F` is a (T-valid) theory lemma generated by theory with TheoryId :math:`tid`.
   ...
   *
   * where :math:`F` is an equality of the form :math:`t = t'` where :math:`t'` is obtained by
   * applying the kind of rewriting given by the method identifier :math:`rid`, which
   * is one of:
   ...
   *   \inferrule{- \mid F}{F}
   *
   * where :math:`F` is an equality of the form :math:`t = t'` where :math:`t` was replaced by :math:`t'`
   * based on some preprocessing pass, or otherwise :math:`F` was added as a new
   ...
   *   \inferrule{- \mid F}{F}
   *
   * where :math:`F` is an existential ``(exists ((x T)) (P x))`` used for introducing
   ...
   *   \inferrule{- \mid F}{F}
   *
   * where :math:`F` is an equality `t = t'` that holds by a form of rewriting that
   ...
   *   \inferrule{- \mid F}{F}
   *
   * where :math:`F` is an equality :math:`t = t'` that holds by a form of substitution that
   * could not be replayed during proof postprocessing.
   * \endverbatim
   */
  TRUST_SUBS,
   ...
   *   \inferrule{F' \mid F}{F}
   *
   * where :math:`F` is a solved equality of the form :math:`x = t` derived as the solved
   * form of :math:`F'`.
   * \endverbatim
   */
  TRUST_SUBS_EQ,
