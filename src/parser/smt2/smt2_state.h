...
class Smt2State : public ParserState
{
 public:
   ...
   */
  std::unique_ptr<CommandInternal> invConstraint(const std::vector<std::string>& names);
   ...
   */
  std::unique_ptr<CommandInternal> handlePush(std::optional<uint32_t> nscopes);
  /**
   * Handles a pop command.
   *
   * @return An instance of `PopCommand`
   */
  std::unique_ptr<CommandInternal> handlePop(std::optional<uint32_t> nscopes);
