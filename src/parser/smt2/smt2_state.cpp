...
std::unique_ptr<CommandInternal> Smt2State::invConstraint(
    const std::vector<std::string>& names)
{
  ...
  return std::unique_ptr<CommandInternal>(new SygusInvConstraintCommand(terms));
}
...
std::unique_ptr<CommandInternal> Smt2State::handlePush(std::optional<uint32_t> nscopes)
{
  checkThatLogicIsSet();
...
std::unique_ptr<CommandInternal> Smt2State::handlePop(std::optional<uint32_t> nscopes)
{
  checkThatLogicIsSet();
