...
.. code:: bash
    ...
    cmake --build .           # use -jN for parallel build with N threads
    cmake --build . -t check  # to run default set of tests
    cmake --install .         # to install into the prefix specified above
...
install the required packages for building cvc5 (you can install `ninja` instead of `make` if you prefer):
...
run ``cmake --build . -t docs`` from within the build directory.
...
call ``cmake --build . -t docs-gh``. The content of directory ``<build_dir>/docs/sphinx-gh``
...
.. code::
    ...
    cmake --build . -t apitests           # build and run all system tests
    cmake --build . -t <api_test>         # build test/system/<system_test>.<ext>
...
.. code::
    ...
    cmake --build . -t ouroborous         # build test/api/ouroborous.cpp
...
.. code::
    ...
    cmake --build . -t units              # build and run all unit tests
    cmake --build . -t <unit_test>        # build test/unit/<subdir>/<unit_test>.<ext>
...
.. code::
    ...
    cmake --build . -t map_util_black     # build test/unit/base/map_util_black.cpp
...
- ``cmake --build . -t systemtests [-jN]``
  Build and run all system tests.

- ``cmake --build . -t units [-jN]``
  Build and run all unit tests.

- ``cmake --build . -t regress [-jN]``
  Build and run regression tests from levels 0 to 2.

- ``cmake --build . -t runexamples [-jN]``
  Build and run all examples.

- ``cmake --build . -t coverage-test [-jN]``
