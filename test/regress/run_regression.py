...
def run_benchmark(dump, wrapper, scrubber, error_scrubber, cvc4_binary, command_line,
    ...
    if dump:
        dump_args = [
            '--preprocess-only', '--dump=clauses', '--output-lang=smt2', '-q'
        ]
...
def main(proof, dump, wrapper, cvc4_binary, benchmark_path):
    ...
    if expected_output == '' and expected_error == '':
        ...
        if status_regex is not None:
            m = re.search(status_regex, metadata_content)

        if m is not None:
            expected_output = status_to_output(m.group(1))
    ...
    if len(extra_command_line_args) > 0:
        command_line_args_configs.append(basic_command_line_args +
                                         extra_command_line_args)
...
if __name__ == "__main__":
    ...
    parser.add_argument('wrapper', nargs='*', default=None)
    ...
    if os.environ.get('VALGRIND') == '1' and len(wrapper) == 0:
        wrapper = ['libtool', '--mode=execute', 'valgrind']

    main(args.proof, args.dump, wrapper, cvc4_binary, args.benchmark)
