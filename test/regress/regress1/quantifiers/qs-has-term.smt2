...
(assert (not (= def use)))
(assert (not (=> (and (and (>= pv1374 0) (<= pv1374 3)) (forall ((?A_59 Int)) (=> (and (>= ?A_59 0) (<= ?A_59 2)) (forall ((?B_60 Int)) (=> (and (>= ?B_60 0) (<= ?B_60 (- pv1374 1))) (= (select (select simplex7_init ?B_60) ?A_59) init)))))) (and (= init init) (forall ((?C_61 Int)) (=> (and (>= ?C_61 0) (<= ?C_61 2)) (forall ((?D_62 Int)) (let ((?v_0 (store simplex7_init pv1374 (store (select simplex7_init pv1374) 0 init)))) (let ((?v_1 (store ?v_0 pv1374 (store (select ?v_0 pv1374) 1 init)))) (=> (and (>= ?D_62 0) (<= ?D_62 (- (+ 1 pv1374) 1))) (= (select (select (store ?v_1 pv1374 (store (select ?v_1 pv1374) 2 init)) ?D_62) ?C_61) init)))))))))))
(check-sat)
(exit)
