; COMMAND-LINE: --tptp-models --tptp-model-verification --force-logic=HO_ALL --mbqi
; EXPECT: sat
; EXPECT: %--------------------------------------------------------
; EXPECT: thf(human_type,type,
; EXPECT:     human: $tType ).
; EXPECT:
; EXPECT: thf(cat_type,type,
; EXPECT:     cat: $tType ).
; EXPECT:
; EXPECT: thf(jon_decl,type,
; EXPECT:     jon: human ).
; EXPECT:
; EXPECT: thf(garfield_decl,type,
; EXPECT:     garfield: cat ).
; EXPECT:
; EXPECT: thf(arlene_decl,type,
; EXPECT:     arlene: cat ).
; EXPECT:
; EXPECT: thf(nermal_decl,type,
; EXPECT:     nermal: cat ).
; EXPECT:
; EXPECT: thf(loves_decl,type,
; EXPECT:     loves: cat > cat ).
; EXPECT:
; EXPECT: thf(owns_decl,type,
; EXPECT:     owns: human > cat > $o ).
; EXPECT:
; EXPECT: %----Types of the domains
; EXPECT: thf(d_human_type,type,
; EXPECT:     d_human: $tType ).
; EXPECT:
; EXPECT: thf(d_cat_type,type,
; EXPECT:     d_cat: $tType ).
; EXPECT:
; EXPECT: %----Types of the promotion functions
; EXPECT: thf(d2human_decl,type,
; EXPECT:     d2human: d_human > human ).
; EXPECT:
; EXPECT: thf(d2cat_decl,type,
; EXPECT:     d2cat: d_cat > cat ).
; EXPECT:
; EXPECT: %----Types of the domain elements
; EXPECT: thf(d_jon_decl,type,
; EXPECT:     d_jon: d_human ).
; EXPECT:
; EXPECT: thf(d_garfield_decl,type,
; EXPECT:     d_garfield: d_cat ).
; EXPECT:
; EXPECT: thf(d_arlene_decl,type,
; EXPECT:     d_arlene: d_cat ).
; EXPECT:
; EXPECT: thf(d_nermal_decl,type,
; EXPECT:     d_nermal: d_cat ).
; EXPECT:
; EXPECT: thf(garfield,interpretation,
; EXPECT:     ( ! [H: human] :
; EXPECT:       ? [DH: d_human] :
; EXPECT:         ( H = ( d2human @ DH ) )
