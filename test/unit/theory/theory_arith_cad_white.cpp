 ...
 **
 ** \brief Unit tests for the CAD module for nonlinear arithmetic
 **
...
poly::AlgebraicNumber get_ran(std::initializer_list<long> init, int lower, int upper)
{
  ...
  Node make_real_variable(const std::string& s) {
    return nodeManager->mkSkolem(s, nodeManager->realType(), "", NodeManager::SKOLEM_EXACT_NAME);
  }
  Node make_int_variable(const std::string& s) {
    return nodeManager->mkSkolem(s, nodeManager->integerType(), "", NodeManager::SKOLEM_EXACT_NAME);
  }
...
TEST_F(TestTheoryWhiteArithCAD, test_projection)
  {
    ...
    EXPECT_EQ(res[4], x*x*x*x*x + 2*x*x*x*x - 2*x*x*x - 5*x*x - 7*x -14);
  }
...
TEST_F(TestTheoryWhiteArithCAD, lazard_simp)
{
  ...
  Node c = d_nodeManager->mkVar(*d_realType);
    Node orig = d_nodeManager->mkAnd(std::vector<Node>{
        ...
        d_nodeManager->mkNode(Kind::EQUAL, 
