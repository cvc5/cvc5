...
TEST_F(TestTheoryWhiteyQuantifiersBvInstantiator, getPvCoeff)
{
  ...
  Node x = mkVar(d_nodeManager, 32);
  Node a = mkVar(d_nodeManager, 32);
  Node one = mkOne(d_nodeManager, 32);
...
TEST_F(TestTheoryWhiteyQuantifiersBvInstantiator, normalizePvMult)
{
  ...
  Node neg_x = mkNeg(x);
  Node a = mkVar(d_nodeManager, 32);
  Node b = mkVar(d_nodeManager, 32);
  Node c = mkVar(d_nodeManager, 32);
  Node d = mkVar(d_nodeManager, 32);
  Node zero = mkZero(d_nodeManager, 32);
  Node one = mkOne(d_nodeManager, 32);
  BvLinearAttribute is_linear;
...
TEST_F(TestTheoryWhiteyQuantifiersBvInstantiator, normalizePvEqual)
{
  ...
  Node zero = mkZero(d_nodeManager, 32);
  Node one = mkOne(d_nodeManager, 32);
  Node ntrue = mkTrue(d_nodeManager);
  BvLinearAttribute is_linear;
  std::unordered_map<Node, bool> contains_x;
  ...
  ASSERT_EQ(norm_neg_xx[0][1], rr->rewrite(mkConst(d_nodeManager, 32, 2)));
  ...
  ASSERT_EQ(norm_neg_axx[0][1],
            rr->rewrite(mkNeg(mkConst(d_nodeManager, 32, 2))));
