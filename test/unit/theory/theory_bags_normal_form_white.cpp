...
TEST_F(TestTheoryWhiteBagsNormalForm, bag_count)
{
  ...
  Node z_5 = d_nodeManager->mkNode(
      Kind::BAG_MAKE, z, d_nodeManager->mkConstInt(Rational(5)));

  Node input1 = d_nodeManager->mkNode(Kind::BAG_COUNT, x, empty);
  Node output1 = zero;
  ASSERT_EQ(output1, rewrite(input1));

  Node input2 = d_nodeManager->mkNode(Kind::BAG_COUNT, x, y_5);
  Node output2 = zero;
  ASSERT_EQ(output2, rewrite(input2));

  Node input3 = d_nodeManager->mkNode(Kind::BAG_COUNT, x, x_4);
  Node output3 = four;
  ASSERT_EQ(output2, rewrite(input2));

  Node unionDisjointXY =
      d_nodeManager->mkNode(Kind::BAG_UNION_DISJOINT, x_4, y_5);
  Node input4 = d_nodeManager->mkNode(Kind::BAG_COUNT, x, unionDisjointXY);
  Node output4 = four;
  ASSERT_EQ(output3, rewrite(input3));

  Node unionDisjointYZ =
      d_nodeManager->mkNode(Kind::BAG_UNION_DISJOINT, y_5, z_5);
  Node input5 = d_nodeManager->mkNode(Kind::BAG_COUNT, x, unionDisjointYZ);
  Node output5 = zero;
...
TEST_F(TestTheoryWhiteBagsNormalForm, union_max)
{
  ...
  Node x_3 = d_nodeManager->mkNode(
      Kind::BAG_MAKE, x, d_nodeManager->mkConstInt(Rational(3)));
  Node x_7 = d_nodeManager->mkNode(
      Kind::BAG_MAKE, x, d_nodeManager->mkConstInt(Rational(7)));
  Node z_2 = d_nodeManager->mkNode(
      Kind::BAG_MAKE, z, d_nodeManager->mkConstInt(Rational(2)));
  Node y_1 = d_nodeManager->mkNode(
      Kind::BAG_MAKE, y, d_nodeManager->mkConstInt(Rational(1)));

  Node A = d_nodeManager->mkNode(Kind::BAG_UNION_DISJOINT, x_4, z_2);
  Node B = d_nodeManager->mkNode(Kind::BAG_UNION_DISJOINT, x_3, y_1);
  Node input = d_nodeManager->mkNode(Kind::BAG_UNION_MAX, A, B);

  // output
  Node output = d_nodeManager->mkNode(
      Kind::BAG_UNION_DISJOINT,
      x_4,
      d_nodeManager->mkNode(Kind::BAG_UNION_DISJOINT, y_1, z_2));
...
TEST_F(TestTheoryWhiteBagsNormalForm, union_disjoint1)
{
  ...
  Node A = d_nodeManager->mkNode(
      Kind::BAG_MAKE, elements[0], d_nodeManager->mkConstInt(Rational(2)));
  Node B = d_nodeManager->mkNode(
      Kind::BAG_MAKE, elements[1], d_nodeManager->mkConstInt(Rational(3)));
  Node C = d_nodeManager->mkNode(
      Kind::BAG_MAKE, elements[2], d_nodeManager->mkConstInt(Rational(4)));
  ...
  Node unionDisjointAB_C =
      d_nodeManager->mkNode(Kind::BAG_UNION_DISJOINT, unionDisjointAB, C);
  Node unionDisjointBC = d_nodeManager->mkNode(Kind::BAG_UNION_DISJOINT, B, C);
  Node unionDisjointA_BC =
      d_nodeManager->mkNode(Kind::BAG_UNION_DISJOINT, A, unionDisjointBC);
  ...
  Node unionDisjointAA = d_nodeManager->mkNode(Kind::BAG_UNION_DISJOINT, A, A);
...
TEST_F(TestTheoryWhiteBagsNormalForm, union_disjoint2)
{
  ...
  Node A = d_nodeManager->mkNode(Kind::BAG_UNION_DISJOINT, x_4, z_2);
  Node B = d_nodeManager->mkNode(Kind::BAG_UNION_DISJOINT, x_3, y_1);
  Node input = d_nodeManager->mkNode(Kind::BAG_UNION_DISJOINT, A, B);
...
TEST_F(TestTheoryWhiteBagsNormalForm, intersection_min)
{
  ...
  Node A = d_nodeManager->mkNode(Kind::BAG_UNION_DISJOINT, x_4, z_2);
  Node B = d_nodeManager->mkNode(Kind::BAG_UNION_DISJOINT, x_3, y_1);
  Node input = d_nodeManager->mkNode(Kind::BAG_INTER_MIN, A, B);
...
TEST_F(TestTheoryWhiteBagsNormalForm, difference_subtract)
{
  ...
  Node A = d_nodeManager->mkNode(Kind::BAG_UNION_DISJOINT, x_4, z_2);
  Node B = d_nodeManager->mkNode(Kind::BAG_UNION_DISJOINT, x_3, y_1);
  Node input = d_nodeManager->mkNode(Kind::BAG_DIFFERENCE_SUBTRACT, A, B);

  // output
  Node output = d_nodeManager->mkNode(Kind::BAG_UNION_DISJOINT, x_1, z_2);
...
TEST_F(TestTheoryWhiteBagsNormalForm, difference_remove)
{
  ...
  Node A = d_nodeManager->mkNode(Kind::BAG_UNION_DISJOINT, x_4, z_2);
  Node B = d_nodeManager->mkNode(Kind::BAG_UNION_DISJOINT, x_3, y_1);
  Node input = d_nodeManager->mkNode(Kind::BAG_DIFFERENCE_REMOVE, A, B);
...
TEST_F(TestTheoryWhiteBagsNormalForm, bag_card)
{
  ...
  Node input1 = d_nodeManager->mkNode(Kind::BAG_CARD, empty);
  Node output1 = d_nodeManager->mkConstInt(Rational(0));

  ASSERT_EQ(output1, rewrite(input1));

  Node input2 = d_nodeManager->mkNode(Kind::BAG_CARD, x_4);
  Node output2 = d_nodeManager->mkConstInt(Rational(4));
  ASSERT_EQ(output2, rewrite(input2));

  Node union_disjoint =
      d_nodeManager->mkNode(Kind::BAG_UNION_DISJOINT, x_4, y_1);
  Node input3 = d_nodeManager->mkNode(Kind::BAG_CARD, union_disjoint);
