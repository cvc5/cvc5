...
TEST_F(TestApiBlackTerm, getInteger)
{
  ...
  ASSERT_THROW(d_tm.mkInteger("-"), CVC5ApiException);
  ASSERT_THROW(d_tm.mkInteger("-1-"), CVC5ApiException);
  ASSERT_THROW(d_tm.mkInteger("0.0"), CVC5ApiException);
  ASSERT_THROW(d_tm.mkInteger("-0.1"), CVC5ApiException);
  ASSERT_THROW(d_tm.mkInteger("012"), CVC5ApiException);
  ASSERT_THROW(d_tm.mkInteger("0000"), CVC5ApiException);
  ASSERT_THROW(d_tm.mkInteger("-01"), CVC5ApiException);
  ASSERT_THROW(d_tm.mkInteger("-00"), CVC5ApiException);
...
TEST_F(TestApiBlackTerm, getString)
{
  ...
  ASSERT_EQ(s1.getStringValue(), U"abcde");
}
...
{
  Sort fS = d_tm.mkFiniteFieldSort("7");
  Term fV = d_tm.mkFiniteFieldElem("1", fS);
...
{
  ASSERT_FALSE(d_tm.mkInteger(15).isRoundingModeValue());
  ASSERT_TRUE(d_tm.mkRoundingMode(RoundingMode::ROUND_NEAREST_TIES_TO_EVEN)
                  .isRoundingModeValue());
  ASSERT_FALSE(d_tm.mkConst(d_tm.getRoundingModeSort()).isRoundingModeValue());
}

TEST_F(TestApiBlackTerm, getRoundingModeValue)
{
  ASSERT_THROW(d_tm.mkInteger(15).getRoundingModeValue(), CVC5ApiException);
  ASSERT_EQ(d_tm.mkRoundingMode(RoundingMode::ROUND_NEAREST_TIES_TO_EVEN)
                .getRoundingModeValue(),
            RoundingMode::ROUND_NEAREST_TIES_TO_EVEN);
  ASSERT_EQ(d_tm.mkRoundingMode(RoundingMode::ROUND_TOWARD_POSITIVE)
                .getRoundingModeValue(),
            RoundingMode::ROUND_TOWARD_POSITIVE);
  ASSERT_EQ(d_tm.mkRoundingMode(RoundingMode::ROUND_TOWARD_NEGATIVE)
                .getRoundingModeValue(),
            RoundingMode::ROUND_TOWARD_NEGATIVE);
  ASSERT_EQ(d_tm.mkRoundingMode(RoundingMode::ROUND_TOWARD_ZERO)
                .getRoundingModeValue(),
            RoundingMode::ROUND_TOWARD_ZERO);
  ASSERT_EQ(d_tm.mkRoundingMode(RoundingMode::ROUND_NEAREST_TIES_TO_AWAY)
                .getRoundingModeValue(),
...
TEST_F(TestApiBlackTerm, getFloatingPoint)
{
  ...
  ASSERT_TRUE(d_tm.mkFloatingPointPosZero(5, 11).isFloatingPointPosZero());
  ASSERT_TRUE(d_tm.mkFloatingPointNegZero(5, 11).isFloatingPointNegZero());
  ASSERT_TRUE(d_tm.mkFloatingPointPosInf(5, 11).isFloatingPointPosInf());
  ASSERT_TRUE(d_tm.mkFloatingPointNegInf(5, 11).isFloatingPointNegInf());
  ASSERT_TRUE(d_tm.mkFloatingPointNaN(5, 11).isFloatingPointNaN());
}
...
TEST_F(TestApiBlackTerm, constArray)
{
  ...
  ASSERT_THROW(d_tm.mkConstArray(arrsort, two), CVC5ApiException);
  ASSERT_THROW(d_tm.mkConstArray(arrsort, iconst), CVC5ApiException);

  ASSERT_EQ(constarr.getKind(), Kind::CONST_ARRAY);
  ASSERT_EQ(constarr.getConstArrayBase(), one);
  ASSERT_THROW(a.getConstArrayBase(), CVC5ApiException);

  arrsort = d_tm.mkArraySort(d_tm.getRealSort(), d_tm.getRealSort());
  Term zero_array = d_tm.mkConstArray(arrsort, d_tm.mkReal(0));
  Term stores =
      d_tm.mkTerm(Kind::STORE, {zero_array, d_tm.mkReal(1), d_tm.mkReal(2)});
  stores = d_tm.mkTerm(Kind::STORE, {stores, d_tm.mkReal(2), d_tm.mkReal(3)});
  stores = d_tm.mkTerm(Kind::STORE, {stores, d_tm.mkReal(4), d_tm.mkReal(5)});
}
...
TEST_F(TestApiBlackTerm, getRealAlgebraicNumber)
{
  ...
  Term eq = d_tm.mkTerm(Kind::EQUAL, {x2, two});
