...
TEST_F(TestApiBlackOpt, pushpoplexico)
{
  ...
  std::pair<api::Result, std::vector<OptimizationResult>> rstPair =
      d_solver.checkOpt({{x, Solver::MINIMIZE},
                         {y, Solver::BV_SIGNED_MAXIMIZE},
                         {z, Solver::MAXIMIZE}},
                        Solver::LEXICOGRAPHIC);
  ...
  // check-opt
  rstPair = d_solver.checkOpt({{x, Solver::MINIMIZE}}, Solver::LEXICOGRAPHIC);
...
TEST_F(TestApiBlackOpt, pareto)
{
  ...
  for (int i = 0; i < 3; ++i)
  {
    if (i == 0)
    {
      ...
      rstPair = d_solver.checkOpt(
          {{a, Solver::MAXIMIZE}, {b, Solver::MAXIMIZE}}, Solver::PARETO);
    }
    else
    {
      rstPair = d_solver.nextParetoSolution();
    }
