...
def test_mk_term(solver):
    ...
    solver.mkTerm(d_solver.mkOp(kinds.Pi))
    solver.mkTerm(d_solver.mkOp(kinds.Pi), v6)
    solver.mkTerm(kinds.RegexpNone)
    solver.mkTerm(kinds.RegexpNone, v6)
    solver.mkTerm(d_solver.mkOp(kinds.RegexpNone))
    solver.mkTerm(d_solver.mkOp(kinds.RegexpNone), v6)
    solver.mkTerm(kinds.RegexpAllchar)
    solver.mkTerm(kinds.RegexpAllchar, v6)
    solver.mkTerm(d_solver.mkOp(kinds.RegexpAllchar))
    solver.mkTerm(d_solver.mkOp(kinds.RegexpAllchar), v6)
    solver.mkTerm(kinds.SepEmp)
    solver.mkTerm(kinds.SepEmp, v6)
    solver.mkTerm(d_solver.mkOp(kinds.SepEmp))
    solver.mkTerm(d_solver.mkOp(kinds.SepEmp), v6)
    with pytest.raises(RuntimeError):
